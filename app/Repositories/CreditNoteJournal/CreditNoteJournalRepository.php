...
class CreditNoteJournalRepository extends AbstractValidator implements CreditNoteJournalInterface {
	...
	private function setTrInputValue($attributes, $journalEntryTr, $key) 
	{
		...
		$journalEntryTr->creditnote_jv_id = $this->journal->id;
		...
		//$journalEntryTr->is_print           = isset($attributes['tag'][$key])?$attributes['tag'][$key]:'';
		
			/*$ids = $attributes['tagids'];
			if($ids!=''){
			$cvid=$this->journal->id;
			$idarr = explode(',', $ids);
			//echo '<pre>';print_r($idarr);exit;
			foreach($idarr as $k => $id) {
				//echo '<pre>';print_r($id);exit;
		DB::table('creditnote_jv_entry')->where('creditnote_jv_id',$cvid)->whereIn('account_id',$idarr)->update(['is_print' => 1]);
			}
		}*/
		...
		if($attributes['group_id'][$key]=='CUSTOMER') { //customer type............
			...
			if( isset($attributes['inv_id'][$key]) ) {
				if($attributes['bill_type'][$key]=='SI') {
					...
					$balance_amount = $attributes['actual_amount'][$key] - $attributes['line_amount'][$key];
					//update as partially paid.
					DB::table('creditnote_jv')
								->where('id', $attributes['inv_id'][$key])
								->update(['balance_amount' => $balance_amount, 'is_transfer' => 2]);
								
					//check if bll is cleared or not...
					$bal = DB::table('creditnote_jv')->where('id', $attributes['inv_id'][$key])->select('balance_amount')->first();
					if($bal && $bal->balance_amount == 0) {
						DB::table('creditnote_jv')->where('id', $attributes['inv_id'][$key])->update(['is_transfer' => 1]);
					}
			...
			try {
                ...
                if($attributes['curno']==$attributes['voucher_no']) {
    				...
    				do {
    					...
    					$inv = DB::table('creditnote_jv')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','CNJV')->where('status',1)->whereNull('deleted_at')->count();
						//
					} while ($inv!=0);
                }
				...
				if($this->journal->id && !empty( array_filter($attributes['line_amount']))) {
					...
					foreach($attributes['line_amount'] as $key => $value) {
						if($value!='' && $value!=0 && $attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0){
							$journalTr =new CreditNoteJournal();
			            ...
			            DB::table('creditnote_jv_entry')->where('creditnote_jv_id',$cvid)->whereIn('account_id',$idarr)->update(['is_print' => 1]);
							...
							if($attributes['group_id'][$key]=='CUSTOMER' && $attributes['account_type'][$key]=='Cr' && $attributes['inv_id'][$key]!='')
							{
								...
								$journalVoucherTr->creditnote_jv_entry_id = $jv_entry_id;
		...
		if($this->isValid($attributes)) { 
			...
			try {
				...
				do {
					...
					$inv = DB::table('creditnote_jv')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','SIN')->where('status',1)->whereNull('deleted_at')->count();
				} while ($inv!=0);
				...
				foreach($attributes['line_amount'] as $key => $value) {
					...
					if($attributes['je_id'][$key]!='') {
						...
						if($attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0) {
							...
							//$jerow['is_print']          = isset($attributes['tag'][$key])?$attributes['tag'][$key]:'';
							
							$journalEntryTr->update($jerow);
							
							if($value=='' || $value==0) {
								DB::table('creditnote_jv_entry')->where('id',$attributes['je_id'][$key])->update(['status' => 0, 'deleted_at' => now()]);
							}

							$ids = $attributes['tagids'];
			if($ids!=''){
			$cvid=$attributes['journal_id'];
			$idarr = explode(',', $ids);
			//echo '<pre>';print_r($idarr);exit;
			DB::table('creditnote_jv_entry')->where('creditnote_jv_id',$cvid)->whereIn('account_id',$idarr)->update(['is_print' => 1]);
		    DB::table('creditnote_jv_entry')->where('creditnote_jv_id',$cvid)->whereNotIn('account_id',$idarr)->update(['is_print' => 0]);
			}
				...
				{ //echo '<pre>';print_r($attributes);exit;
					...
					foreach($arrids as $id) {
						$jes = DB::table('creditnote_jv_entry')
											->leftJoin('account_master', 'account_master.id', '=', 'creditnote_jv_entry.account_id')
											->select('creditnote_jv_entry.*','account_master.category')
											->where('creditnote_jv_entry.id', $id)->first();
						if($jes) {
							DB::table('creditnote_jv_entry')->where('id', $id)->update(['status' => 0, 'deleted_at' => now()]);
							if($jes->category=='SUPPLIER') {
								...
								foreach($invs as $inv) {
									if($inv->bill_type=='PI')
										...
										DB::table('creditnote_jv')->where('id', $inv->purchase_invoice_id)->update(['is_transfer' => 0, 'balance_amount' => DB::raw('balance_amount + '.$inv->assign_amount) ]);
								}
								...
								if($entry) {
									foreach($entry as $ent) {
										if($ent->bill_type=='SI')
											...
											DB::table('creditnote_jv')->where('id', $ent->sales_invoice_id)->update(['is_transfer' => 0, 'balance_amount' => DB::raw('balance_amount + '.$ent->assign_amount) ]);
	...
	public function delete($id)
	{
		...
		try {
			...
			$rows = DB::table('creditnote_jv_entry')->where('creditnote_jv_id', $id)->select('id','account_id','entry_type','amount')->get();
			foreach($rows as $row) {
				...
				$entry = DB::table('creditnote_jv_tr')->where('creditnote_jv_entry_id', $row->id)->where('status',1)->whereNull('deleted_at')->get();
				if($entry) {
					foreach($entry as $ent) {
						if($ent->bill_type=='SI')
						...
						DB::table('creditnote_jv_tr')->where('id', $ent->id)->update(['status' => 0,'deleted_at' => now() ]);
					}
				}
			}
			
			DB::table('creditnote_jv_entry')->where('creditnote_jv_id', $id)->update(['status' => 0,'deleted_at' => now() ]);
			$this->journal->delete();
	...
	public function journalList2($type)
	{
		$result = $this->journal->where('creditnote_jv.status', 1)
							 ->join('creditnote_jv_entry AS JE', function($join) {
	...
	{
		$result = DB::table('creditnote_jv_entry')->where('creditnote_jv_id', $id)
						->join('account_master', 'account_master.id', '=', 'creditnote_jv_entry.account_id')
						->leftJoin('account_master AS AM', 'AM.id', '=', 'creditnote_jv_entry.party_account_id')
						->where('creditnote_jv_entry.status', 1)
						->select('creditnote_jv_entry.*','account_master.master_name','AM.master_name AS party_name','account_master.category')
						->orderBy('creditnote_jv_entry.id','ASC')
						->get();
	//echo '<pre>';print_r($result);exit;					
	return $result;
	}
	public function findJEPdata($id)
	{
		$result = DB::table('creditnote_jv_entry')->where('creditnote_jv_id', $id)
						->join('account_master', 'account_master.id', '=', 'creditnote_jv_entry.account_id')
						->leftJoin('account_master AS AM', 'AM.id', '=', 'creditnote_jv_entry.party_account_id')
						->where('creditnote_jv_entry.status', 1)
						->where('creditnote_jv_entry.is_print', 1)
						->select('creditnote_jv_entry.*','account_master.master_name','AM.master_name AS party_name','account_master.category')
						->orderBy('creditnote_jv_entry.id','ASC')
						->get();
	//echo '<pre>';print_r($result);exit;					
	...
	{
		$query = $this->journal->where('creditnote_jv.status', 1)
							 ->join('creditnote_jv_entry AS JE', function($join) {
								 $join->on('JE.creditnote_jv_id', '=', 'creditnote_jv.id');
							 })
							 ->where('JE.status',1)
					 ...
					 if($search) {
						 $query->where(function($query) use ($search){
							 $query->where('creditnote_jv.voucher_no','LIKE',"%{$search}%");
									//->orWhere('JE.description', 'LIKE',"%{$search}%");
						});
					 }
							 $query->select('creditnote_jv.*','JE.description');
	...
	{
		return $this->journal->where('creditnote_jv.status', 1)
							 ->join('creditnote_jv_entry AS JE', function($join) {
								 $join->on('JE.creditnote_jv_id', '=', 'creditnote_jv.id');
							 })
							 ->where('JE.status',1)
	...
	public function findJVData($id) {
	  ...
	  return DB::table('creditnote_jv')->where('creditnote_jv.id',$id)
	                    ->leftjoin('users AS U1','U1.id','=','creditnote_jv.created_by')
	                    ->leftjoin('users AS U2','U2.id','=','creditnote_jv.modify_by')
	                    ->select('creditnote_jv.*','U1.name as createdby','U2.name as modifiedby')
	                    ->first();
	}
}
