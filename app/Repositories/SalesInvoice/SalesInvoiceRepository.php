<?php namespace App\Repositories\SalesInvoice;
...
class SalesInvoiceRepository extends AbstractValidator implements SalesInvoiceInterface {
	...
	{ //echo '<pre>';print_r($attributes);exit;
		...
		$this->sales_invoice->reference_no  = $attributes['reference_no'] ?? null;
        ...
        $this->sales_invoice->document_type = ($attributes['document_type']=='')?'SI':$attributes['document_type'] ?? null;//Purchase Invoice type
		...
		$this->sales_invoice->document_id   = isset($attributes['document_id'])?$attributes['document_id'] ?? null:'';
		$this->sales_invoice->terms_id 		= isset($attributes['terms_id'])?$attributes['terms_id'] ?? 0:0;
		$this->sales_invoice->job_id 		= isset($attributes['job_id'])?$attributes['job_id'] ?? 0:0;
		$this->sales_invoice->description   = isset($attributes['description'])?$attributes['description'] ?? null:'';
		$this->sales_invoice->is_fc 		= isset($attributes['is_fc'])?1:0;
		$this->sales_invoice->currency_id   = (isset($attributes['currency_id']))?$attributes['currency_id'] ?? 0:'';
		$this->sales_invoice->currency_rate = (isset($attributes['currency_rate']))?$attributes['currency_rate'] ?? 0:'';
		...
		$this->sales_invoice->lpo_no 		= $attributes['lpo_no'] ?? null;
		$this->sales_invoice->salesman_id   = (isset($attributes['salesman_id']))?$attributes['salesman_id']:'';
		$this->sales_invoice->is_export		= $this->isExport($attributes)?1:0;
		$this->sales_invoice->location_id   = isset($attributes['location_id'])?$attributes['location_id'] ?? 0:'';
		$this->sales_invoice->department_id   = isset($attributes['department_id'])?$attributes['department_id'] ?? 0:'';
		$this->sales_invoice->items_description   = isset($attributes['items_description'])?$attributes['items_description']:'';
		$this->sales_invoice->vehicle_id   = isset($attributes['vehicle_id'])?$attributes['vehicle_id'] ?? 0:'';
	...
	private function setJobdetailsUpdate($attributes) {
		...
		if(isset($attributes['opr_description']) && $attributes['remove_desc']!='')
		{
			...
			foreach($arrids as $row) {
				DB::table('jobinvoice_details')->where('id', $row)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
			}
		}
	}
	...
	private function setItemInputValue($attributes, $salesInvoiceItem, $key, $value, $lineTotal) 
	{
		...
		$salesInvoiceItem->tax_include 		= $attributes['tax_include'][$key] ?? 0;
	...
	protected function addService($attributes) {
		...
		if($discount > 0)
			...
			foreach($attributes['lbitem_id'] as $key => $value){ 
				...
				$tax_code = ($this->isExport($attributes))?"ZR":$attributes['lbtax_code'][$key];
				if($this->isExport($attributes) || $tax_code=="EX" || $tax_code=="ZR") {
	...
	{ 	
		...
		if($this->isValid($attributes)) {
			...
			try {
				...
				$qry = DB::table('sales_invoice')->where('deleted_at', '0000-00-00 00:0:00')->where('status', 1);
				if($dept > 0)	
					...
					foreach($attributes['item_id'] as $key => $value){ 
						...
						if($this->mod_location->is_active==1 && ($updated == false)) {
							...
							$qtys = DB::table('item_location')
										  ...
										  ->whereNull('deleted_at')->select('item_location.id','location.code','bin_location.code AS bin')->first();
...
//print_r($dolog);exit;
									...
									DB::table('item_location_si')->where('id', $attributes['editid'][$key][$lk])->update(['quantity' => $lcqty,'status' => 1, 'deleted_at' => null, 'qty_entry' => $lq]);

								} else { //NOV24
									DB::table('item_location_si')->where('id', $attributes['editid'][$key][$lk])->update(['quantity' => $lcqty,'status' => 0, 'deleted_at' => date('Y-m-d h:i:s'), 'qty_entry' => $lq]);
								}
							}
						}
            				...
            				if(isset($attributes['batchNos'][$key]) && $attributes['batchNos'][$key]!='' && $attributes['qtyBatchs'][$key]!='') {
                			    ...
                			    foreach($remArr as $rem) {
                			        ...
                			        $batch_log = DB::table('batch_log')
                        				                ...
                        				                ->select('id','quantity')->first();
                			        if($batch_log) {
                    			        DB::table('batch_log')->where('id',$batch_log->id)->update(['deleted_at' => date('Y-m-d h:i:s'), 'deleted_by' => Auth::User()->id]);
			...
			if($this->matservice->is_active==1 && $this->sales_invoice->id) {
				...
				if($attributes['lbremove_item']!='')
				{
					...
					$remline_total = $remtax_total = 0;
					foreach($arrids as $row) {
						DB::table('sales_invoice_item')->where('id', $row)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
					}
				}
			}
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					...
					DB::table('con_location')->where('invoice_id',$row)->where('is_do',0)->update(['status'=>0,'deleted_at'=>date('Y-m-d H:i:s')]);
					foreach($sirow as $srow) {
						DB::table('item_location_si')->where('id',$srow->id)->update(['status'=>0,'deleted_at'=>date('Y-m-d H:i:s')]);
			...
			if($this->setInputValue($attributes)) {
				$this->sales_invoice->modify_at = date('Y-m-d H:i:s');
		...
		if(isset($attributes['footer_id']) && isset($attributes['footer']))	{					  
			...
			if(Session::get('cost_accounting')==1) {
				...
				$this->PurchaseAndSalesMethod($attributes, $subtotal, $tax_total, $net_amount, $this->sales_invoice->id, $taxtype);
			}
			...
			if(isset($attributes['remove_oc']) && $attributes['remove_oc']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('si_selling_exp')->where('id', $row)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
	...
	public function delete($id)
	{
		...
		try {
				...
				if($this->sales_invoice->document_type=='SQ') {
				   ...
				   // if($sicount==0) {
					    ...
					    foreach($ids as $idd) {
					        ...
					        if($dorow)
					            DB::table('item_log')->where('document_type','CDO')->where('document_id',$idd)->update(['quantity' => $dorow->quantity, 'status' => 1,'deleted_at' => null]); //($dorow->balance_quantity > 0)?$dorow->balance_quantity:
					    }
				    //}
			...
			if($this->sales_invoice->is_rental == 1) {         
				DB::table('account_transaction')->where('voucher_type', 'SRL')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
			}else{
			    DB::table('account_transaction')->where('voucher_type', 'SI')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
			}
	...
	public function salesInvoiceList($type,$start,$limit,$order,$dir,$search,$dept=null)
	{
		...
		$query = $this->sales_invoice->where('sales_invoice.status',1)->where('sales_invoice.is_rental',0) //->where('sales_invoice.department_id','!=',4)
				...
				if($type=='get'){
					return $query->select('sales_invoice.*','am.master_name AS customer','V.reg_no','JM.name AS job','sales_invoice.vehicle_no AS vehicle','SO.voucher_no AS jo_no')
									->offset($start)
									->limit($limit)
	...
	public function getItems($id,$mod=null)
	{
		...
		return $query->select('poi.*','u.unit_name','im.item_code','im.batch_req','iu.packing','iu.is_baseqty','iu.cur_quantity','iu.pkno','ci.balance_quantity as do_balance_quantity')
  ...
  if( $attributes['search_type']=='job_order' )
	...
	public function updateLastPurchaseCostOnTransfer($attributes, $key, $type, $DOlogs)
	{
		...
		if($DOlogs->quantity==$attributes['quantity'][$key]) {
	    	...
	    	DB::table('item_log')->where('document_type','CDO')->whereIn('document_id',$pids)->update(['status'=> 0,'deleted_at'=>date('Y-m-d H:i:s')]);
		} else {
		    ...
		    foreach($pids as $pid) {
		       ...
		       if($drow && $drow->quantity <= $siquantity) {
		           DB::table('item_log')->where('document_type','CDO')->where('document_id',$pid)->update(['status'=> 0,'deleted_at'=>date('Y-m-d H:i:s')]);
	...
	public function updateLastPurchaseCostOnTransferUpdate($attributes, $key, $type, $DOlogs)
	{
		...
		if($DOlogs->quantity==$attributes['quantity'][$key]) {
	    	...
	    	DB::table('item_log')->where('document_type','CDO')->whereIn('document_id',$pids)->update(['quantity' => 0, 'status'=> 0, 'deleted_at'=>date('Y-m-d H:i:s')]);
		} else {
		    //PARTIAL TRANSFER OF DO TO SI HANDLING LOG PARIAL QUANTITY.....
		    $siquantity = $attributes['quantity'][$key];
		    ...
		    foreach($pids as $pid) {
		       ...
		       if($drow && $drow->quantity < $siquantity) {
		           ...
		           if($finalqty==0)
		                DB::table('item_log')->where('document_type','CDO')->where('document_id',$pid)->update(['quantity' => 0, 'status'=> 0, 'deleted_at'=>date('Y-m-d H:i:s')]);
		           else
