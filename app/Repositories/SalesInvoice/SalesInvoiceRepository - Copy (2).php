...
class SalesInvoiceRepository extends AbstractValidator implements SalesInvoiceInterface {
	...
	private function setInputValue($attributes) 
	{
		...
		$this->sales_invoice->cr_account_id = $attributes['cr_account_id'];
	...
	{ 	
		...
		if($this->isValid($attributes)) {
			...
			try {
				...
				Session::put('SI_cracid', $attributes['cr_account_id']);
					...
					foreach($attributes['item_id'] as $key => $value){ 
						...
						//if($arrResult['line_total']) {
							...
							if($this->mod_do_qty->is_active==1) {
								...
								if($this->checkCDOLogs($attributes, $key)==false) { //CHECK CDO LOG INSERTED IN LOG TABLE OR NOT...
									//update last purchase cost and cost average....
									$CostAvg_log = $this->objUtility->updateLastPurchaseCostAndCostAvg($attributes, $key, 0); 
								} else {
									
									$CostAvg_log = $this->updateLastPurchaseCostOnTransfer($attributes, $key,0);
						...
						if(isset($attributes['locqty'][$key])) {
							foreach($attributes['locqty'][$key] as $lk => $lq) {
								if($lq!='') {
									...
									$loctext .= ($loctext!='')?'<br/>'.$attributes['locnm'][$key][$lk].': '.$lq:$attributes['locnm'][$key][$lk].': '.$lq;
						...
						if($this->mod_location->is_active==1 && ($updated == false)) {
							...
							$qtys = DB::table('item_location')->where('status',1)->where('location_id', $sales_loc)
															  ->where('item_id', $value)->where('unit_id', $attributes['unit_id'][$key])
															  ->whereNull('deleted_at')->select('id')->first();
							if($qtys) {
				...
				//API calls...
				/* $attributes['invoice_id'] = $this->sales_invoice->id;
				$response = Curl::to($this->api_url.'salesadd.php')
							->withData($attributes)
							->asJson()
							...
							$bquantity = $attributes['quantity'][$key] - $exi_quantity;
							  ...
							  $sale_cost = $this->objUtility->updateItemQuantitySales($attributes, $key, $bquantity);//exit;
									$CostAvg_log = $this->objUtility->updateLastPurchaseCostAndCostAvgonEdit($attributes, $key, 0);
										$logid = $this->setSaleLog($attributes, $key, $this->sales_invoice->id, $CostAvg_log, $sale_cost, 'update', $itemsobj); //DEC 23 UPDATE...
		...
		try {
			...
			if($this->sales_invoice->document_id > 0) {
				if($this->sales_invoice->document_type=='SQ') {
					...
					DB::table('customer_do')->where('id', $this->sales_invoice->document_id)->update(['is_transfer' => 0, 'is_editable' => 0]);

					DB::table('customer_do_item')->where('customer_do_id', $this->sales_invoice->document_id)->update(['is_transfer' => 0]);
				}
			}
			...
			DB::table('sales_invoice_item')->where('sales_invoice_id', $id)
									->update(['status' => 0, 'deleted_at' => now()]);
	...
	public function getItems($id,$mod=null)
	{
		...
		return $query->select('poi.*','u.unit_name','im.item_code','iu.packing','iu.is_baseqty','iu.cur_quantity')->groupBy('poi.id')->orderBY('poi.id')->get();
					  //->groupBY('im.id')
...
if($attributes['vehicle_no']!='') { 
  ...
  if( $attributes['search_type']=='job_order' )
	...
	public function updateLastPurchaseCostOnTransfer($attributes, $key, $type)
	{
		...
		DB::table('item_log')->where('document_type','CDO')->whereIn('document_id',$pid)->update(['status'=>0,'deleted_at'=>now()]);
