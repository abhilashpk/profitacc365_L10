...
class ReceiptVoucherRepository extends AbstractValidator implements ReceiptVoucherInterface {
	...
	{  //echo '<pre>';print_r($attributes);exit; 
		if($this->isValid($attributes)) {
			...
			/*if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					if(isset($attributes['department_id']) && Session::get('department')==1)
						$inv = DB::table('receipt_voucher')->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->whereNull('deleted_at')->count();
					else
						$inv = DB::table('receipt_voucher')->where('voucher_no',$attributes['voucher_no'])->where('status',1)->whereNull('deleted_at')->count();
			...
			//VOUCHER NO INCREMENT LOGIC//
				...
				//VOUCHER NO INCREMENT LOGIC//
				if( $attributes['curno'] == $attributes['voucher_no'] ) {
					$attributes['rowid'] = $receipt_voucher_id; 
	...
	private function voucherNoGenerate($attributes) {
		...
		do {
			...
			if(isset($attributes['department_id']) && Session::get('department')==1)
				$inv = DB::table('receipt_voucher')->where('id','!=',$attributes['rowid'])->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->whereNull('deleted_at')->count();
			else
				$inv = DB::table('receipt_voucher')->where('id','!=',$attributes['rowid'])->where('voucher_no',$attributes['voucher_no'])->where('status',1)->whereNull('deleted_at')->count();
	...
	{ 	//echo '<pre>';print_r($attributes);exit;
		...
		try {
				...
				foreach($attributes['line_amount'] as $key => $value) {
					...
					if($attributes['je_id'][$key]!='') {
						...
						if($attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0) {
							...
							if($value=='' || $value==0) {
								DB::table('receipt_voucher_entry')->where('id',$attributes['je_id'][$key])->update(['status' => 0, 'deleted_at' => now()]);
							}
							...
							if($attributes['group_id'][$key]=='PDCR') {
								//UPDATE PDC...
								$pdcrow = DB::table('pdc_received')
												//->where('voucher_id', $this->receipt_voucher->id)
												//->where('customer_id', $attributes['partyac_id'][$key]) ########******CHANGED AS PER REALESTATE ORV PDC TESTING NEEDED  ->where('customer_id', $attributes['account_id'][$key])
								...
								/*$pdcrow = DB::table('pdc_received')
												->where('voucher_id', $this->receipt_voucher->id)
												->where('customer_id', $attributes['account_id'][$key])
												->select('id')->first();
								echo '<pre>';print_r($pdcrow);exit;
								if($pdcrow) { */
									//SET AS DELETED
									DB::table('pdc_received')
													->where('entry_id', $attributes['je_id'][$key])
			...
			if($difference==0.00) { //JN1
				...
				throw new ValidationException('Receipt entry validation error! Please try again.');
			}
			...
			foreach($attributes['tagadv'] as $row => $val)
			{
				...
				foreach($attributes['tag'] as $k => $key) { 
					...
					if($ar==1) {
						...
						$rv_entry_id = isset($attributes['receipt_voucher_entry_id'][$row])?$attributes['receipt_voucher_entry_id'][$row]:''; 
						if($attributes['bill_type'][$val]=='OB') { //SI
								...
								if($attributes['actual_amountadv'][$k]==$attributes['advance_amount'][$k]) { //key
									...
									DB::table('opening_balance_tr')
										...
										'balance_amount' => (int)($attributes['actual_amountadv'][$k]-(int)$attributes['advance_amount'][$k])]); //key
	...
	{	//echo '<pre>';print_r($attributes);exit;
		...
		foreach($trnarr['id'] as $key => $val) { 
			...
			if($pdcs) {
				...
				DB::table('pdc_received')->where('id',$id)->update(['status' => 1, 'dr_bank_id' => $trnarr['drid'][$key] ]); //JL27
