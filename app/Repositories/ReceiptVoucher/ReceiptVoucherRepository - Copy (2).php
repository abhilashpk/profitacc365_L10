...
class ReceiptVoucherRepository extends AbstractValidator implements ReceiptVoucherInterface {
	...
	public function findRVdata($id)
	{
		return DB::table('receipt_voucher_entry')->where('receipt_voucher_entry.receipt_voucher_id', $id)
						...
						->where('receipt_voucher_entry.status', 1)
						->orderBy('receipt_voucher_entry.id', 'ASC')
						->select('receipt_voucher_entry.*','account_master.master_name','bank.name','CE.voucher_date')->get();
	}
	...
	{ 
		...
		if($pdcr) {
			$query1 = $this->receipt_voucher->where('receipt_voucher.status',1)
							...
							$query1->select('receipt_voucher.id','receipt_voucher.voucher_no','receipt_voucher.voucher_date','receipt_voucher.tr_description AS description',
										...
										DB::raw("(SELECT account_master.id FROM receipt_voucher_entry 
												   JOIN account_master ON(account_master.id = receipt_voucher_entry.account_id)
												   WHERE receipt_voucher_entry.receipt_voucher_id=receipt_voucher.id 
												   AND receipt_voucher_entry.entry_type='Cr' AND account_master.category='CUSTOMER' LIMIT 0,1) AS customer_id"),
			...
			$query2 = DB::table('journal')->where('journal.status',1)
									...
									->where('journal_entry.deleted_at','0000-00-00 00:00:00'); //cheque_date
				...
				$query2->select('journal.id','journal.voucher_no','journal.voucher_date','journal_entry.description',
										 ...
										 DB::raw("(SELECT account_master.master_name FROM journal_entry 
												   ...
												   AND journal_entry.entry_type='Dr' LIMIT 0,1) AS debitor"),//Dr
										 DB::raw("(SELECT account_master.master_name FROM journal_entry 
												   JOIN account_master ON(account_master.id = journal_entry.account_id)
												   WHERE journal_entry.journal_id=journal.id 
										...
										DB::raw("(SELECT account_master.id FROM journal_entry 
												   JOIN account_master ON(account_master.id = journal_entry.account_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Dr' LIMIT 0,1) AS pdcr_id"),
										DB::raw("(SELECT account_master.id FROM journal_entry 
												   JOIN account_master ON(account_master.id = journal_entry.account_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Cr' AND account_master.category='CUSTOMER' LIMIT 0,1) AS customer_id"),
										...
										DB::raw("(SELECT bank.id FROM journal_entry 
												   JOIN bank ON(bank.id = journal_entry.bank_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Dr' LIMIT 0,1) AS bank_id"),
...
/	
			...
			return $result = $query1->union($query2)->orderBy('cheque_date','ASC')->get();
		} else 
			return []; */ //COMMENTED ON 2021 FEB 23.............. month
		
		$result = DB::table('pdc_received')->where('pdc_received.status',0)
	...
	{	//echo '<pre>';print_r($attributes);exit;
		...
		foreach($trnarr['id'] as $key => $val) { 
			...
			$pdcs = DB::table('pdc_received')->where('id', $val)->first(); //SP3
			$id = null;
			if($pdcs) {
...
/	
	...
	public function PdcReceivedSubmit($attributes)
	{
		...
		foreach($trnarr['id'] as $key => $val) { 
			...
			$pdcs = DB::table('pdc_received')->where('voucher_id', $val)->first();
			$id = null;
			if($pdcs) {
