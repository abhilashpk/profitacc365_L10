...
namespace App\Repositories\PurchaseSplitReturn;

use App\Models\PurchaseSplitReturn;
use App\Models\PurchaseSplitReturnItem;
...
class PurchaseSplitReturnRepository extends AbstractValidator implements PurchaseSplitReturnInterface {
	
	public $objUtility;
	
	protected $purchasesplit_return;
	
	protected static $rules = [];
	
	public function __construct(PurchaseSplitReturn $purchasesplit_return) {
		$this->purchasesplit_return = $purchasesplit_return;
		$this->objUtility = new UpdateUtility();
	}
	
	public function all()
	{
		return $this->purchasesplit_return->get();
	}
	
	public function find($id)
	{
		return $this->purchasesplit_return->where('id', $id)->first();
	}
	
	//set input fields values
	private function setInputValue($attributes)
	{
		$this->purchasesplit_return->voucher_id = $attributes['voucher_id']; 
		$this->purchasesplit_return->voucher_no = $attributes['voucher_no']; 
		$this->purchasesplit_return->reference_no = $attributes['reference_no'];
		$this->purchasesplit_return->purchase_split_id = isset($attributes['purchase_split_id'])?$attributes['purchase_split_id']:'';
		$this->purchasesplit_return->voucher_date = ($attributes['voucher_date']=='')?date('Y-m-d'):date('Y-m-d', strtotime($attributes['voucher_date']));
		$this->purchasesplit_return->supplier_id = $attributes['supplier_id'];
		$this->purchasesplit_return->job_id = $attributes['job_id'];
		$this->purchasesplit_return->description = isset($attributes['description'])?$attributes['description']:'';
		$this->purchasesplit_return->is_fc = isset($attributes['is_fc'])?1:0;
		$this->purchasesplit_return->currency_id = (isset($attributes['currency_id']))?$attributes['currency_id']:'';
		$this->purchasesplit_return->currency_rate = (isset($attributes['currency_rate']))?$attributes['currency_rate']:'';
		$this->purchasesplit_return->department_id   = isset($attributes['department_id'])?$attributes['department_id']:'';
		$this->purchasesplit_return->is_pettycash   = isset($attributes['is_pettycash'])?$attributes['is_pettycash']:'';
		$this->purchasesplit_return->foot_description   =(isset($attributes['foot_description']))?$attributes['foot_description']:'';
	...
	private function setItemInputValue($attributes, $purchaseSplitReturnItem, $key, $value,$total, $lineTotal, $total_quantity=null)
	{
		...
		$purchaseSplitReturnItem->purchasesplit_return_id = $this->purchasesplit_return->id;
		$purchaseSplitReturnItem->account_id = $attributes['account_id'][$key];
		$purchaseSplitReturnItem->unit_id = isset($attributes['unit_id'][$key])?$attributes['unit_id'][$key]:'';
		$purchaseSplitReturnItem->item_description = $attributes['item_description'][$key];
		$purchaseSplitReturnItem->quantity = $attributes['quantity'][$key];
		$purchaseSplitReturnItem->unit_price = (isset($attributes['is_fc']))?$attributes['cost'][$key]*$attributes['currency_rate']:$attributes['cost'][$key];
		$purchaseSplitReturnItem->vat = $attributes['line_vat'][$key];
		$purchaseSplitReturnItem->item_vat = $attributes['vatline_amt'][$key];
		$purchaseSplitReturnItem->item_total = $line_total;
		$purchaseSplitReturnItem->tax_code 	= $tax_code;
		$purchaseSplitReturnItem->line_total = $item_total;
		$purchaseSplitReturnItem->item_jobid = isset($attributes['jobid'][$key])?$attributes['jobid'][$key]:$attributes['job_id'];
		$purchaseSplitReturnItem->item_supname = isset($attributes['supname'][$key])?$attributes['supname'][$key]:'';
		$purchaseSplitReturnItem->item_vatno = isset($attributes['taxno'][$key])?$attributes['taxno'][$key]:'';
		$purchaseSplitReturnItem->unit_price_fc = $attributes['cost'][$key];
		$purchaseSplitReturnItem->item_vat_fc = (isset($attributes['is_fc']))?round(($tax_total /  $attributes['currency_rate']),2):$tax_total;
		$purchaseSplitReturnItem->item_total_fc = (isset($attributes['is_fc']))?round(($line_total /  $attributes['currency_rate']),2):$line_total;
		$purchaseSplitReturnItem->line_total_fc = (isset($attributes['is_fc']))?round(($item_total /  $attributes['currency_rate']),2):$item_total;
	...
	//	echo '<pre>';print_r($amount);exit;
		...
		DB::table('account_transaction')
					->insert([  'voucher_type' 		=> 'PSR',
								'voucher_type_id'   => $this->purchasesplit_return->id,
	...
	private function SetItemAccountTransactionUpdate($itemid, $amount, $key, $attributes, $type) {
		...
		DB::table('account_transaction')
					->where('voucher_type','PSR')
					->where('voucher_type_id', $this->purchasesplit_return->id)
	...
	{
		if($attributes['purchase_split_id']) {
			$row1 = DB::table('purchase_split_item')->where('purchase_split_id', $attributes['purchase_split_id'])->count();
			$row2 = DB::table('purchase_split_item')->where('purchase_split_id', $attributes['purchase_split_id'])->where('is_transfer',1)->count();
			if($row1==$row2) {
				
				DB::table('purchase_split')
						->where('id', $attributes['purchase_split_id'])
	...
	private function AccountingMethod($attributes, $line_total, $tax_total, $net_amount, $purchasesplit_return_id)
	{
		if( isset($attributes['is_fc']) ) 
		...
		if( $this->setAccountTransaction($attributes, $net_amount, $purchasesplit_return_id, $type='Dr', $amount_type='NTAMT') ) {
		
			$this->setAccountTransaction($attributes, $discount, $purchasesplit_return_id, $type='Dr', $amount_type='DIS');
		}
		
	}
	
	private function AccountingMethodUpdate($attributes, $line_total, $tax_total, $net_amount, $purchasesplit_return_id)
	{
		if( isset($attributes['is_fc']) ) //M14..
			...
			//Credit Supplier Accounting
			if( $this->setAccountTransactionUpdate($attributes, $net_amount, $purchasesplit_return_id, $type='Dr', $amount_type='NTAMT') ) {
				
				$this->setAccountTransactionUpdate($attributes, $discount, $purchasesplit_return_id, $type='Dr', $amount_type='DIS');
			}
	}
		...
		if($this->isValid($attributes)) {
		 ...
		 try {
			...
			if($this->setInputValue($attributes)) {
				$this->purchasesplit_return->status = 1;
				$this->purchasesplit_return->created_at = now();
				$this->purchasesplit_return->created_by = 1;
				$this->purchasesplit_return->fill($attributes)->save();
			}
			
			//invoice items insert
			if($this->purchasesplit_return->id && !empty( array_filter($attributes['account_id']))) { 
				...
				foreach($attributes['account_id'] as $key => $value) { 
					$purchaseSplitReturnItem = new PurchaseSplitReturnItem();
					$vat = $attributes['line_vat'][$key];
					$arrResult 	= $this->setItemInputValue($attributes, $purchaseSplitReturnItem, $key, $value, $total, $total_quantity);
					if($arrResult['line_total']) {
						$line_total			   += $arrResult['line_total'];
						$tax_total      	   += $arrResult['tax_total'];
						$item_total			   += $arrResult['item_total'];
						
						$purchaseSplitReturnItem->status = 1;
						$inv_item = $this->purchasesplit_return->doItem()->save($purchaseSplitReturnItem);
				...
				DB::table('purchasesplit_return')
							->where('id', $this->purchasesplit_return->id)
				...
				$this->AccountingMethod($attributes, $subtotal, $tax_total, $net_amount, $this->purchasesplit_return->id);
				
				
				//update voucher no........
				if( ($this->purchasesplit_return->id) && ($attributes['curno'] <= $attributes['voucher_no']) ) {  
					//Update voucher no based on department or not...
					if(Session::get('department')==1) { //if dept active...
						 DB::table('account_setting')
							->where('voucher_type_id', 30) 
			...
			if(isset($attributes['purchase_split_id'])) 
	...
	{ 	//echo '<pre>';print_r($attributes);exit;
		$this->purchasesplit_return = $this->find($id);
		...
		try {
			...
			if($this->purchasesplit_return->id && !empty( array_filter($attributes['account_id']))) {
				...
				foreach($attributes['account_id'] as $key => $value) { 
					...
					if($attributes['order_item_id'][$key]!='') {
						...
						$purchaseSplitReturnItem = PurchaseSplitReturnItem::find($attributes['order_item_id'][$key]);
						$oldqty = $purchaseSplitReturnItem->quantity;
						...
						$exi_item_id = $purchaseSplitReturnItem->item_id;
						$exi_unit_id = $purchaseSplitReturnItem->unit_id;
						$exi_qty = $purchaseSplitReturnItem->quantity;
						$exi_price = $purchaseSplitReturnItem->unit_price;
						$itemsobj = (object)['item_id' => $exi_item_id, 'unit_id' => $exi_unit_id]; 
						
						$purchaseSplitReturnItem->update($items);
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					$itm = DB::table('purchasesplit_return_item')->where('id', $row)->select('id','purchasesplit_return_id','account_id')->first();
					DB::table('purchasesplit_return_item')->where('id', $row)->update(['status' => 0, 'deleted_at' => now()]);
					
					//REMOVE FROM TRANSACTION TABLE..
					DB::table('account_transaction')->where('voucher_type','PSR')->where('voucher_type_id',$itm->purchasesplit_return_id)->where('account_master_id',$itm->account_id)
							->where('other_info',$itm->id)->update(['status'=>0,'deleted_at'=>'0000-00-00 00:00:00']);
							
					//IF VAT ALSO
					DB::table('account_transaction')->where('voucher_type','PSR')->where('voucher_type_id',$itm->purchasesplit_return_id)
			...
			if($this->setInputValue($attributes)) {
				...
				$this->purchasesplit_return->modify_at = now();
				$this->purchasesplit_return->modify_by = 1;
				$this->purchasesplit_return->fill($attributes)->save();
			...
			$this->AccountingMethodUpdate($attributes, $subtotal, $tax_total, $net_amount, $this->purchasesplit_return->id);
	...
	public function delete($id)
	{
		...
		$this->purchasesplit_return = $this->purchasesplit_return->find($id);
		//inventory update...
		DB::beginTransaction();
		try {
		    
		    if($this->purchasesplit_return->purchase_split_id!='') {
				DB::table('purchase_split')
							->where('id', $this->purchasesplit_return->purchase_split_id)
							->update(['is_transfer' => 0,'is_editable' => 0]);
			}
			
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'PSR')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => now(),'deleted_by' => Auth::User()->id ]);
			
			$this->objUtility->tallyClosingBalance( $this->purchasesplit_return->supplier_id );
			
			$itemacs = DB::table('purchasesplit_return_item')->where('purchasesplit_return_id', $id)->select('account_id')->get();
			...
			DB::table('purchasesplit_return')->where('id', $id)->update(['deleted_by' => Auth::User()->id   ]);
			$this->purchasesplit_return->delete();
	...
	{
		return $this->purchasesplit_return->select('id','name')->where('status', 1)->orderBy('name', 'ASC')->get()->toArray();
	}
	
	public function check_reference_no($refno, $id = null) { 
		
		if($id)
			return $this->purchasesplit_return->where('reference_no',$refno)->where('id', '!=', $id)->count();
		else
			return $this->purchasesplit_return->where('reference_no',$refno)->count();
	}
	
	public function check_invoice_id($invoice_id) { 
		
		return $this->purchasesplit_return->where('voucher_no', $invoice_id)->count();
	}
	
	public function getPIdata($did=null)
	{
		$query = $this->purchasesplit_return->where('purchasesplit_return.status',1);
		if($did)
			$query->where('purchasesplit_return.department_id', $did);
				
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','purchasesplit_return.supplier_id');
						} )
					->where('purchasesplit_return.is_return',0)
					->select('purchasesplit_return.*','am.master_name AS supplier')
	...
	public function getSupplierInvoice($supplier_id,$mod=null,$pvid=null)
	{
		...
		if($pvid) {
			...
			return $this->purchasesplit_return->where('purchasesplit_return.status',1)
									   ->leftJoin('payment_voucher_tr AS PV', function($join){
										   $join->on('PV.purchase_split_id','=','purchasesplit_return.id');
										   $join->where('PV.deleted_at','=','0000-00-00 00:00:00');
										   $join->where('PV.status','=',1);
									   }) 
									   ->where('purchasesplit_return.supplier_id', $supplier_id)
									   ->whereIn('purchasesplit_return.amount_transfer',$arr)
									   ->groupBY('purchasesplit_return.voucher_no')
									   ->orderBY('purchasesplit_return.voucher_date', 'ASC')
									   ->select('purchasesplit_return.*','PV.assign_amount')
									   ->get();
			
								   
		} else {
			return $this->purchasesplit_return->where('status',1)
	...
	public function getOtherCostBills($supplier_id,$mod=null,$pvid=null)
	{
		...
		if($pvid) {
			...
			return DB::table('pi_other_cost')->where('pi_other_cost.status',1)
							   ->join('purchasesplit_return AS PI', function($join) {
	...
	{
		$invoice = $this->purchasesplit_return->where('purchasesplit_return.id', $attributes['document_id'])
								   ->join('account_master AS AM', function($join) {
									   $join->on('AM.id','=','purchasesplit_return.supplier_id');
								   })
								   ->leftJoin('currency AS C', function($join) {
									   $join->on('C.id','=','purchasesplit_return.currency_id');
								   })
								   ->leftJoin('terms AS TR', function($join) {
									   $join->on('TR.id','=','purchasesplit_return.terms_id');
								   })
								   ->select('AM.account_id','AM.master_name AS supplier','purchasesplit_return.*','AM.address','AM.city','AM.state','AM.contact_name','AM.vat_no','AM.phone','C.name AS currency','TR.description AS terms')
								   ->orderBY('purchasesplit_return.id', 'ASC')
								   ->first();
								   
		$items = $this->purchasesplit_return->where('purchasesplit_return.id', $attributes['document_id'])
								   ->join('purchasesplit_return_item AS PI', function($join) {
									   $join->on('PI.purchasesplit_return_id','=','purchasesplit_return.id');
								   })
								   ...
								   ->select('PI.*','purchasesplit_return.id','IM.item_code','U.unit_name')
								   ->orderBY('PI.id')
								   ->get();
	...
	{
		$query = $this->purchasesplit_return->where('purchasesplit_return.id', $id);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','purchasesplit_return.supplier_id');
						} )
						 ->leftJoin('jobmaster AS J',function($join) {
						$join->on('J.id','=','purchasesplit_return.job_id');
					})
					->select('purchasesplit_return.*','am.master_name AS supplier','J.code')
					->orderBY('purchasesplit_return.id', 'ASC')
					->first();
	}
	...
	public function getItems($id)
	{
		...
		return $query->join('purchasesplit_return_item AS PSI', function($join) {
							$join->on('PSI.purchasesplit_return_id','=','purchasesplit_return.id');
						} )
					  ->join('account_master AS AM', function($join){
	...
	{
		$query = $this->purchasesplit_return->where('purchasesplit_return.supplier_id',$supplier_id)->where('purchasesplit_return.status',1);
		
		return $query->join('purchasesplit_return_item AS poi', function($join) {
							$join->on('poi.purchasesplit_return_id','=','purchasesplit_return.id');
						} )
					  ->join('units AS u', function($join){
					  ...
					  ->where('poi.status',1)
					  ->select('poi.*','u.unit_name','im.item_code','purchasesplit_return.voucher_date','purchasesplit_return.reference_no')->get();
	}
	
	public function check_voucher_no($refno, $id = null) { 
		
		if($id)
			return $this->purchasesplit_return->where('voucher_no',$refno)->where('id', '!=', $id)->count();
		else
			return $this->purchasesplit_return->where('voucher_no',$refno)->count();
	}
	...
	public function getInvoiceReport($attributes) 
	{
		...
		if($attributes['date_from']!=null && $attributes['date_to']!=null) {
			...
			$result = $this->purchasesplit_return->where('purchasesplit_return.status',1)
								   ->join('account_master AS AM', function($join) {
									   $join->on('AM.id','=','purchasesplit_return.supplier_id');
								   })
								   ->leftJoin('jobmaster AS JM', function($join) {
									   $join->on('JM.id','=','purchasesplit_return.job_id');
								   })
								   ->whereBetween('purchasesplit_return.voucher_date', array($date_from, $date_to))
								    ->select('AM.master_name AS supplier','AM.vat_no','purchasesplit_return.*','JM.name AS job')
								   ->orderBY('id', 'ASC')
								   ->get();
		} else {
			$result = $this->purchasesplit_return->where('purchasesplit_return.status',1)
	...
	{ //echo '<pre>';print_r($attributes);exit;
		...
		$department_id = (isset($attributes['department_id']))?$attributes['department_id']:'';	
		$query = $this->purchasesplit_return
					->join('purchasesplit_return_item AS POI', function($join) {
							$join->on('POI.purchasesplit_return_id','=','purchasesplit_return.id');
							})
		...
		$query->whereIn('purchasesplit_return.supplier_id', $attributes['supplier_id']);			
	    //$orderby = ($attributes['search_type']=='jobwise')?'jobcode':'master_name';
		$result = $query->select('purchasesplit_return.voucher_no','purchasesplit_return.reference_no','purchasesplit_return.total','purchasesplit_return.vat_amount','purchasesplit_return.subtotal','purchasesplit_return.supplier_id',
		 						'purchasesplit_return.amount_transfer','purchasesplit_return.discount','purchasesplit_return.total','purchasesplit_return.id AS id','purchasesplit_return.net_amount',
		 						'purchasesplit_return.voucher_date','POI.quantity','AM.id AS cid','POI.unit_price','AM.account_id','AM.master_name','AM.master_name AS supplier',
								'AM.vat_no','POI.tax_code','J.code AS jobcode','purchasesplit_return.amount_transfer AS amount_transfer',
								DB::raw("(SELECT SUM(SI.quantity) FROM purchasesplit_return_item SI WHERE (SI.purchasesplit_return_id=purchasesplit_return.id) AND (SI.status=1) AND (SI.deleted_at='0000-00-00 00:00:00')
		 						)AS quantity") )
		 						->groupBy('purchasesplit_return.id')
								->orderBY('purchasesplit_return.voucher_date','ASC')
							->get()->toArray();
							return $result;
	...
	public function getOtherCost($id)
	{
		...
		return $query->join('pi_other_cost AS pi', function($join) {
							$join->on('pi.purchase_split_id','=','purchasesplit_return.id');
						} )
	...
	public function purchaseInvoiceListCount()
	{
		...
		$query = $this->purchasesplit_return->where('purchasesplit_return.status',1);
		if($deptid!=0)
			$query->where('purchasesplit_return.department_id', $deptid);
	...
	public function purchaseInvoiceList($type,$start,$limit,$order,$dir,$search,$dept=null)
	{
		...
		$query = $this->purchasesplit_return->where('purchasesplit_return.status',1)
						...
						->leftjoin('jobmaster','jobmaster.id','=','purchasesplit_return.job_id');
						
				if($deptid!=0) //dept chk
					$query->where('purchasesplit_return.department_id', $deptid);
				else {
					if($dept!='' && $dept!=0) {
						$query->where('purchasesplit_return.department_id', $dept);
					}
				}
					
				if($search) {
					$query->where(function($qry) use($search) {
						$qry->where('purchasesplit_return.voucher_no','LIKE',"%{$search}%")
							->orWhere('purchasesplit_return.reference_no', 'LIKE',"%{$search}%")
				...
				$query->select('purchasesplit_return.*','am.master_name AS supplier','jobmaster.code AS jobno')
					->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir);
	...
	public function getPurchaseIitems($id, $attributes)
	{
		...
		$date_from = ($attributes['date_from']!='')?date('Y-m-d', strtotime($attributes['date_from'])):'';
		$date_to = ($attributes['date_to']!='')?date('Y-m-d', strtotime($attributes['date_to'])):'';
		$query = $this->purchasesplit_return->where('purchasesplit_return.supplier_id',$id);
		...
		$query->join('account_master AS AM', function($join) {
				...
				if($date_from!='' && $date_to!='')
					$query->whereBetween('purchasesplit_return.voucher_date', [$date_from, $date_to]);
					  
						
		return $query->select('poi.*','im.item_code','iu.packing','iu.is_baseqty','poi.item_total AS line_total',
							 'purchasesplit_return.voucher_no','purchasesplit_return.voucher_date','AM.master_name')
					 ->groupBy('poi.id')
					 ->orderBY('poi.id')
					 ->get();
	}
	...
	public function getTransactionList($attributes) 
	{
		...
		$qry = $this->purchasesplit_return
							...
							if($date_from !='' && $date_to != '')	   
								$qry->whereBetween('purchasesplit_return.voucher_date',[$date_from, $date_to]);
					
		$result = $qry->select('AM.master_name AS supplier','purchasesplit_return.id','purchasesplit_return.voucher_no','purchasesplit_return.voucher_date',
