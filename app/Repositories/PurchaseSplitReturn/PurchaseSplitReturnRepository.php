...
class PurchaseSplitReturnRepository extends AbstractValidator implements PurchaseSplitReturnInterface {
	...
	{ //echo '<pre>';print_r($attributes);exit;
		...
		$result = $query->select('purchasesplit_return.voucher_no','purchasesplit_return.reference_no','purchasesplit_return.total','purchasesplit_return.vat_amount','purchasesplit_return.subtotal','purchasesplit_return.supplier_id',
								...
								DB::raw("(SELECT SUM(SI.quantity) FROM purchasesplit_return_item SI WHERE (SI.purchasesplit_return_id=purchasesplit_return.id) AND (SI.status=1) AND (deleted_at IS NULL)
		 						)AS quantity") )
