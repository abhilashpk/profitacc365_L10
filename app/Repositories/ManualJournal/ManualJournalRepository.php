<?php namespace App\Repositories\ManualJournal;
...
class ManualJournalRepository extends AbstractValidator implements ManualJournalInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->manual_journal->voucher_type  = $this->getVoucherType( $attributes['voucher_type'] );
	...
	private function setInputValueRC($attributes)
	{
		//echo $this->getVoucherType( $attributes['voucher_type'] );
		$this->manual_journal->voucher_type  = $this->getVoucherType( $attributes['voucher_type'] );
		$this->manual_journal->voucher_date  = ($attributes['voucher_date_rc']=='')?date('Y-m-d'):date('Y-m-d', strtotime($attributes['voucher_date_rc'])); //date('Y-m-d', strtotime($attributes['voucher_date']));
		$this->manual_journal->supplier_name = isset($attributes['supplier_name'])?$attributes['supplier_name']:'';
		$this->manual_journal->trn_no = isset($attributes['trn_no'])?$attributes['trn_no']:'';
		$this->manual_journal->group_id = $attributes['group_id'][0];
		$this->manual_journal->department_id = isset($attributes['department_id'])?$attributes['department_id']:'';
	...
	private function setTrInputValue($attributes, $journalEntryTr, $key) 
	{
		...
		$journalEntryTr->manual_journal_id = $this->manual_journal->id;
		...
		if($attributes['group_id'][$key]=='CUSTOMER') { //customer type............
			...
			if( isset($attributes['inv_id'][$key]) ) {
				if($attributes['bill_type'][$key]=='SI') {
					...
					//check if bll is cleared or not...
					$bal = DB::table('manual_journal')->where('id', $attributes['inv_id'][$key])->select('balance_amount')->first();
					if($bal && $bal->balance_amount == 0) {
						DB::table('manual_journal')->where('id', $attributes['inv_id'][$key])->update(['is_transfer' => 1]);
					}
					...
					foreach($attributes['line_amount'] as $key => $value) {
						if($value!='' && $value!=0 && $attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0){
							...
							if($attributes['group_id'][$key]=='CUSTOMER' && $attributes['account_type'][$key]=='Cr' && $attributes['inv_id'][$key]!='')
							{
								...
								$journalVoucherTr->manual_journal_entry_id = $jv_entry_id;
	...
	{ 
		//echo '<pre>';print_r($attributes);exit;
		$this->manual_journal = $this->find($id);
		...
		try {
			...
			if($this->manual_journal->id && !empty( array_filter($attributes['line_amount']))) {
				...
				foreach($attributes['line_amount'] as $key => $value) {
					...
					if($attributes['je_id'][$key]!='') {
						...
						$currentVersion = DB::table('account_transaction')->where('voucher_type', 'MJV')->where('voucher_type_id', $attributes['je_id'][$key])->max('version_no');
						$newVersion = $currentVersion + 1;
						$attributes['version_no'] = $newVersion;

						//SOFT DELETE OLD VERSION
						DB::table('account_transaction')->where('voucher_type', 'MJV')->where('voucher_type_id', $attributes['je_id'][$key])
									->update([
												'status' => 0,
												'deleted_at' => date('Y-m-d h:i:s'),
												'deleted_by'  => Auth::User()->id,
						...
						if($attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0) {
							$journalEntryTr = ManualJournalEntry::find($attributes['je_id'][$key]);
							...
							if($value=='' || $value==0) {
								DB::table('manual_journal_entry')->where('id',$attributes['je_id'][$key])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
								DB::table('account_transaction')->where('voucher_type', 'MJV')->where('voucher_type_id', $attributes['je_id'][$key])->update(['status' => 0, 'deleted_at' => date('Y-m-d h:i:s')]);
							}
				...
				{ //echo '<pre>';print_r($attributes);exit;
					...
					foreach($arrids as $id) {
						$jes = DB::table('manual_journal_entry')
											->leftJoin('account_master', 'account_master.id', '=', 'manual_journal_entry.account_id')
											->select('manual_journal_entry.*','account_master.category')
											->where('manual_journal_entry.id', $id)->first();
						if($jes) {
							DB::table('manual_journal_entry')->where('id', $id)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
							DB::table('pdc_received')->where('entry_id',$id)->where('entry_type','MJV')->where('status',0)->update(['deleted_at' => date('Y-m-d H:i:s')]); 
							...
							if($jes->category=='SUPPLIER') {
								...
								foreach($invs as $inv) {
									if($inv->bill_type=='PI')
										...
										DB::table('manual_journal')->where('id', $inv->purchase_invoice_id)->update(['is_transfer' => 0, 'balance_amount' => DB::raw('balance_amount + '.$inv->assign_amount) ]);
								}
								...
								if($entry) {
									foreach($entry as $ent) {
										if($ent->bill_type=='SI')
											...
											DB::table('manual_journal')->where('id', $ent->sales_invoice_id)->update(['is_transfer' => 0, 'balance_amount' => DB::raw('balance_amount + '.$ent->assign_amount) ]);
			...
			if(bccomp($difference, '0.00', 2) == 0) { 
				...
				$this->manual_journal->supplier_name = isset($attributes['supplier_name'])?$attributes['supplier_name']:'';
				$this->manual_journal->trn_no = isset($attributes['trn_no'])?$attributes['trn_no']:'';
				$this->manual_journal->voucher_date  = ($attributes['voucher_date']=='')?date('Y-m-d'):date('Y-m-d', strtotime($attributes['voucher_date']));
				$this->manual_journal->debit = $dr_amount;
				$this->manual_journal->credit = $cr_amount;
				$this->manual_journal->difference = $difference;
				$this->manual_journal->modify_at = date('Y-m-d H:i:s');
				$this->manual_journal->modify_by = Auth::User()->id;
				$this->manual_journal->fill($attributes)->save();
	...
	{
		$this->manual_journal = $this->manual_journal->find($id);
		
		DB::beginTransaction();
		try {
			
			$rows = DB::table('manual_journal_entry')->where('manual_journal_id', $id)->select('id','account_id','entry_type','amount','cheque_no','bank_id')->get();
			foreach($rows as $row) {
				
				if($row->entry_type=='Dr') {
					$account_id = $row->account_id; $amount = $row->amount;
					if($this->manual_journal->voucher_type=='PDCR') {
					...
					if($entry) {
						foreach($entry as $ent) {
							...
							DB::table('manual_journal_voucher_tr')->where('id', $ent->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
						}
					}

					//Transaction update....
					DB::table('account_transaction')->where('voucher_type', 'MJV')->where('voucher_type_id',$row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'), 'deleted_by' => Auth::User()->id  ]);
				...
				if($row->entry_type=='Cr') {
					$account_id = $row->account_id; $amount = $row->amount;
					if($this->manual_journal->voucher_type=='PDCI') {
					...
					DB::table('manual_journal_entry')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id  ]);
			...
			DB::table('manual_journal_entry')->where('manual_journal_id', $id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id  ]);

			DB::table('manual_journal')->where('id', $id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id  ]);
			$this->manual_journal->delete();
	...
	public function journalListCommon($type)
	{
		$result = $this->manual_journal->where('manual_journal.status', 1)
							 ->join('manual_journal_entry AS JE', function($join) {
								 $join->on('JE.manual_journal_id', '=', 'manual_journal.id');
							 })
							 ...
							 ->select('manual_journal.*','JE.description','JE.reference','AM.master_name')
	...
	{
		$query = $this->manual_journal->where('manual_journal.status', 1)
							 ->join('manual_journal_entry AS JE', function($join) {
								 $join->on('JE.manual_journal_id', '=', 'manual_journal.id');
							 })
							 ->Join('account_master AS AM', function($join) {
							 ...
							 if($search) {
								$query->where(function($query) use ($search){
									$query->where('manual_journal.voucher_no','LIKE',"%{$search}%")
							...
							$query->select('manual_journal.*','JE.description','JE.reference','AM.master_name');
							if($type=='get')
	...
	{
		$query = $this->manual_journal->where('manual_journal.status', 1)->where('manual_journal.voucher_type','MJV')
							 ->join('manual_journal_entry AS JE', function($join) {
								 $join->on('JE.manual_journal_id', '=', 'manual_journal.id');
							 })
							 ->where('JE.status',1)
					  ...
					  if($search) {
					 	 $query->where(function($query) use ($search){
					 		 $query->where('manual_journal.voucher_no','LIKE',"%{$search}%")
					 				->orWhere('JE.description', 'LIKE',"%{$search}%")
					 				->orWhere('JE.reference', 'LIKE',"%{$search}%")
					 				->orWhere('manual_journal.voucher_type', 'LIKE',"%{$search}%");
					 	});
					  }
					 		 $query->select('manual_journal.*','JE.description','JE.reference');
	...
	{
		return $this->manual_journal->where('manual_journal.status', 1)
							 /* ->join('manual_journal_entry AS JE', function($join) {
								 $join->on('JE.manual_journal_id', '=', 'manual_journal.id');
							 })
							 ->where('JE.status',1)
	...
	
	{
		...
		$result = $this->manual_journal->where('manual_journal.status', 1)
									...
									->where('voucher_type','MJV')->where('voucher_no', $voucher_no)
								 ->where('JE.status',1)->where('JE.deleted_at','0000-00-00 00:00:00')
						 			 ->select('manual_journal.*','JE.description')->get();
		elseif($type ==5)
		       $result = $this->manual_journal->where('manual_journal.status', 1)
		                               ->join('manual_journal_entry AS JE', function($join) {
		                                  $join->on('JE.manual_journal_id', '=', 'manual_journal.id');
	...
	
	{
		...
		if($type ==16 )
		    $result = $this->manual_journal->where('manual_journal.status', 1)->where('voucher_type','MJV')->orderBy('id','desc')->first();
		
		elseif($type ==5)
		  $result = $this->manual_journal->where('manual_journal.status', 1)->where('voucher_type','PIN')->orderBy('id','desc')->first();
		elseif($type ==6)
	...
	{
		$result = DB::table('manual_journal_entry')->where('manual_journal_entry.manual_journal_id', $id)
						->join('account_master', 'account_master.id', '=', 'manual_journal_entry.account_id')
						->leftJoin('account_master AS AM', 'AM.id', '=', 'manual_journal_entry.party_account_id')
						->leftJoin('jobmaster AS J', 'J.id', '=', 'manual_journal_entry.job_id')
						->where('manual_journal_entry.status', 1)
						->select('manual_journal_entry.*','account_master.master_name','AM.master_name AS party_name','account_master.category','J.code')
	...
	public function check_vno($refno, $id = null) { 
		...
		if($id)
			return $this->manual_journal->where('voucher_no',$refno)->where('id', '!=', $id)->count();
		else
			return $this->manual_journal->where('voucher_no',$refno)->count();
	}
