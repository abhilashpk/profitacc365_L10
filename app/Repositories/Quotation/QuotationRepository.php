...
class QuotationRepository extends AbstractValidator implements QuotationInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->quotation->reference_no = $attributes['reference_no'] ?? null;
	...
	//	$this->quotation->salesman_id  = $attributes['salesman_id'];
		...
		$this->quotation->job_id 		 = (isset($attributes['job_id']))?($attributes['job_id'] ?? 0):0;
		$this->quotation->header_id 	 = (isset($attributes['header_id']))?($attributes['header_id'] ?? 0):0;
		$this->quotation->footer_id 	 = (isset($attributes['footer_id']))?($attributes['footer_id'] ?? 0):0;
		$this->quotation->is_fc 		  = isset($attributes['is_fc'])?1:0;
		$this->quotation->currency_id   = (isset($attributes['currency_id']))?($attributes['currency_id'] ?? 0):0;
		$this->quotation->currency_rate = (isset($attributes['currency_rate']))?($attributes['currency_rate'] ?? 0):0;
	...
	//	$this->quotation->vehicle_id		= isset($attributes['vehicle_id'])?$attributes['vehicle_id']:'';
		$this->quotation->job_type		= isset($attributes['job_type'])?($attributes['job_type'] ?? 0):0;
		$this->quotation->jobnature		= isset($attributes['jobnature'])?($attributes['jobnature'] ?? 0):0;
		$this->quotation->fabrication		= isset($attributes['fabrication'])?($attributes['fabrication'] ?? 0):0;
		$this->quotation->prefix			= isset($attributes['prefix'])?($attributes['prefix'] ?? 0):0;
	//	$this->quotation->kilometer		= isset($attributes['kilometer'])?$attributes['kilometer']:'';
		$this->quotation->terms_id 	 = isset($attributes['terms_id'])?($attributes['terms_id'] ?? 0):0;
	...
	private function setItemInputValue($attributes, $purchaseOrderItem, $key, $value, $lineTotal) 
	{
		...
		$purchaseOrderItem->vat_amount 		   = $attributes['vatline_amt'][$key] ?? 0;
	...
	public function create($attributes)
	{
		...
		if($this->isValid($attributes)) {
			...
			try {
				...
				$attributes['voucher_no'] = $this->objUtility->generateVoucherNoDoc('QP', $maxNumeric, $dept, $attributes['voucher_no']);
				//VOUCHER NO LOGIC.....................
				...
				try {
						if($this->setInputValue($attributes)) {
							...
							$this->quotation->created_at = date('Y-m-d H:i:s');
			...
			if($this->quotation->id && !empty( array_filter($attributes['item_id']))) {
				...
				if($this->matservice->is_active==1 && $this->quotation->id) {
					...
					if($attributes['lbremove_item']!='')
					{
						...
						$remline_total = $remtax_total = 0;
						foreach($arrids as $row) {
							DB::table('quotation_item')->where('id', $row)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
						}
					}
				}
