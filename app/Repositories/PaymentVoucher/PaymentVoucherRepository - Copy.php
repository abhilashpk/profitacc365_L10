...
class PaymentVoucherRepository extends AbstractValidator implements PaymentVoucherInterface {
		...
		if($this->isValid($attributes)) {
			...
			try {
				...
				if($this->setInputValue($attributes)) {
					...
					$this->payment_voucher->created_by = 1;
	...
	public function PDCIssuedList($date=null)
	{
		...
		if($pdci) {
			$query1 = $this->payment_voucher->where('payment_voucher.status',1)
											->whereIn('payment_voucher.voucher_type', ['PDCI',10]) 
											->where('payment_voucher.is_transfer', 0); 
									if($date)
										 $query1->whereBetween('payment_voucher.voucher_date', [$date->py_from_date, $date->py_to_date]);
						...
						$query1->select('payment_voucher.id','payment_voucher.voucher_no','payment_voucher.voucher_date','payment_voucher.tr_description AS description',
										 ...
										 DB::raw("(SELECT account_master.master_name FROM payment_voucher_entry 
												   JOIN account_master ON(account_master.id = payment_voucher_entry.account_id)
												   WHERE payment_voucher_entry.payment_voucher_id=payment_voucher.id 
												   AND payment_voucher_entry.entry_type='Dr' AND account_master.category='SUPPLIER' LIMIT 0,1) AS creditor"),
										DB::raw("(SELECT account_master.id FROM payment_voucher_entry 
												   JOIN account_master ON(account_master.id = payment_voucher_entry.account_id)
												   WHERE payment_voucher_entry.payment_voucher_id=payment_voucher.id 
												   AND payment_voucher_entry.entry_type='Cr' LIMIT 0,1) AS pdci_id"),
										DB::raw("(SELECT account_master.id FROM payment_voucher_entry 
												   JOIN account_master ON(account_master.id = payment_voucher_entry.account_id)
												   WHERE payment_voucher_entry.payment_voucher_id=payment_voucher.id 
												   AND payment_voucher_entry.entry_type='Dr' AND account_master.category='SUPPLIER' LIMIT 0,1) AS creditor_id"),
										...
										DB::raw("(SELECT bank.id FROM payment_voucher_entry 
												   JOIN bank ON(bank.id = payment_voucher_entry.bank_id)
												   WHERE payment_voucher_entry.payment_voucher_id=payment_voucher.id 
												   AND payment_voucher_entry.entry_type='Cr' LIMIT 0,1) AS bank_id"),
										DB::raw("(SELECT account_id FROM payment_voucher_entry 
												   WHERE payment_voucher_entry.payment_voucher_id=payment_voucher.id 
												   AND payment_voucher_entry.entry_type='Cr' LIMIT 0,1) AS cr_account_id"),
										DB::raw('EXTRACT(MONTH FROM payment_voucher.voucher_date) AS month'),
										DB::raw('"PV" AS type')
									);
			...
			$query2 = DB::table('journal')->where('journal.status',1)
								...
								$query2->select('journal.id','journal.voucher_no','journal.voucher_date','journal_entry.description',
										 ...
										 DB::raw("(SELECT account_master.master_name FROM journal_entry 
												   ...
												   AND journal_entry.entry_type='Cr' LIMIT 0,1) AS debitor"),
										 DB::raw("(SELECT account_master.master_name FROM journal_entry 
												   JOIN account_master ON(account_master.id = journal_entry.account_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Dr' AND account_master.category='SUPPLIER' LIMIT 0,1) AS creditor"),
										DB::raw("(SELECT account_master.id FROM journal_entry 
												   JOIN account_master ON(account_master.id = journal_entry.account_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Cr' LIMIT 0,1) AS pdci_id"),
										DB::raw("(SELECT account_master.id FROM journal_entry 
												   JOIN account_master ON(account_master.id = journal_entry.account_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Dr' AND account_master.category='SUPPLIER' LIMIT 0,1) AS creditor_id"),
										...
										DB::raw("(SELECT bank.id FROM journal_entry 
												   JOIN bank ON(bank.id = journal_entry.bank_id)
												   WHERE journal_entry.journal_id=journal.id 
												   AND journal_entry.entry_type='Cr' LIMIT 0,1) AS bank_id"),
			...
			$query3 = DB::table('petty_cash')->where('petty_cash.status',1)
							...
							$query3->select('petty_cash.id','petty_cash.voucher_no','petty_cash.voucher_date','petty_cash_entry.description',
									 ...
									 DB::raw("(SELECT account_master.master_name FROM petty_cash_entry 
											   ...
											   AND petty_cash_entry.entry_type='Cr' LIMIT 0,1) AS debitor"),
									 DB::raw("(SELECT account_master.master_name FROM petty_cash_entry 
											   JOIN account_master ON(account_master.id = petty_cash_entry.account_id)
											   WHERE petty_cash_entry.petty_cash_id=petty_cash.id 
											   AND petty_cash_entry.entry_type='Dr' AND account_master.category='SUPPLIER' LIMIT 0,1) AS creditor"),
									DB::raw("(SELECT account_master.id FROM petty_cash_entry 
											   JOIN account_master ON(account_master.id = petty_cash_entry.account_id)
											   WHERE petty_cash_entry.petty_cash_id=petty_cash.id 
											   AND petty_cash_entry.entry_type='Cr' LIMIT 0,1) AS pdci_id"),
									DB::raw("(SELECT account_master.id FROM petty_cash_entry 
											   JOIN account_master ON(account_master.id = petty_cash_entry.account_id)
											   WHERE petty_cash_entry.petty_cash_id=petty_cash.id 
											   AND petty_cash_entry.entry_type='Dr' AND account_master.category='SUPPLIER' LIMIT 0,1) AS creditor_id"),
									...
									DB::raw("(SELECT bank.id FROM petty_cash_entry 
											   JOIN bank ON(bank.id = petty_cash_entry.bank_id)
											   WHERE petty_cash_entry.petty_cash_id=petty_cash.id 
											   AND petty_cash_entry.entry_type='Cr' LIMIT 0,1) AS bank_id"),
									DB::raw("(SELECT account_id FROM petty_cash_entry 
											   WHERE petty_cash_entry.petty_cash_id=petty_cash.id 
											   AND petty_cash_entry.entry_type='Cr' LIMIT 0,1) AS cr_account_id"),
									DB::raw('EXTRACT(MONTH FROM petty_cash.voucher_date) AS month'),DB::raw('"PC" AS type')
									);
									
			return $result = $query1->union($query2)->union($query3)->orderBy('cheque_date','ASC')->get();
		} else
			return []; */
		
		$result = DB::table('pdc_issued')->where('pdc_issued.status',0)
