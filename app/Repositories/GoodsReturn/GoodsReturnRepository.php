<?php namespace App\Repositories\GoodsReturn;
...
class GoodsReturnRepository extends AbstractValidator implements GoodsReturnInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->goods_return->job_id = $attributes['job_id'] ?? 0;
		$this->goods_return->description = $attributes['description'] ?? null;
		$this->goods_return->account_master_id = $attributes['account_master_id'];
		$this->goods_return->job_account_id = $attributes['job_account_id'] ?? 0;
	...
	private function setItemInputValue($attributes, $goodsReturnItem, $key, $value, $total_quantity=null)
	{
		...
		$goodsReturnItem->discount = $attributes['line_discount'][$key] ?? 0;
	...
	private function setAccountTransaction($attributes, $amount, $voucher_id, $type, $amount_type=null, $key=null)
	{
		...
		DB::table('account_transaction')
				->insert([  'voucher_type' 		=> 'GR', //JUL22
							...
							'created_by' 		=> 1,
							'description' 		=> $attributes['description'] ?? 'GR',
							'reference'			=> '',
		...
		if($this->isValid($attributes)) {
			...
			try {
				...
				//VOUCHER NO LOGIC.....................
					...
					$qry = DB::table('goods_return')->where('deleted_at', '0000-00-00 00:0:00')->where('status', 1);
					if($dept > 0)	
					...
					while (!$saved && $retryCount < $maxRetries) {
						try {
								if ($this->setInputValue($attributes)) {
									...
									$this->goods_return->created_at = date('Y-m-d H:i:s');
									$this->goods_return->created_by = 1;
									$this->goods_return->save();
	...
	public function update($id, $attributes)
	{
		...
		if($this->setInputValue($attributes)) {
			$this->goods_return->modify_at = date('Y-m-d H:i:s');
		...
		if($attributes['remove_item']!='')
		{
			...
			foreach($arrids as $row) {
				...
				DB::table('goods_return_item')->where('id', $row)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
	...
	public function delete($id)
	{
		...
		DB::table('account_transaction')->where('voucher_type', 'GR')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
