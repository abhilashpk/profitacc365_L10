...
class PurchaseInvoiceRepository extends AbstractValidator implements PurchaseInvoiceInterface {
	...
	private function setItemInputValue($attributes, $purchaseInvoiceItem, $key, $value, $other_cost, $lineTotal, $total_quantity=null)
	{
		...
		$purchaseInvoiceItem->discount = (int)$attributes['line_discount'][$key];
		 ...
		 try {
			 ...
			 if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					if(Session::get('department')==1)
						$inv = DB::table('purchase_invoice')->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->whereNull('deleted_at')->count();
					else
						$inv = DB::table('purchase_invoice')->where('voucher_no',$attributes['voucher_no'])->where('status',1)->whereNull('deleted_at')->count();
			...
			if($this->purchase_invoice->id && !empty( array_filter($attributes['item_id']))) { 
				...
				//VOUCHER NO INCREMENT LOGIC//
				if( $attributes['curno'] == $attributes['voucher_no'] ) {
					$attributes['rowid'] = $this->purchase_invoice->id;
	...
	private function voucherNoGenerate($attributes) {
		...
		do {
			...
			if(Session::get('department')==1)
				$inv = DB::table('purchase_invoice')->where('id','!=',$attributes['rowid'])->where('voucher_no',$newattributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->whereNull('deleted_at')->count();
			else
				$inv = DB::table('purchase_invoice')->where('id','!=',$attributes['rowid'])->where('voucher_no',$newattributes['voucher_no'])->where('status',1)->whereNull('deleted_at')->count();
			//echo $inv.' - ';
			$cnt++;
		} while ($inv!=0);
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					...
					foreach($pirow as $prow) {
						DB::table('item_location_pi')->where('id',$prow->id)->update(['status'=>0,'deleted_at'=>now()]);
	...
	public function delete($id)
	{
		...
		try {
			...
			$items = DB::table('purchase_invoice_item')->where('purchase_invoice_id', $id)->select('id','item_id','quantity','unit_id','unit_price')->get();
			//echo '<pre>';print_r($items);exit;
			//foreach($items as $item) {
		...
		if($attributes['search_type']=="summary") {
			...
			if(isset($attributes['type']))
				...
				$query->select('purchase_invoice.voucher_no','purchase_invoice.voucher_date','purchase_invoice.reference_no','IM.item_code','IM.description','purchase_invoice.total','purchase_invoice.vat_amount',
								'POI.quantity','POI.balance_quantity','POI.unit_price','POI.total_price','AM.account_id','AM.master_name','purchase_invoice.net_amount','POI.item_total',
