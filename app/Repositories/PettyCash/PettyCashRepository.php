...
class PettyCashRepository extends AbstractValidator implements PettyCashInterface {
			...
			if($this->pettycash->id && !empty( array_filter($attributes['line_amount']))) {
				...
				foreach($attributes['line_amount'] as $key => $value) {
					...
					if($attributes['je_id'][$key]!='') {
						...
						$currentVersion = DB::table('account_transaction')->where('voucher_type', 'PC')->where('voucher_type_id', $attributes['je_id'][$key])->max('version_no');
						$newVersion = $currentVersion + 1;
						$attributes['version_no'] = $newVersion;

						//SOFT DELETE OLD VERSION
						DB::table('account_transaction')->where('voucher_type', 'PC')->where('voucher_type_id', $attributes['je_id'][$key])
									->update([
												'status' => 0,
												'deleted_at' => date('Y-m-d h:i:s'),
	...
	public function delete($id)
	{
		...
		try {
			...
			foreach($rows as $row) {
				//Transaction update....
				DB::table('account_transaction')->where('voucher_type', 'PC')->where('voucher_type_id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
	...
	{
		$query = $this->pettycash->where('petty_cash.status', 1)
							 ->join('petty_cash_entry AS PE', function($join) {
								 $join->on('PE.petty_cash_id', '=', 'petty_cash.id');
							 });
