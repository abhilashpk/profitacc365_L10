...
namespace App\Repositories\LocationTransfer;
...
class LocationTransferRepository extends AbstractValidator implements LocationTransferInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->location_transfer->reference_no = $attributes['reference_no'];
		$this->location_transfer->description = $attributes['description'];
		$this->location_transfer->locfrom_id = $attributes['locfrom_id'];
		$this->location_transfer->locto_id = $attributes['locto_id'];
	...
	private function setItemInputValue($attributes, $locationTransferItem, $key, $value) 
	{
		...
		$locationTransferItem->quantity = $attributes['quantity'][$key];
		
		return array('line_total' => $attributes['quantity'][$key]);
		...
		if($this->isValid($attributes)) {
		  ...
		  try {
				if($this->setInputValue($attributes)) {
					...
					$this->location_transfer->created_at = now();
				...
				if($this->location_transfer->id) {
					 DB::table('voucher_no')
						->where('voucher_type', 'LT')
	...
	public function delete($id)
	{
		...
		DB::table('location_transfer_item')->where('location_transfer_id',$id)->update(['status' => 0, 'deleted_at' => now()]);					  
