...
namespace App\Repositories\ProformaInvoice;

use App\Models\ProformaInvoice;
use App\Models\ProformaInvoiceItem;
use App\Models\ProformaInvoiceInfo;
...
class ProformaInvoiceRepository extends AbstractValidator implements ProformaInvoiceInterface {
	...
	public function __construct(ProformaInvoice $proforma_invoice) {
	...
	public function all()
	{
		return $this->proforma_invoice->get();
	}
	
	public function find($id)
	{
		return $this->proforma_invoice->where('id', $id)->first();
	}
	...
	private function setInputValue($attributes)
	{
		...
		$this->proforma_invoice->voucher_no    = $attributes['voucher_no'];
		$this->proforma_invoice->reference_no  = $attributes['reference_no'];
		$this->proforma_invoice->voucher_date  = ($attributes['voucher_date']=='')?date('Y-m-d'):date('Y-m-d', strtotime($attributes['voucher_date']));
		$this->proforma_invoice->lpo_date      = ($attributes['lpo_date']!='')?date('Y-m-d', strtotime($attributes['lpo_date'])):'';
		$this->proforma_invoice->customer_id   = $attributes['customer_id'];
		$this->proforma_invoice->quotation_id  = isset($attributes['quotation_id'])?$attributes['quotation_id']:'';
		$this->proforma_invoice->description   = $attributes['description'];
		$this->proforma_invoice->job_id 		  = isset($attributes['job_id'])?$attributes['job_id']:'';
		$this->proforma_invoice->terms_id 	  = $attributes['terms_id'];
		$this->proforma_invoice->footer_id 	  = isset($attributes['footer_id'])?$attributes['footer_id']:'';
		$this->proforma_invoice->is_fc 		  = isset($attributes['is_fc'])?1:0;
		$this->proforma_invoice->currency_id   = (isset($attributes['currency_id']))?$attributes['currency_id']:'';
		$this->proforma_invoice->currency_rate = (isset($attributes['currency_rate']))?$attributes['currency_rate']:'';
		$this->proforma_invoice->salesman_id  = (isset($attributes['salesman_id']))?$attributes['salesman_id']:'';
		$this->proforma_invoice->is_export		= isset($attributes['is_export'])?1:0;
		$this->proforma_invoice->vehicle_id		= isset($attributes['vehicle_id'])?$attributes['vehicle_id']:'';
		
		$this->proforma_invoice->next_due  = (isset($attributes['next_due'])&&$attributes['next_due']!='')?date('Y-m-d', strtotime($attributes['next_due'])):'';
		$this->proforma_invoice->kilometer		= isset($attributes['kilometer'])?$attributes['kilometer']:'';
		$this->proforma_invoice->present_km		= isset($attributes['present_km'])?$attributes['present_km']:'';
		$this->proforma_invoice->service_km		= isset($attributes['service_km'])?$attributes['service_km']:'';
		$this->proforma_invoice->next_km		= isset($attributes['next_km'])?$attributes['next_km']:'';
		$this->proforma_invoice->job_type		= isset($attributes['job_type'])?$attributes['job_type']:(isset($attributes['jobtype'])?$attributes['jobtype']:''); //SUPPLIMENTRY JOB TYPE
		$this->proforma_invoice->jobnature		= isset($attributes['jobnature'])?$attributes['jobnature']:'';
		$this->proforma_invoice->fabrication		= isset($attributes['fabrication'])?$attributes['fabrication']:(isset($attributes['parent_job'])?$attributes['parent_job']:''); //SUPPLIMENTRY JOB PARENT id
		$this->proforma_invoice->prefix			= isset($attributes['prefix'])?$attributes['prefix']:'';
		$this->proforma_invoice->less_amount	    = isset($attributes['less_amount'])?$attributes['less_amount']:'';
		$this->proforma_invoice->less_description = isset($attributes['less_description'])?$attributes['less_description']:'';
		
		$this->proforma_invoice->less_amount2	    = isset($attributes['less_amount2'])?$attributes['less_amount2']:'';
		$this->proforma_invoice->less_description2 = isset($attributes['less_description2'])?$attributes['less_description2']:'';
		$this->proforma_invoice->less_amount3	    = isset($attributes['less_amount3'])?$attributes['less_amount3']:'';
		$this->proforma_invoice->less_description3 = isset($attributes['less_description3'])?$attributes['less_description3']:'';
		$this->proforma_invoice->location_id = (isset($attributes['location_id']))?$attributes['location_id']:(isset($attributes['department_id'])?$attributes['department_id']:'');
		...
		if(isset($attributes['jobtype']) && $attributes['jobtype']==1) {
			...
			if($attributes['btn']=='submit')
				DB::table('proforma_invoice')->where('id', $attributes['parent_job'])->update(['jctype' => $narr[1]]);
		} 
		
		$this->proforma_invoice->jctype 	  = isset($attributes['jctype'])?$attributes['jctype']:'';
		$this->proforma_invoice->is_warning 	  = isset($attributes['is_warning'])?$attributes['is_warning']:'';
		$this->proforma_invoice->items_inside 	  = isset($attributes['items_inside'])?$attributes['items_inside']:$jbno; //SUPPLIMENTRY PARENT JOB No
		$this->proforma_invoice->remarks 	  = isset($attributes['remarks'])?$attributes['remarks']:'';
		$this->proforma_invoice->signature 	  = isset($attributes['sign'])?($attributes['customer_id'].'_'.date('d-m-Y').'.png') : '';
		$this->proforma_invoice->fuel_level 	  = isset($attributes['fuel_level'])?$attributes['fuel_level']:'';
		...
		if(isset($attributes['opr_description']) && !empty( array_filter($attributes['opr_description'])) ) {
			...
			foreach($attributes['opr_description'] as $key => $value){ 
				if($value != '') {
					...
					$jobOrderDetails->joborder_id = $this->proforma_invoice->id;
	...
	private function setItemInputValue($attributes, $proformaInvoiceItem, $key, $value,$lineTotal) 
	{
		...
		$proformaInvoiceItem->proforma_invoice_id = $this->proforma_invoice->id;
		$proformaInvoiceItem->item_id    	= $value;
		$proformaInvoiceItem->item_name  	= $attributes['item_name'][$key];
		$proformaInvoiceItem->unit_id 		= $attributes['unit_id'][$key];
		$proformaInvoiceItem->quantity   	= $attributes['quantity'][$key];
		$proformaInvoiceItem->unit_price 	= (isset($attributes['is_fc']))?$attributes['cost'][$key]*$attributes['currency_rate']:$attributes['cost'][$key];
		$proformaInvoiceItem->vat		    = isset($attributes['line_vat'][$key])?$attributes['line_vat'][$key]:'';
		$proformaInvoiceItem->vat_amount 	= isset($tax_total)?$tax_total:'';
		$proformaInvoiceItem->discount   	= isset($attributes['line_discount'][$key])?$attributes['line_discount'][$key]:'';
		$proformaInvoiceItem->line_total 	= $line_total;
		$proformaInvoiceItem->tax_code 		= isset($tax_code)?$tax_code:'';
		$proformaInvoiceItem->tax_include 	= isset($attributes['tax_include'][$key])?$attributes['tax_include'][$key]:'';
		$proformaInvoiceItem->item_total 	= $item_total;
		$proformaInvoiceItem->orderno 		= isset($attributes['orderno'][$key])?$attributes['orderno'][$key]:''; //JN23
		
		//CHNG SEP 17
		$proformaInvoiceItem->pay_pcntg 		= $pay_pcntg;
		$proformaInvoiceItem->pay_amount 	= $pay_amount;
		$proformaInvoiceItem->pay_pcntg_desc 		= $pay_pcntg_desc;
		
		return array('line_total' => $line_total, 'tax_total' => $tax_total, 'type' => $type, 'item_total' => $item_total);
	}
	
	private function setInfoInputValue($attributes, $proformaInvoiceInfo, $key, $value)
	{
		$proformaInvoiceInfo->proforma_invoice_id = $this->proforma_invoice->id;
		$proformaInvoiceInfo->title 			= $attributes['title'][$key];
		$proformaInvoiceInfo->description 	= $attributes['desc'][$key];
		return true;
	}
		...
		if($discount > 0)
			...
			foreach($attributes['lbitem_id'] as $key => $value){ 
				...
				if($value!='') {
					...
					$proformaInvoiceItem->proforma_invoice_id = $this->proforma_invoice->id;
					$proformaInvoiceItem->item_id    		= $value;
					$proformaInvoiceItem->item_name  		= $attributes['lbitem_name'][$key];
					$proformaInvoiceItem->unit_id 			= $attributes['lbunit_id'][$key];
					$proformaInvoiceItem->quantity   		= $attributes['lbquantity'][$key];
					$proformaInvoiceItem->unit_price 		= (isset($attributes['is_fc']))?$attributes['lbcost'][$key]*$attributes['currency_rate']:$attributes['lbcost'][$key];
					$proformaInvoiceItem->vat		    	= $attributes['lbline_vat'][$key];
					$proformaInvoiceItem->vat_amount 		= $lbtax_total;
					$proformaInvoiceItem->discount   		= $attributes['lbline_discount'][$key];
					$proformaInvoiceItem->line_total 		= $lbline_total;
					$proformaInvoiceItem->tax_code 			= $attributes['lbtax_code'][$key];
					$proformaInvoiceItem->tax_include 		= $attributes['lbtax_include'][$key];
					$proformaInvoiceItem->item_total 		= $lbitem_total;
					$proformaInvoiceItem->item_type = 1;
					$proformaInvoiceItem->status = 1;
					$itemObj = $this->proforma_invoice->proformaInvoiceItem()->save($proformaInvoiceItem);
	...
	protected function updateService($attributes)
	{
		...
		foreach($attributes['lbitem_id'] as $key => $value) { 
			...
			if($attributes['lborder_item_id'][$key]!='') {
				...
				$proformaInvoiceItem = ProformaInvoiceItem::find($attributes['lborder_item_id'][$key]);
				$oldqty = $proformaInvoiceItem->quantity;
				...
				$exi_quantity = $proformaInvoiceItem->quantity;
				$proformaInvoiceItem->update($items);
	...
	{ //echo '<pre>';print_r($attributes);exit;
		if($this->isValid($attributes)) {
			DB::beginTransaction();
			try {
				//$so = DB::table('voucher_no')->where('voucher_type', 'SO')->where('status',1)->select('no AS voucher_no')->first();
				//$attributes['voucher_no'] = $so->voucher_no;
				
				if($this->setInputValue($attributes)) {
					$this->proforma_invoice->status 	   = 1;
					$this->proforma_invoice->created_at = now();
					$this->proforma_invoice->created_by = 1;
					$this->proforma_invoice->fill($attributes)->save();
					
					//check workshop version active...
					if($this->module->is_active==1 && $this->proforma_invoice->id) {
				...
				if($this->proforma_invoice->id && isset($attributes['item_id']) && !empty( array_filter($attributes['item_id']))) {
					...
					foreach($attributes['item_id'] as $key => $value){ 
						...
						$arrResult 		= $this->setItemInputValue($attributes, $proformaInvoiceItem, $key, $value,$total);
						//if($arrResult['line_total']) {
							...
							$proformaInvoiceItem->status = 1;
							$this->proforma_invoice->proformaInvoiceItem()->save($proformaInvoiceItem);
					...
					DB::table('proforma_invoice')
								->where('id', $this->proforma_invoice->id)
				...
				if(isset($attributes['tempid'])) {
					foreach($attributes['tempid'] as $key => $row) {
						DB::table('crm_info')->insert(['temp_id' => $row, 'textval' => $attributes['crmtext'][$key], 'doc_id' => $this->proforma_invoice->id, 'textval2' => (isset($attributes['crmtext2'][$key]))?$attributes['crmtext2'][$key]:'']);
					}
				}
				
				if($this->proforma_invoice->id && isset($attributes['packageid'])) { 
				
					foreach($attributes['packageid'] as $pkg) {
						DB::table('joborder_pkgs')
									->insert(['job_order_id' => $this->proforma_invoice->id, 
											  'package_id'	=> $pkg
											]);
					}
				}
				
				/* if($this->proforma_invoice->id && isset($attributes['photo_name'])) {
					$photos = explode(',',$attributes['photo_name']);
					foreach($photos as $photo) {
						if($photo!='')
							DB::table('job_photos')->insert(['job_order_id' => $this->proforma_invoice->id, 'photo' => $photo]);
					}
				...
				if($this->proforma_invoice->id && isset($attributes['title']) && !empty( array_filter($attributes['title'])) ) {
					foreach($attributes['title'] as $key => $value) {
						$proformaInvoiceInfo = new ProformaInvoiceInfo();
						if($this->setInfoInputValue($attributes, $proformaInvoiceInfo, $key, $value)) {
							$proformaInvoiceInfo->status = 1;
							$this->proforma_invoice->proformaInvoiceInfo()->save($proformaInvoiceInfo);
						}
					}
				}
				...
				if( ($attributes['curno']!='') && ($this->proforma_invoice->id && $attributes['autoincrement']==1) && ($attributes['curno'] == $attributes['voucher_no']) ) { 
					//update voucher number
					if($this->proforma_invoice->id) {
		...
		//throw new ValidationException('proforma_invoice validation error12!', $this->getErrors());
	}
	
	public function update($id, $attributes)
	{
		$this->proforma_invoice = $this->find($id);
		...
		try {
			...
			$lineTotal = $this->calculateTotalAmount($attributes);
			if($this->proforma_invoice->id && $attributes['item_id'] && !empty( array_filter($attributes['item_id']))) {
				...
				foreach($attributes['item_id'] as $key => $value) { 
					...
					if($attributes['order_item_id'][$key]!='') {
						...
						if( isset($attributes['is_fc']) ) {
							...
							// $is_rental = (isset($attributes['is_rental']))?$attributes['is_rental']:''; 
							$proformaInvoiceItem = ProformaInvoiceItem::find($attributes['order_item_id'][$key]);
				...
				//check workshop version active...
				if($this->matservice->is_active==1 && $this->proforma_invoice->id && isset($attributes['lbitem_id'])) {
					...
					if($attributes['lbremove_item']!='')
					{
						...
						$remline_total = $remtax_total = 0;
						foreach($arrids as $row) {
							DB::table('proforma_invoice_item')->where('id', $row)->update(['status' => 0,'deleted_at' => now()]);
						}
					}
				}
				...
				if(isset($attributes['infoid'])) {
    				foreach($attributes['infoid'] as $key => $value) {
    					if($value=='') {
    						...
    						DB::table('proforma_invoice_info')->where('id',$value)->update(['title' => $attributes['title'][$key], 'description' => $attributes['desc'][$key]]);
    					}
    				}
				}
				...
				if(isset($attributes['tempid'])) {
					DB::table('crm_info')->where('doc_id',$this->proforma_invoice->id)->delete();
					foreach($attributes['tempid'] as $key => $row) {
						DB::table('crm_info')->insert(['temp_id' => $row, 'textval' => $attributes['crmtext'][$key],'doc_id' => $this->proforma_invoice->id, 'textval2' => isset($attributes['crmtext2'][$key])?$attributes['crmtext2'][$key]:'']);
					}
				}
	...
	{
		$this->proforma_invoice = $this->proforma_invoice->find($id);
		
		//Quotatio unlock....
		DB::table('quotation_sales')->where('id',$this->proforma_invoice->quotation_id)->update(['is_transfer' => 0, 'is_editable' => 0]);
			
		$this->proforma_invoice->delete();
	}
	
	public function quotationSalesList()
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
						} )
					->leftJoin('vehicle AS V', function($join) {
							$join->on('V.id','=','proforma_invoice.vehicle_id');
						} )
					->select('proforma_invoice.*','am.master_name AS customer','V.reg_no','V.name AS vehicle')
	...
	public function check_reference_no($refno, $id = null) { 
		...
		if($id)
			return $this->proforma_invoice->where('reference_no',$refno)->where('id', '!=', $id)->count();
		else
			return $this->proforma_invoice->where('reference_no',$refno)->count();
	}
		
	public function getCustomerOrder($customer_id, $type=null)
	{
		 $query = $this->proforma_invoice
						->leftJoin('vehicle AS V', function($join) {
							$join->on('V.id','=','proforma_invoice.vehicle_id');
						} )
						->leftJoin('jobmaster AS J', function($join) {
							$join->on('J.id','=','proforma_invoice.job_id');
						} )
					 ->where('proforma_invoice.status', 1)
					 ->where('proforma_invoice.customer_id', $customer_id)
					 ->where('proforma_invoice.is_transfer', 0);
					
					if($type) {
						$query->where(function($qry) {
							$qry->where('proforma_invoice.salesman_id', 0)->where('proforma_invoice.doc_status', 0)
								->orWhere('proforma_invoice.salesman_id','>',0)->where('proforma_invoice.doc_status', 1);
						});
					}
					 
		return	$query->select('proforma_invoice.id','proforma_invoice.voucher_no','proforma_invoice.voucher_date','proforma_invoice.net_total','V.reg_no','V.name AS vehicle',
							   'proforma_invoice.prefix','J.code')->get();
		
	}
	
	public function findOrderData($id)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.id', $id)->where('proforma_invoice.is_transfer', 0);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
						} )
					->leftJoin('header_footer AS f',function($join) {
						$join->on('f.id','=','proforma_invoice.footer_id');
					})
					->leftJoin('vehicle AS V', function($join) {
							$join->on('V.id','=','proforma_invoice.vehicle_id');
						} )
					->leftJoin('salesman AS S', function($join) {
							$join->on('S.id','=','proforma_invoice.salesman_id');
						} )
					->select('proforma_invoice.*','am.master_name AS customer','f.title AS footer','V.reg_no','V.name AS vehicle',
							 'S.name AS salesman','V.issue_plate','V.code_plate','V.make','V.model')
					->orderBY('proforma_invoice.id', 'ASC')
					->first();
	}
	...
	public function getOrderReport($attributes) 
	{
		...
		if($attributes['date_from']!=null && $attributes['date_to']!=null) {
			...
			$result = $this->proforma_invoice->where('proforma_invoice.status',1)
								    ->join('account_master AS AM', function($join) {
									   $join->on('AM.id','=','proforma_invoice.customer_id');
								   })
								   ->leftJoin('jobmaster AS JM', function($join) {
									   $join->on('JM.id','=','proforma_invoice.job_id');
								   })
								   ->whereBetween('voucher_date', array($date_from, $date_to))
								   ->select('AM.master_name AS supplier','proforma_invoice.*','JM.name AS job')
								   ->orderBY('id', 'ASC')
								   ->get();
		} else {
			$result = $this->proforma_invoice->where('proforma_invoice.status',1)
	...
	{
		$order = $this->proforma_invoice->where('proforma_invoice.id', $attributes['document_id'])
								   ->join('account_master AS AM', function($join) {
									   $join->on('AM.id','=','proforma_invoice.customer_id');
								   })
								   ->leftJoin('currency AS C', function($join) {
									   $join->on('C.id','=','proforma_invoice.currency_id');
								   })
								   ->leftJoin('jobmaster AS J', function($join) {
									   $join->on('J.id','=','proforma_invoice.job_id');
								   })
								    ...
								    ->leftJoin('salesman AS SL', function($join) {
									   $join->on('SL.id','=','proforma_invoice.salesman_id');
								   })
								   ->leftJoin('terms AS TR', function($join) {
									   $join->on('TR.id','=','proforma_invoice.terms_id');
								   })
								   ->select('AM.account_id','AM.master_name AS supplier','proforma_invoice.*','AM.address','AM.city','AM.state','AM.pin','AM.vat_no','AM.phone','C.name AS currency',
								   'V.name AS vehicle','V.reg_no','V.make','V.color','V.issue_plate','V.code_plate','SL.name AS salesman','TR.description AS terms')
								   ->orderBY('proforma_invoice.id', 'ASC')
								   ->first();
								   
		$items = $this->proforma_invoice->where('proforma_invoice.id', $attributes['document_id'])
								   ->join('proforma_invoice_item AS PI', function($join) {
									   $join->on('PI.proforma_invoice_id','=','proforma_invoice.id');
								   })
	...
	{
		$query = $this->proforma_invoice->where('proforma_invoice.id', $id);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
						} )
					->leftjoin('jobmaster AS J', function($join){
					...
					->select('proforma_invoice.*','am.master_name AS customer','f.title AS footer','S.name AS salesman','V.name AS vehicle','J.name',
	...
	public function findPOdataold($id)
	{
		...
		return $query->join('account_master AS am', function($join) {
					...
					->select('proforma_invoice.*','am.master_name AS customer','f.title AS footer','S.name AS salesman','V.name AS vehicle',
							 'V.reg_no','V.model','V.make','V.issue_plate','V.code_plate','am.email')
					->first();
	}
	...
	public function getItems($id,$mod=null)
	{
		...
		$query->join('proforma_invoice_item AS poi', function($join) {
							$join->on('poi.proforma_invoice_id','=','proforma_invoice.id');
						} )
	...
	public function getSOItems($id)
	{
		...
		$query = $this->proforma_invoice->whereIn('proforma_invoice.id',$id);
		
		return $query->join('proforma_invoice_item AS poi', function($join) {
							$join->on('poi.proforma_invoice_id','=','proforma_invoice.id');
						} )
					  ->leftjoin('units AS u', function($join){
						  $join->on('u.id','=','poi.unit_id');
	...
	public function getVehicleExpiryInfo() {
		//	$parameter1 = DB::table('proforma_invoice')->where('proforma_invoice.is_editable','=',1)->get();
	...
	//	->where('proforma_invoice.is_editable','=',1)
	//->whereBetween('proforma_invoice.next_due', array($fromdate,$endDate))
			$query = $this->proforma_invoice	->where('proforma_invoice.is_editable','=',1)
			->where('proforma_invoice.next_due','!=',0);
			$doc_warnda =  $query->join('account_master AS am', function($join) {
								$join->on('am.id','=','proforma_invoice.customer_id');
							} )->leftJoin('sales_invoice AS SI', function($join) {
								$join->on('SI.document_id','=','proforma_invoice.id');
							} )
						...
						->select('proforma_invoice.*','am.master_name AS customer','proforma_invoice.next_due AS expiry_date','V.name AS vehicle',
								 'V.reg_no','V.model','V.make','V.issue_plate','V.code_plate','am.email')
						->get();
	...
	public function getPendingReport($attributes)
	{
		...
		switch($attributes['search_type']) {
			case 'summary':
				$query = $this->proforma_invoice
								->join('proforma_invoice_item AS SOI', function($join) {
									$join->on('SOI.proforma_invoice_id','=','proforma_invoice.id');
								})
								->join('account_master AS AM', function($join) {
									$join->on('AM.id','=','proforma_invoice.customer_id');
								})
						...
						if( $date_from!='' && $date_to!='' ) { 
							$query->whereBetween('proforma_invoice.voucher_date', array($date_from, $date_to));
						}
						
						if($attributes['salesman']!='') { 
							$query->where('proforma_invoice.salesman_id', $attributes['salesman']);
						}
						
						if(isset($attributes['job_id']) && $attributes['job_id']!='')
								$query->whereIn('proforma_invoice.job_id', $attributes['job_id']);	
							
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount','S.name AS salesman',
									  'SOI.quantity','SOI.balance_quantity','SOI.unit_price','AM.master_name','proforma_invoice.net_total','proforma_invoice.discount',
									  'V.reg_no','V.issue_plate','V.code_plate','proforma_invoice.voucher_date','J.code AS jobcode','J.name AS jobname','J.code AS jobcode')
								->groupBy('proforma_invoice.id')->get();
				break;
			...
			case 'jobwise':
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount','S.name AS salesman',
									  'SOI.quantity','SOI.balance_quantity','SOI.unit_price','AM.master_name','proforma_invoice.net_total','proforma_invoice.discount',
									  'proforma_invoice.voucher_date','J.code AS jobcode','J.name AS jobname','proforma_invoice.less_amount','proforma_invoice.less_amount2','proforma_invoice.less_amount3')
								->orderBY('jobcode','ASC')
								->groupBy('proforma_invoice.id')->get();
				break;
			...
			case 'summary_pending':
				$query = $this->proforma_invoice->where('SOI.is_transfer','!=',1)
								->join('proforma_invoice_item AS SOI', function($join) {
									$join->on('SOI.proforma_invoice_id','=','proforma_invoice.id');
								})
								->join('account_master AS AM', function($join) {
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount','proforma_invoice.discount','J.code AS jobcode',
									  'SOI.quantity','SOI.balance_quantity','J.code AS jobcode','J.name AS jobname','SOI.unit_price','AM.master_name','proforma_invoice.net_total','S.name AS salesman','SOI.vat_amount AS unit_vat',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();//->groupBy('proforma_invoice.id')
			...
			case 'detail':
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','IM.item_code','IM.description','S.name AS salesman','SOI.vat_amount AS unit_vat','J.code as jobcode',
									  'SOI.quantity','SOI.unit_price','SOI.line_total','J.code AS jobcode','J.name AS jobname','AM.master_name','proforma_invoice.net_total','proforma_invoice.discount',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();
				break;
				
			case 'detail_pending':
				$query = $this->proforma_invoice->where('QSI.is_transfer','!=',1)
								->join('proforma_invoice_item AS QSI', function($join) {
									$join->on('QSI.proforma_invoice_id','=','proforma_invoice.id');
								})
								->join('account_master AS AM', function($join) {
									$join->on('AM.id','=','proforma_invoice.customer_id');
								})
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','IM.item_code','IM.description','proforma_invoice.total','proforma_invoice.vat_amount','proforma_invoice.discount','J.code AS jobcode',
									  'QSI.quantity','QSI.balance_quantity','QSI.unit_price','J.code AS jobcode','J.name AS jobname','QSI.line_total','AM.master_name','proforma_invoice.net_total','S.name AS salesman',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();
				break;
		}
	...
	public function getPendingReportrental($attributes)
	{
		...
		switch($attributes['search_type']) {
			case 'summary':
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount','S.name AS salesman',
									  'SOI.quantity','SOI.balance_quantity','SOI.unit_price','AM.master_name','proforma_invoice.net_total','proforma_invoice.discount',
									  'V.reg_no','V.issue_plate','V.code_plate','proforma_invoice.voucher_date','J.code AS jobcode','J.name AS jobname')
								->groupBy('proforma_invoice.id')->get();
				break;
			...
			case 'summary_pending':
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount','proforma_invoice.discount',
									  'SOI.quantity','SOI.balance_quantity','J.code AS jobcode','J.name AS jobname','SOI.unit_price','AM.master_name','proforma_invoice.net_total','S.name AS salesman','SOI.vat_amount AS unit_vat',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();//->groupBy('proforma_invoice.id')
			...
			case 'detail':
				$query = $this->proforma_invoice
						...
						if(isset($attributes['job_id']) && $attributes['job_id']!='')
						$query->whereIn('jobmaster.id', $attributes['job_id']);
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','IM.item_code','IM.description','S.name AS salesman','SOI.vat_amount AS unit_vat',
									  'SOI.quantity','SOI.unit_price','SOI.line_total','J.code AS jobcode','J.name AS jobname','AM.master_name','proforma_invoice.net_total','proforma_invoice.discount',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();
				break;
			...
			case 'detail_pending':
				$query = $this->proforma_invoice->where('QSI.is_transfer','!=',1)
						...
						if(isset($attributes['job_id']) && $attributes['job_id']!='')
						$query->whereIn('jobmaster.id', $attributes['job_id']);
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','IM.item_code','IM.description','proforma_invoice.total','proforma_invoice.vat_amount','proforma_invoice.discount',
									  'QSI.quantity','QSI.balance_quantity','QSI.unit_price','J.code AS jobcode','J.name AS jobname','QSI.line_total','AM.master_name','proforma_invoice.net_total','S.name AS salesman',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();
				break;
	...
	{
		return DB::table('proforma_invoice')
						->join('proforma_invoice_item AS QSI', function($join) {
							$join->on('QSI.proforma_invoice_id', '=', 'proforma_invoice.id');
						})
	...
	public function check_voucher_no($refno, $id = null) { 
		...
		if($id)
			return $this->proforma_invoice->where('voucher_no',$refno)->where('id', '!=', $id)->count();
		else
			return $this->proforma_invoice->where('voucher_no',$refno)->count();
	}
	...
	public function proformaInvoiceListCountRent()
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1);
		//->where('is_rental',1)
		return $query->join('account_master AS am', function($join) {
	...
	public function proformaInvoiceListrental($type,$start,$limit,$order,$dir,$search)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
				...
				if($search) {
					$query->where('proforma_invoice.voucher_no','LIKE',"%{$search}%")
                          ->orWhere('am.master_name', 'LIKE',"%{$search}%");
				}
				
				$query->select('proforma_invoice.*','am.master_name AS customer','V.reg_no','V.name AS vehicle')
					->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir);
	...
	public function proformaInvoiceListCount()
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
	...
	public function proformaInvoiceList($type,$start,$limit,$order,$dir,$search)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
				...
				if($search) {
					...
					->orWhere('proforma_invoice.reference_no', 'LIKE',"%{$search}%")
                          ->orWhere('am.master_name', 'LIKE',"%{$search}%");
				}
				
				$query->select('proforma_invoice.*','am.master_name AS customer','V.reg_no','V.name AS vehicle',
								DB::raw('SUBSTRING_INDEX(proforma_invoice.voucher_no, "/", 1) AS bin_name1'))
								//DB::raw('SUBSTRING_INDEX(proforma_invoice.voucher_no, "/", -1) AS bin_name2'))
					->offset($start)
                    ->limit($limit)
					//->orderBy($order,$dir)
					->orderBy('bin_name1','DESC');
					//->orderBy('bin_name2','ASC');
					//->sortBy('proforma_invoice.voucher_no');
	...
	public function proformaInvoicePendingList($type,$start,$limit,$order,$dir,$search)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
						...
						->where('proforma_invoice.is_warning',0);
				...
				$query->select('proforma_invoice.*','am.master_name AS customer','V.reg_no','V.name AS vehicle')
					->offset($start)
                    ->limit($limit)
                    ->groupBy('soi.proforma_invoice_id')
                    ->orderBy($order,$dir);
	...
	public function proformaInvoicePendingListCom($type,$start,$limit,$order,$dir,$search)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						->join('account_master AS am', function($join) {
							$join->on('am.id','=','proforma_invoice.customer_id');
						...
						->where('proforma_invoice.is_warning',1);
	...
	public function getPaymentCertificate($attributes)
	{
		...
		switch($attributes['search_type']) {
			case 'jobwise':
				$query = $this->proforma_invoice
						...
						if($attributes['jobmaster_id']!='') { 
							$query->where('proforma_invoice.job_id', $attributes['jobmaster_id']);
						}
						 
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount',
									  'SOI.quantity','SOI.balance_quantity','SOI.unit_price','AM.master_name','proforma_invoice.net_total',
									  'proforma_invoice.voucher_date')
								->groupBy('proforma_invoice.id')->get();
				break;
			...
			case 'summary_pending':
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','proforma_invoice.total','proforma_invoice.vat_amount','proforma_invoice.discount',
									  'SOI.quantity','SOI.balance_quantity','SOI.unit_price','AM.master_name','proforma_invoice.net_total','S.name AS salesman','SOI.vat_amount AS unit_vat',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();//->groupBy('proforma_invoice.id')
			...
			case 'detail':
				...
				return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','IM.item_code','IM.description','S.name AS salesman','SOI.vat_amount AS unit_vat',
									  'SOI.quantity','SOI.unit_price','SOI.line_total','AM.master_name','proforma_invoice.net_total','proforma_invoice.discount',
									  'V.reg_no','V.issue_plate','V.code_plate')
								->get();
				break;
		}
	...
	public function TechnicianOrderList($type,$search=null)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						...
						->leftjoin('salesman AS S', 'S.id','=','proforma_invoice.salesman_id')
						->leftJoin('vehicle AS V', function($join) {
							$join->on('V.id','=','proforma_invoice.vehicle_id');
						} );
				
				if(Session::has('technician_id')){
				    $query->where('proforma_invoice.salesman_id', Session::get('technician_id'));
						 // ->where('proforma_invoice.doc_status',0)
				}
				...
				if($type=='Assigned') {
					$query->where('proforma_invoice.start_time','')
							->where('proforma_invoice.end_time','');
				}
				...
				if($type=='Approved') {
					$query->where('proforma_invoice.start_time','!=','')
							->where('proforma_invoice.end_time','!=','')
							->where('proforma_invoice.doc_status',1);
				}
				
			$result = $query->select('proforma_invoice.*','am.master_name AS customer','V.reg_no','V.name AS vehicle','S.name AS salesman')->get();
	...
	public function TechnicianOrderListById($techid,$type)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
				...
				if($techid && $techid!='all') {
					$query->where('proforma_invoice.salesman_id',$techid);
				}
	...
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						->join('proforma_invoice_item AS IT', function($join) {
							$join->on('IT.proforma_invoice_id','=','proforma_invoice.id');
						})
						->join('itemmaster AS IM', function($join) {
			...
			$result = $query->select('proforma_invoice.id','IM.description','IT.quantity')->get();
	...
	public function VehicleDetails($type)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
			...
			$result = $query->select('proforma_invoice.id','V.reg_no','V.issue_plate','V.code_plate','V.make','V.model')->get();
	...
	public function getJobImages($type)
	{
		$query = $this->proforma_invoice->where('proforma_invoice.status',1)
						->join('job_photos AS P', function($join) {
							$join->on('P.job_order_id','=','proforma_invoice.id');
						});
			...
			$result = $query->select('proforma_invoice.id','P.photo','P.description')->get();
	...
	public function getReportById($id) {
		...
		return $query->select('proforma_invoice.voucher_no','proforma_invoice.reference_no','IM.item_code','IM.description','proforma_invoice.total','sales_order.vat_amount','sales_order.discount',
							  'QSI.quantity','QSI.balance_quantity','QSI.unit_price','QSI.line_total','proforma_invoice.net_total')
							->get();
	}
