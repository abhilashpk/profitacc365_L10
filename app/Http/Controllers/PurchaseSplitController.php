...
class PurchaseSplitController extends Controller
{
	...
	public function __construct(PurchaseSplitInterface $purchase_split, SupplierDOInterface $supplierdo, PurchaseOrderInterface $purchase_order, ItemmasterInterface $itemmaster, TermsInterface $terms, JobmasterInterface $jobmaster, AccountMasterInterface $accountmaster, CurrencyInterface $currency, VoucherNoInterface $voucherno, AreaInterface $area,CountryInterface $country,AcgroupInterface $group, LocationInterface $location, AccountSettingInterface $accountsetting, FormsInterface $forms, MaterialRequisitionInterface $material_requisition) {
		...
		$this->group = $group;
		$this->forms = $forms;
		$this->formData = $this->forms->getFormData('PS');
    ...
    public function index() {
		...
		return view('body.purchasesplit.index')
					->withOrders($orders)
	...
	public function add(Request $request, $id = null, $doctype = null) {
		...
		$views=	($this->mod_ps_pettycash->is_active==1)?'addpc':'add';
		
			//echo '<pre>';print_r($views);exit;
		return view('body.purchasesplit.'.$views)
					->withItems($itemmaster)
					->withUnits($units)
					->withTerms($terms)
					->withJobs($jobs)
	...
	public function edit($id) { 
		...
		$views=	($orderrow->is_pettycash==1)?'editps':'edit';
		return view('body.purchasesplit.'.$views)
					->withItems($itemmaster)
	...
	public function getPS($did=null)
	{
		$data = array();
		$pidata = $this->purchase_split->getPSRdata($did);//echo '<pre>';print_r($pidata);exit;
