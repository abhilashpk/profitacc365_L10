...
class SalesOrderBookingController extends Controller
{
	...
	public function view($id) { 
		...
		$orditems = $this->sales_order->getItems($id);
		$subitems = [];
		$locations = DB::table('location')->where('is_default',0)->where('status',1)->whereNull('deleted_at')->select('id','name')->get();
		//echo '<pre>';print_r($orditems);exit;
		foreach($orditems as $ik => $orow) {

			$template[$ik] = DB::table('item_template')->where('item_template.item_id', $orow->item_id)
							->where('item_template.deleted_at',null)
							->leftJoin('units','units.id','=','item_template.unit_id')
							->select('item_template.*','units.unit_name')
							->orderBy('item_template.order_no','ASC')->get();
				...
				if($request->get('status')==1) {
					...
					if($sorow) {
						...
						if($girow) {  //GOES TO UPDATE CURRENT ROW ID WITH GI
							...
							$gitms = DB::table('goods_issued_item')->where('goods_issued_id',$girow->id)->where('status',1)->whereNull('deleted_at')->select('*')->get();
							...
							$gitms = DB::table('goods_issued_item')->where('goods_issued_id',$sorow->id)->where('status',1)->whereNull('deleted_at')->select('*')->get();
					...
					//echo  $edit;exit;
					//echo '<pre>';print_r($request->all());exit;  so_joborder
					if($edit==true) {
					...
					$row1 = DB::table('sales_order_item')->where('sales_order_id', $request->get('id'))->where('status',1)->whereNull('deleted_at')->count();
					$row2 = DB::table('sales_order_item')->where('sales_order_id', $request->get('id'))->where('status',1)->whereNull('deleted_at')->where('discount',1)->count();
					if($row1==$row2) {
					...
					if($girow) {
						...
						$cnt = DB::table('goods_issued_item')->where('goods_issued_id',$girow->gi_id)->where('status',1)->whereNull('deleted_at')->count();
						if($cnt==0)
	...
	public function driverAssign(Request $request) {
		...
		if($request->get('ordid')!==null) {
			foreach($request->get('ordid') as $id) {
				
				DB::table('sales_order')->where('id', $id)->update(['job_type' => 1]);
				
				DB::table('order_assign')->insert(['driver_id' => $request->get('driver_id'), 'order_id' => $id, 'assign_date' => date('Y-m-d',strtotime($request->get('assign_date'))), 'tr_status' => 0 ]);
			}
