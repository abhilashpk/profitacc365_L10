...
class SupplierPaymentController extends Controller
{
	...
	public function save(Request $request) { //echo '<pre>';print_r($request->all());exit;
		...
		$id=$this->payment_voucher->create(Input::all());
	...
	public function update(Request $request, $id)
	{
		...
		$attributes	= Input::all();
		if( $this->payment_voucher->update($id, Input::all()) )
       {
		### Mail 
		if($attributes['send_email']==1) {
		$data['crrow']= DB::table('payment_voucher')
...
public function getSearch()
	{
	    ...
	    if(Input::get('search_type')=="summary")
		{
			...
			$report = $this->payment_voucher->getReport(Input::all());
...
public function dataExport()
	...
	//	$reports = $this->purchase_invoice->getReportExcel(Input::all());
		...
		{
			$voucher_head = 'Payment Voucher Summary';
			$reports = $this->payment_voucher->getReport(Input::all());
	...
	public function checkVchrNo() {
		...
		$check = $this->payment_voucher->check_voucher_no(Input::get('voucher_no'), Input::get('id'));
