...
class ItemTemplateController extends Controller
{
	...
	public function getTemplate($id,$no) {
		...
		foreach($template as $row) {
			//$items[] = ['name' => $row->type_name, 'const' => str_replace(' ', '_',$row->type_name)];
			$cls = str_replace(' ', '_',$row->type_name);
			if($one)
				$links .= ($links=='')?'<a href="#" class="view" data-id="'.$cls.'">'.$row->type_name.'</a>':' | '.'<a href="#" class="view" data-id="'.$cls.'">'.$row->type_name.'</a>';
			
			$one = true;
		}
		//echo '<pre>';print_r($template);
		return view('body.itemtemplate.template')
					->withTemplate($template)->withNo($no)->withItemid($id)->withLinks($links);
	}
	
	public function getTemplateEdit($id,$jid,$no) {
		
		$template = DB::table('item_template_edit')->where('item_template_edit.item_id',$id) //OCT
							->where('item_template_edit.deleted_at',null)
							->where('item_template_edit.so_item_id',$jid)
							->leftJoin('units','units.id','=','item_template_edit.unit_id')
							->select('item_template_edit.*','units.unit_name')
							->orderBy('item_template_edit.order_no','ASC')
		...
		foreach($template as $k => $trow) {
			if($trow->input_type=="item") {
				$items[$k] = DB::table('itemmaster')->where('group_id',$trow->group_id)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','item_code AS text')->get();
			}
		}
	...
	public function getItemsbkp($gid) { //print_r(Input::all());
	...
	public function getItems($gid) { //print_r(Input::all());
		if(Input::get('term')!='')
		   $items = DB::table('itemmaster')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->where('item_code','like','%'.Input::get('term').'%')->select('id','item_code AS text')->get();
	   else
		   $items = DB::table('itemmaster')->where('group_id',$gid)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','item_code AS text')->get();
	...
	public function saveJoborder(Request $request) {
		...
		foreach($attributes['input_type'] as $key => $val) {
			...
			if($val == 'item' && $attributes['is_dimension'][$key]==1) {
				$other_value = ['height' => isset($attributes['height'][$key])?$attributes['height'][$key]:'', 'width' => isset($attributes['width'][$key])?$attributes['width'][$key]:'', 'quantity' => isset($attributes['quantity'][$key])?$attributes['quantity'][$key]:'', 'remarks' => isset($attributes['remarks'][$key])?$attributes['remarks'][$key]:'']; //OCT
			...
			if(isset($attributes['input_itemsub'][$key])) {
				...
				$subitem = null;
			$arrVal[$i] = ['input_type' => $val, 'input_value' => $attributes['input_item'][$key], 'other_value' => $other_value, 'dmond' => $subitem];
			
			//OCT
			if($attributes['tplid'][$key]=='') {

				$it_id[] = DB::table('item_template_edit')
							->insertGetId([
								'so_item_id' => isset($attributes['jid'])?$attributes['jid']:'',
								'item_id' => $attributes['item_id'],
								'type_name' => $attributes['type_name'][$key],
								'input_type'	=> $attributes['input_type'][$key],
								'is_stock'	=> isset($attributes['is_stockitm'][$key])?$attributes['is_stockitm'][$key]:0,
								'unit_id'	=> isset($attributes['unit'][$key])?$attributes['unit'][$key]:0,
								'group_id'	=> isset($attributes['grp'][$key])?$attributes['grp'][$key]:0,
								'is_dimension'	=> isset($attributes['is_dimension'][$key])?$attributes['is_dimension'][$key]:0,
								'order_no'	=> $key+1,
								'is_required'	=> $attributes['required'][$key]
							]);
			} else {
				DB::table('item_template_edit')
							->where('id', $attributes['tplid'][$key])
							->update([
								'item_id' => $attributes['item_id'],
								'type_name' => $attributes['type_name'][$key],
								'input_type'	=> $attributes['input_type'][$key],
		...
		if($attributes['jid']=='') {
			$id = DB::table('so_joborder')
						->insertGetId([
							'job_value' => json_encode($vals) //$attributes['form_tmplt']
						]);
			foreach($it_id as $it) {			
				DB::table('item_template_edit')
						->where('id', $it)
