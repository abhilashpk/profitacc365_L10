...
class SysparameterController extends Controller
{
	...
	public function para1_update($id)
	{ 
		$this->parameter1->update($id, Input::all());
		Session::flash('message', 'Sysparameter details updated successfully');
		return redirect('sysparameter');
	}
	
	public function para2_update()
	{ 
		//echo '<pre>';print_r(Input::all());exit;
		$this->parameter2->update(null, Input::all());
	...
	{ 
		...
		DB::table('default_loc')->where('id',Input::get('dftlocid'))->update(['pur_loc' => Input::get('loc_purchase'), 'sales_loc' => Input::get('loc_sales'), 'mfg_loc' => Input::get('loc_mfg')]); 
	...
	public function para4_update($id)
	{ 
		$this->parameter4->update($id, Input::all());
		Session::flash('message', 'Sysparameter details updated successfully');
		return redirect('sysparameter');
	}
	
	public function para5_update()
	{ 
		DB::table('design_view')->where('id',1)->update(['view_name' => Input::get('file_name')]);
		Session::flash('message', 'Sysparameter details updated successfully');
		return redirect('sysparameter');
	}
}
