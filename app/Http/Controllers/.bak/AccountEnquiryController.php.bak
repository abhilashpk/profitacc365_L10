...
class AccountEnquiryController extends Controller
{
	...
	public function searchAccount()
	{
		...
		//***************NOT IN USE		
			...
			$sarr = explode('|',Input::get('salesman_id')); 
			...
			if(Input::get('type')=='statement') {
				...
				if(Input::get('is_con')==1) {
					...
					$transactions1 = $this->makeConsolidated($this->accountmaster->getPrintViewByAccount(Input::all()));
			...
			//	echo '<pre>';print_r($resultrow);exit;
				...
				if(Input::get('is_custom')==1 && !empty(Input::get('salesman_id'))) {
					$transactions = $this->SortByCategory($transactions);
					foreach($transactions as $key => $val) {
						$headarr[$key] = DB::table('account_category')->where('id',$key)->select('name AS heading')->first();
					}
			...
			} else if(Input::get('type')=='ageing'){ //AGEING......
			...
			} else if(Input::get('type')=='ageing_summary'){ //AGEING......	
				//AGEING SUMMARY.....
				$voucher_head = ($infc=='')?'Statement of Account - Ageing Summary':'Statement of Account - Ageing Summary in FC';
				$query = DB::table('account_master')->where('status',1)->where('deleted_at','0000-00-00 00:00:00');
				$inputs = Input::all(); //echo '<pre>';print_r(Input::all());exit;
		...
		if($is_default==1) {
			...
			if(Input::get('type')=='ageing') {
				...
				if(Input::get('type')=='statement') {
					...
					if(Input::get('is_con')==1) {
						$reports = $this->SortByAccount($this->makeConsolidated( $this->accountmaster->getPrintViewByAccount(Input::all())) );
					} else 
						$reports = $this->SortByAccount($this->accountmaster->getPrintViewByAccount(Input::all()));
					...
					$pdcreports = $this->accountmaster->getPDCPrintViewByAccount(Input::all());
						...
						if(count($pdcreports) > 0) {
							...
							foreach($pdcreports as $transaction) {
								...
								$datareport[] = ['voucher_type' => ($transaction->voucher_type=='PDCR')?'RV':'PV',
												 ...
												 'vdate'	=> ($transaction->voucher_date=='0000-00-00' || $transaction->voucher_date=='01-01-1970')?date('d-m-Y', strtotime($settings->from_date)):date('d-m-Y', strtotime($transaction->voucher_date)),
				...
				//$datareport[] = ['','','','','Grand Total',$Sdr_total,$Scr_total,$Sbalance_prnt];
					...
					$pdcreports = $this->accountmaster->getPDCPrintViewByAccount(Input::all(),'OS');
					...
					if(Input::get('is_pdc')==1) { 
						if(count($pdcreports) > 0) {
							...
							foreach($pdcreports as $transaction) {
								...
								$datareport[] = [ 'date' => ($transaction->voucher_date=='0000-00-00' || $transaction->voucher_date=='01-01-1970')?date('d-m-Y', strtotime($settings->from_date)):date('d-m-Y', strtotime($transaction->voucher_date)),
				...
				} else if(Input::get('type')=='ageinggroup') {
					
					$accounts = DB::table('account_master')->where('category', Input::get('account_id'))->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id')->get();
	...
	public function outStandingBillsAdv($id,$mod=null) {
	...
	public function searchAccountAR()
	{
		//echo '<pre>';print_r(Input::all());exit;
		$data = $pdctransactions = array(); $opn_balnce = null;
		$frmdate = Input::get('date_from');
				...
				if((date('d-m-Y',strtotime($this->acsettings->from_date))!=Input::get('date_from'))) { 
					foreach($transactions as $key => $transaction ) {
						...
						if(Input::get('exclude_ob')==1) {
							...
							if($transaction[0]->voucher_type!='OB') {
								$obArr = $this->getOpeningBalanceAR($this->accountmaster->getPrintViewByAccount(Input::all()), $actrnsdata, $frmdate, $todate);
