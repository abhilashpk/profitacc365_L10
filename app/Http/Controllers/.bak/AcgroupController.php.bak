...
class AcgroupController extends Controller
{
	...
	{
		$this->acgroup->update($id, Input::all());//print_r(Input::all());exit;
	...
	public function checkname() {
		...
		$check = $this->acgroup->check_group_name(Input::get('name'), Input::get('category_id'), Input::get('id'));
	...
	public function checkcode() {
		...
		$check = $this->acgroup->check_group_code(Input::get('code'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
