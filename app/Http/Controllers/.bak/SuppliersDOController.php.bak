...
class SuppliersDOController extends Controller
{
	...
	public function save(Request $request, $id = null) {
		...
		if( $this->supplierdo->create(Input::all()) ) {
			...
			if($this->mod_autocost->is_active==1) {
				...
				app('App\Http\Controllers\UtilityController')->updateItemCurrentQtyByItems(Input::get('item_id'));
			}
	...
	public function checkRefNo() {
		...
		$check = $this->supplierdo->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	public function getSearch()
	{
		...
		$reports = $this->supplierdo->getReport(Input::all());
	...
	public function getSearchBkp()
	{
		...
		//$report = $this->supplierdo->getReport(Input::all());
	...
	public function dataExport()
	{
		...
		else if(Input::get('search_type')=='summary') {
	...
	public function update(Request $request, $id)
	{
		...
		if($this->supplierdo->update($id, Input::all())) {
			//AUTO COST REFRESH CHECK ENABLE OR NOT
			if($this->mod_autocost->is_active==1) {
				$this->objUtility->reEvalItemCostQuantity(Input::get('item_id'),$this->acsettings);
				app('App\Http\Controllers\UtilityController')->updateItemCurrentQtyByItems(Input::get('item_id'));
	...
	public function checkVchrNo() {
		...
		$check = $this->supplierdo->check_voucher_no(Input::get('voucher_no'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
