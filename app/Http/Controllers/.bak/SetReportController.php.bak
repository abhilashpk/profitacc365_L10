...
class SetReportController extends Controller
{
	...
	public function update() {
		...
		if(Input::get('id')!='') {
			if(Input::get('opt')==1)
				DB::table('report_view_detail')->where('id', Input::get('id'))->update(['is_default' => 0]);
			
			DB::table('report_view_detail')->where('id', Input::get('id'))
						->update([ 'name' => Input::get('name'),
								   'print_name' => Input::get('file'),
								   'is_default' => Input::get('opt')
								 ]);
		} else { 
			DB::table('report_view_detail')
						->insert([ 'report_view_id' => Input::get('rid'),
								   'name' => Input::get('name'),
								   'print_name' => Input::get('file'),
								   'is_default' => Input::get('opt')
								 ]);
