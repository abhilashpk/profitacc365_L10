...
class StockTransferoutController extends Controller
{
   ...
   protected $itemmaster;
	...
	public function save() {
		...
		if( $this->stock_transferout->create(Input::all()) ) {
	...
	public function checkRefNo() {
		...
		$check = $this->stock_transferout->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	public function update($id)
	{
		...
		if($this->stock_transferout->update($id, Input::all())) {
			//AUTO COST REFRESH CHECK ENABLE OR NOT
			if($this->mod_autocost->is_active==1) {
				$this->objUtility->reEvalItemCostQuantity(Input::get('item_id'),$this->acsettings);
			}
	...
	public function getSearch()
	{
		...
		$itemid = '';
		$voucher_head  = '';
    	//$report = $this->stock_transferout->getr(Input::all());
	...
	public function dataExport()
	{
		...
		$report = $this->sales_invoice->getReport(Input::all());
		...
		if(Input::get('search_type')=="summary")
		{
			...
			$report = $this->sales_invoice->getReportsales(Input::all());
	...
	//	
		...
		if(Input::get('search_type')=='sales_register') {
		...
		} else if(Input::get('search_type')=='itemwise') {
		...
		} else if(Input::get('search_type')=='customer_wise') {
