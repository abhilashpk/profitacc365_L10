...
class AccategoryController extends Controller
{
	...
	public function save() {
		...
		try {
			$this->accategory->create(Input::all());
	...
	{
		$this->accategory->update($id, Input::all());//print_r(Input::all());exit;
	...
	// 	$idarr = explode(',', $ids);
	// 	//$status = $this->accategory->check_categoryacc($idarr);
	// 	//echo '<pre>';print_r($status);exit;
	// 	$row = DB::table('account_master')->where('account_category_id',$idarr)->where('status', 1)->where('deleted_at', '0000-00-00 00:00:00')->count(); //
	...
	public function checkname() {
		...
		$check = $this->accategory->check_accategory_name(Input::get('name'), Input::get('id'));
