...
class JobEstimateController extends Controller
{
	...
	public function save(Request $request) { //echo '<pre>';print_r( Input::all() );exit;
		...
		//echo '<pre>';print_r($this->quotation_sales->create(Input::all()));exit;
	...
	public function getSearch()
	{
		...
		$reports = $this->quotation_sales->getPendingReportJob(Input::all());
		...
		return view('body.jobestimate.preprint')
					...
					->withCustomerid(Input::get('customer_id'))
					->withSettings($this->acsettings)
					->withData($data);
	}
	...
	public function getvehSearch()
	{
		$data = array();
		
		$vehicle_data = $this->jobmaster->getVehicleDetails(Input::all());
		//echo '<pre>';print_r(Input::all());exit;
		$itemmaster = $this->itemmaster->activeItemmasterList();
		$terms = $this->terms->activeTermsList();
		$jobs = $this->jobmaster->activeJobmasterList();
