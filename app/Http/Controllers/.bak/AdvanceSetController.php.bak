...
class AdvanceSetController extends Controller
{
	...
	public function save() {
		try { 
			if(Input::get('account_for') == 1)
				$this->receipt_voucher->advance_set(Input::all());
			else
				$this->payment_voucher->advance_set(Input::all());
	...
	public function update($id)
	{
		...
		$this->receipt_voucher->update($id, Input::all());
