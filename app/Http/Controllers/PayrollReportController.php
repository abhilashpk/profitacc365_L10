...
class PayrollReportController extends Controller
{
	...
	public function getSearch()
	{
		...
		$we_others = '';
		if($request->get('search_type')=='pay_slip') {
			$voucher_head = 'Pay Slip';
			$titles = ['main_head' => 'Pay Slip','subhead' => 'Pay Slip'];
			$employee = $this->employee->find($request->get('employee_id'));
			$results = $this->wageentry->getPayslip($request->all());  //echo '<pre>';print_r($employee);exit;
			
		} else if($request->get('search_type')=='pay_slip_summery') {
			$voucher_head = 'Pay Slip(Summery)'; $we_others = [];
			$titles = ['main_head' => 'Pay Slip(Summery)','subhead' => 'Pay Slip(Summery)'];
			$results = $this->employee->find($request->get('employee_id')); 
			$wages = $this->summary_wage( $this->wageentry->getPayslip_summery($request->all()) );//echo '<pre>';print_r($wages);exit;
			if($wages)
				$we_others = DB::table('wage_entry_others')->where('wage_entry_id',$wages['wage_entry_id'])->first();
			//echo '<pre>';print_r($wages);exit;
		} else if($request->get('search_type')=='jobwise_summery') {
			$voucher_head = 'Payroll Jobwise - Summery'; $wages = [];
			$titles = ['main_head' => 'Payroll Jobwise - Summery','subhead' => 'Payroll Jobwise - Summery'];
			$wages = $this->wageentry->get_jobwise_summery($request->all());//echo '<pre>';print_r($wages);exit;
			if($wages)
				$results = $this->calculate_job_wages($wages);
			
		} else if($request->get('search_type')=='payroll_summery') {
			$voucher_head = 'Payroll - Summery';
			$titles = ['main_head' => 'Payroll - Summery','subhead' => 'Payroll - Summery'];
			$results = $this->wageentry->payslip_summery($request->all());
			//echo '<pre>';print_r($results);exit;
		} else if($request->get('search_type')=='attendance') {
			$voucher_head = 'Employee Attendance';
			$titles = ['main_head' => 'Employee Attendance','subhead' => 'Employee Attendance'];
			$results = $this->group_emp( $this->wageentry->get_attendance($request->all()) );
			//echo '<pre>';print_r($results);exit;
		} else if($request->get('search_type')=='jobwise') {
			$voucher_head = 'Jobwise Detail';
			$titles = ['main_head' => 'Jobwise Detail','subhead' => 'Jobwise Detail'];
			$results = $this->wageentry->get_jobwise2($request->all());
			//echo '<pre>';print_r($results);exit;
		} 
	...
	public function getPrint()
	{
		$data = array();
		$voucher_head = 'Pay Slip';
		$results = $this->employee->find($request->get('employee_id'));  //echo '<pre>';print_r($results);exit;//getPayslip
