...
class ProfitAnalysisController extends Controller
{
	...
	private function profitCalc($results) {
		...
		foreach($results as $result) {
			$sprice = $cost = $profit = $percentage = $discount = $pprice = $tcst = $ptotal = $pertotal = $sqty = $sptotal = 0; $asm = 'false'; $i=1;
			...
			$percentage = ($ptotal > 0 && $sptotal > 0)?($ptotal / $sptotal * 100):0;
			
			//$profit = $sprice - $pprice - $discount;
			//if($pprice==0) { JAN25
				//$pprice = $row['sale_cost']; //JAN25
				//$profit = $sprice - $pprice - $discount;
			//}
			
			$aritm = explode(',',$row['assembly_items']);
	...
	private function profitCalc5($results) {
		
		$report = array();
		foreach($results as $result) {
			$sprice = $cost = $profit = $percentage = $discount = $pprice = 0; $asm = 'false';
			foreach($result as $row) {
				$sprice += $row['squantity'] * $row['sunit_price'];
				if($row['class_id']==1)
					$cost = $row['sale_cost'];
...
//$repor=  $this->profitCalc($this->makeTree($this->sales_invoice->getProfitSummary($request->all())));
//echo '<pre>';print_r($repor); exit();
			
			//$report = $this->sales_invoice->getProfitSummary($request->all()); echo '<pre>';print_r($report);exit;
			//$res=$this->makeTree( $this->sales_invoice->getProfitSummary($request->all()) ); echo '<pre>';print_r($res);exit;
	...
	public function dataExport(Request $request)
	{
		...
		if($request->get('search_type')=='summary') {
			...
			$tot_sprice = $tot_discount = $tot_cost = $tot_profit = $tot_price = 0;
			foreach($reports as $report) {
				...
				$ptype = ($report['is_assebly']==1)?'(Assembly Item Included)':'';
				
				$datareport[] = ['voucher_no' => $report['voucher_no'].' '.$ptype,
								 'voucher_date'	=> date('d-m-Y', strtotime($report['voucher_date'])),
								 'supplier'	=> $report['supplier'],
								  'salesman'	=> $report['salesman'],
								 'sprice'	=> number_format($report['sptotal'],3),
								 'discount'	=> number_format($report['discount'],3),
								 'price'	=> number_format($report['sptotal'], 3),
								 'cost'		=> number_format($report['cost'],3),
								 'profit'	=> number_format($report['profit'],3),
								 'percentage'	=> number_format($report['percentage'],3)
								];
				
			}
			
			$datareport[] = ['','','','Total',number_format($tot_price,3),number_format($tot_discount,3),number_format($tot_sprice,3),number_format($tot_cost,3),number_format($tot_profit,3)];
