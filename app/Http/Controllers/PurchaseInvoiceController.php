...
class PurchaseInvoiceController extends Controller
{
	...
	public function checkRefNo(Request $request) {
		...
		$check = $this->purchase_invoice->check_reference_no($request->get('reference_no'), $request->get('id'));
	...
	public function checkInvoice(Request $request) {
		...
		$check = $this->purchase_invoice->check_invoice_id( $request->get('purchase_invoice_id') );
		$isAvailable = ($check) ? false : true;
		echo $isAvailable;
	}
		...
		if( $this->purchase_invoice->update($id, $request->all()) ) {
			...
			if( isset($attributes['is_pv']) && $attributes['is_pv']==1 ) { 
				...
				if($sirv) {
					...
					$this->RVformSet($attributes, $id);
					$this->payment_voucher->update($request->get('rvedit_id'), $request->all());
				} else {
					$this->RVformSet($attributes, $id);
					$this->payment_voucher->create($request->all());
				}
	...
	public function checkVchrNo(Request $request) {
		...
		$check = $this->purchase_invoice->check_voucher_no($request->get('voucher_no'), $request->get('deptid'), $request->get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
