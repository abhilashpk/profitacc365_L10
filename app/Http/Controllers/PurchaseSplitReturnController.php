...
use App\Repositories\PurchaseSplitReturn\PurchaseSplitReturnInterface;
...
class PurchaseSplitReturnController extends Controller
{
	...
	public function __construct(PurchaseSplitReturnInterface $purchasesplit_return,PurchaseSplitInterface $purchase_split, SupplierDOInterface $supplierdo, PurchaseOrderInterface $purchase_order, ItemmasterInterface $itemmaster, TermsInterface $terms, JobmasterInterface $jobmaster, AccountMasterInterface $accountmaster, CurrencyInterface $currency, VoucherNoInterface $voucherno, AreaInterface $area,CountryInterface $country,AcgroupInterface $group, LocationInterface $location, AccountSettingInterface $accountsetting, FormsInterface $forms, MaterialRequisitionInterface $material_requisition) {
		...
		$this->group = $group;
		$this->forms = $forms;
		$this->formData = $this->forms->getFormData('PSR');
		...
		$this->mod_ps_pettycash = DB::table('parameter2')->where('keyname', 'mod_ps_pettycash')->where('status',1)->select('is_active')->first();
    ...
    public function index() {
		...
		return view('body.purchasesplitreturn.index')
					->withOrders($orders)
	...
	public function ajaxPaging(Request $request)
	{
		...
		$totalData = $this->purchasesplit_return->purchaseInvoiceListCount();
		...
		$invoices = $this->purchasesplit_return->purchaseInvoiceList('get', $start, $limit, $order, $dir, $search, $dept);
		
		if($search)
			$totalFiltered =  $this->purchasesplit_return->purchaseInvoiceList('count', $start, $limit, $order, $dir, $search, $dept);
		
		$prints = DB::table('report_view_detail')
							->join('report_view','report_view.id','=','report_view_detail.report_view_id')
							->where('report_view.code','PSR')
        ...
        if(!empty($invoices))
        {
            ...
            {
                $edit =  '"'.url('purchase_split_return/edit/'.$row->id).'"';
                $delete =  'funDelete("'.$row->id.'")';
				$print = url('purchase_split_return/print/'.$row->id);
				...
				$printfc = url('purchase_split_return/printfc/'.$row->id.'/'.$prints[0]->id);
				if($row->is_fc==1) {		
	...
	public function add(Request $request, $id = null, $doctype = null) {
		...
		$lastid = DB::table('purchasesplit_return')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->orderBy('id','DESC')->select('id')->first();
			$footertxt = DB::table('header_footer')->where('doc','PS')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->first();
		...
		$did = ($deptid=='')?0:$deptid;
		$vouchers = DB::table('account_setting')->where('voucher_type_id',30)->where('status',1)->where('department_id',$did)->where('deleted_at','0000-00-00 00:00:00')->get(); //'Purchase Stock' voucher from account settings...
		
		$account_name=DB::table('account_master')->where('id',$vouchers[0]->cr_account_master_id)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->first();
		if($account_name!=''){
	...
	//	echo '<Pre>';print_r($this->formData);exit;
		//echo '<pre>';print_r($account_name);exit;
		if($id!=null) {
		    $orderrow = $this->purchase_split->findPOdata($id);
		     $orditems = $this->purchase_split->getPSItems($id); 
		    $jobmaster = DB::table('jobmaster')
							...
							->get(); //echo '<Pre>';print_r($orderrow);exit;
             
			return view('body.purchasesplitreturn.addps')
		...
		$views=	($this->mod_ps_pettycash->is_active==1)?'add':'add';
		
			//echo '<pre>';print_r($views);exit;
		return view('body.purchasesplitreturn.'.$views)
					->withItems($itemmaster)
					->withUnits($units)
					->withTerms($terms)
					->withJobs($jobs)
					...
					->withAccountid($accountid)
					->withSettings($this->acsettings)
					->withModpettycash($this->mod_ps_pettycash->is_active)
					->withVatdata($this->vatdata)
					->withSettings($this->acsettings)
					->withLocdefault($locdefault)
					->withPurlocation($pur_location)
	...
	public function save(Request $request) {
		...
		if( $this->validate(
			...
			return redirect('purchase_split_return/add')->withInput()->withErrors();
		  }
		...
		if($this->purchasesplit_return->create($request->all())) {
			Session::flash('message', 'Purchase Split Return added successfully.');
		} else {
			Session::flash('error', 'Something went wrong, Invoice failed to add!');
		}
		return redirect('purchase_split_return/add');
	}
	
	public function destroy($id)
	{
			
		if($this->purchasesplit_return->delete($id)) {
			Session::flash('message', 'Purchase split Return deleted successfully.');
		} else
			Session::flash('error', 'Something went wrong, split failed to delete!');
		
		return redirect('purchase_split_return');
	}
	
	public function checkRefNo(Request $request) {

		$check = $this->purchasesplit_return->check_reference_no($request->get('reference_no'), $request->get('id'));
	...
	public function edit($id) { 
		...
		$jobs = $this->jobmaster->activeJobmasterList();
		$currency = $this->currency->activeCurrencyList();
		$orderrow = $this->purchasesplit_return->findPOdata($id);
		$orditems = $this->purchasesplit_return->getItems($id); 
	...
	{ 	 //echo '<pre>';print_r($request->input());exit;
		$id = $request->input('purchasesplit_return_id');
		if( $this->validate(
			$request, 
			[//'reference_no' => ($this->formData['reference_no']==1)?'required':'nullable',
			 'supplier_name' => 'required','supplier_id' => 'required',
			...
			return redirect('purchase_split_return/edit/'.$id)->withInput()->withErrors();
		}
		
		if( $this->purchasesplit_return->update($id, $request->all()) ) {
	...
	public function checkVchrNo(Request $request) {
		...
		$check = $this->purchasesplit_return->check_voucher_no($request->get('voucher_no'), $request->get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	public function getSearch(Request $request)
	{
		...
		$data = array();
		$supid = $itemid = '';
		$reports = $this->purchasesplit_return->customerWiseSummary($request->all());
		//echo '<pre>';print_r($reports); exit();
		if(Session::get('department')==1) {
			 		if($request->get('department_id')!='') {
			 			$rec = DB::table('department')->where('id', $request->get('department_id'))->select('name')->first();
		...
		if($request->get('search_type')=='summary') {
			$voucher_head = 'Purchase Split Return  by Summary';
			$reports =( $this->makeTreeSumm($this->purchasesplit_return->customerWiseSummary($request->all())));
			//echo '<pre>';print_r($reports); exit();
			
		}  else if($request->get('search_type')=='supplier') {
			$voucher_head = 'Purchase Split Return by supplierwise';
		    $reports =( $this->makeTreeSup($this->purchasesplit_return->customerWiseSummary($request->all())));
		...
		return view('body.purchasesplitreturn.preprint')
					->withReports($reports)
					->withVoucherhead($voucher_head)
					->withType($request->get('search_type'))
					->withFromdate($request->get('date_from'))
	...
	public function getCash($cum)
	{
		//echo '<pre>';print_r($cum);exit;
		$data = array();
		$accounts = $this->accountmaster->getCashList();
		return view('body.purchasesplit.cash')
					->withAccounts($accounts)
					->withCum($cum)
					->withData($data);
	}
	...
	public function dataExport(Request $request)
	{
		...
		{
			$reports = $this->purchasesplit_return->customerWiseSummary($request->all());
			$voucher_head = 'Purchase Split Return Summary';

			$datareport[] = ['','','','',strtoupper($voucher_head), '','',''];
			$datareport[] = ['','','','','','',''];
			
		
				$datareport[] = ['SI.No.','PSR#','Vchr.Date','PSR.Ref#', 'Supplier','TRN No','Gross Amt.','VAT Amt.','Net Total'];
				$i=0;
