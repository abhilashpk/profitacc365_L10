...
class SalesOrderBookingController extends Controller
{
	...
	//echo '<pre>';print_r($photos);print_r($jobdata);print_r($items);exit;
	...
	public function submit(Request $request) {
		...
		//DB::table('sales_order')->where('id',$request->get('id'))->update(['is_warning' => $request->get('status')]);
		...
		if($request->get('status')==1) {
			...
			$sorow = DB::table('sales_order_gi')
							...
							->first(); //print_r($sorow);exit;
			if($sorow) {
				if($sorow->voucher_date!=date('Y-m-d')) { //CHECKING VOUCHER DATE IS SAME, DO EDIT ELSE ADD.
					$edit = false;
					$orditmid[]=0;
				} else {
					$edit = true;
			...
			DB::table('sales_order')->where('id',$request->get('id'))->update(['discount' => $request->get('status')]);
