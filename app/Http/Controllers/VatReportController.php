...
class VatReportController extends Controller
{
	...
	private function getVatSummary($reports) {
		$payable = $amount_inc = $amount = 0;
		foreach($reports as $key => $report) {
			$grossAmt = $discount = $taxable = $vatAmt = $netAmt = $dramount = $cramount = 0;
			$grossAmt_inc = $discount_inc = $taxable_inc = $vatAmt_inc = $netAmt_inc = $dramount_inc = $cramount_inc = 0;
			$isvat_inc = false;$name_inc=$sub_inc='';
			foreach($report as $row) {
				if($row->tax_include==0) {
					$name = $row->vat_name;
					$sub = '(Excluding)';
					$grossAmt += ($row->trtype=='Dr')?$row->gross_total:($row->gross_total*-1);
					$discount += $row->discount;
					$taxable += ($row->trtype=='Dr')?($row->gross_total - $row->discount):(($row->gross_total - $row->discount)*-1);
					$vatAmt += ($row->trtype=='Dr')?$row->vat_amount:($row->vat_amount*-1);//$row->vat_amount;
					$netAmt += ($row->trtype=='Dr')?$row->net_total:($row->net_total*-1);
					...
					if($row->trtype=='Dr')
				        ...
				        $isvat_inc = true;
				    	$name_inc = $row->vat_name;
				    	$sub_inc = '(Including)';
    					$grossAmt_inc += $row->gross_total;
    					$discount_inc += $row->discount;
    					$taxable_inc += $row->subtotal;
    					$vatAmt_inc += ($row->trtype=='Dr')?$row->vat_amount:($row->vat_amount*-1);//$row->vat_amount;
    					$netAmt_inc += $row->net_total;
    					
    					if($row->trtype=='Dr')
    						$dramount_inc += $row->vat_amount;
    					else
    						$cramount_inc += $row->vat_amount;
				}
			}
			$amount = $dramount - $cramount; $amount += $dramount_inc - $cramount_inc; $payable += $amount;
			$vatArr['vat'][] = (object)['name' => $name, 'gross_amt' => $grossAmt, 'discount' => $discount, 'taxable' => $taxable, 'vat_amt' => $vatAmt, 'net_amt' => $netAmt,'end' => false, 'sub' => $sub];
			//if($isvat_inc)
			    $vatArr['vat'][] = (object)['name' => $name_inc, 'gross_amt' => $grossAmt_inc, 'discount' => $discount_inc, 'taxable' => $taxable_inc, 'vat_amt' => $vatAmt_inc, 'net_amt' => $netAmt_inc,'end' => true, 'sub' => $sub_inc ];
		}
		$vatArr['payable'] = $payable;
		return $vatArr;
	}
	
	private function getVatSummaryBKP($reports) {
		$payable = 0;
		foreach($reports as $key => $report) {
			$grossAmt = $discount = $taxable = $vatAmt = $netAmt = $dramount = $cramount = 0;
			foreach($report as $row) {
	...
	public function getSearch(Request $request)
	{
		...
		$request->merge(['search_type' => 'detail']);
			//$result = $this->accountmaster->getVatDetail($request->all()); echo '<pre>';print_r($result);exit;
			$result = $this->getVatSummary( $this->GroupByAccount($this->accountmaster->getVatDetail($request->all())) ); //echo '<pre>';print_r($result);exit;
			$reports = isset($result['vat'])?$result['vat']:'';
	...
	//	echo '<pre>';print_r($reports);exit;
		return view('body.vatreport.'.$view)
					...
					->withCodetype($code_type)
				     ->withDepartment($request->get('department_id'))
					->withType($type)
					->withPayable($payable)
