...
class StockLedgerController extends Controller
{
	...
	public function index(Request $request) //search
	{
		...
		} else if($request->get('search_type')=='quantity_cost') { */
	...
	public function getPrint(Request $request)
	{
		...
		if($request->get('search_type')=='quantity') {
			$voucher_head = 'Stock Ledger with Quantity';
			$results = $this->itemmaster->getStockLedgerReport($request->all()); 
			...
			$itemdata = DB::table('itemmaster')->where('id',$request->get('document_id'))->select('item_code','description')->first();
			$locdata = [];
		} else if($request->get('search_type')=='quantity_loc') {
		...
		} else if($request->get('search_type')=='quantity_loc_cost') {
		...
		return view('body.stockledger.print')
					...
					->withJobno($request->get('is_jobno'))
	...
	public function dataExport(Request $request)
	{
	    ...
	    $request->merge(['is_jobno' =>(isset($data->is_jobno))?$data->is_jobno:'']);
	   
	   $jobno=$request->get('is_jobno');
	   //echo '<pre>';print_r($jobno);exit;
		...
		} else if($request->get('search_type')=='quantity_loc'||$request->get('search_type')=='quantity_conloc') {
		    if($request->get('search_type')=='quantity_loc'){
		    $voucher_head = 'Stock Ledger with Location';
		    }
		    else if($request->get('search_type')=='quantity_conloc'){
		     $voucher_head = 'Stock Ledger with Consignment Location';   
		...
		}else if($request->get('search_type')=='quantity_loc_cost'||$request->get('search_type')=='quantity_conloc_cost') {
		    if($request->get('search_type')=='quantity_loc_cost'){
