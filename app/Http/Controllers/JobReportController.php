	...
	$vehicle='';
		if($request->get('type') == 'workshop') {
			...
			if($request->get('search_type')=='summary') {
				...
				//$reports = $this->jobmaster->getJobReport($request->all()); echo '<pre>';print_r($reports);exit;
...
//	echo '<pre>';print_r($reports);exit;		
	...
	public function getPrint()
	{
		...
		if($request->get('search_type')=='opening_quantity' || $request->get('search_type')=='qtyhand_ason_date') {
			$voucher_head = 'Job Report';
			$results = $this->jobmaster->getQuantityReport($request->all()); 
