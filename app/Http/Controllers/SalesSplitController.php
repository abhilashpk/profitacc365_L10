...
class SalesSplitController extends Controller
{
	...
	public function __construct(SalesSplitInterface $sales_split, SupplierDOInterface $supplierdo, PurchaseOrderInterface $purchase_order, ItemmasterInterface $itemmaster, TermsInterface $terms, JobmasterInterface $jobmaster, AccountMasterInterface $accountmaster, CurrencyInterface $currency, VoucherNoInterface $voucherno, AreaInterface $area,CountryInterface $country,AcgroupInterface $group, LocationInterface $location, AccountSettingInterface $accountsetting, FormsInterface $forms, MaterialRequisitionInterface $material_requisition) {
		...
		$this->group = $group;
		$this->forms = $forms;
		$this->formData = $this->forms->getFormData('SS');
    ...
    public function index() {
		...
		return view('body.salessplit.index')
	...
	public function ajaxPaging(Request $request)
	{
		...
		$totalData = $this->sales_split->purchaseInvoiceListCount();
		...
		$invoices = $this->sales_split->purchaseInvoiceList('get', $start, $limit, $order, $dir, $search, $dept);
		
		if($search)
			$totalFiltered =  $this->sales_split->purchaseInvoiceList('count', $start, $limit, $order, $dir, $search, $dept);
		
		$prints = DB::table('report_view_detail')
							->join('report_view','report_view.id','=','report_view_detail.report_view_id')
							->where('report_view.code','SS')
        ...
        if(!empty($invoices))
        {
            ...
            {
                $edit =  '"'.url('sales_split/edit/'.$row->id).'"';
	...
	public function add(Request $request, $id = null, $doctype = null) {
		...
		$lastid = DB::table('sales_split')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->orderBy('id','DESC')->select('id')->first();
		...
		$did = ($deptid=='')?0:$deptid;
		$vouchers = DB::table('account_setting')->where('voucher_type_id',24)->where('status',1)->where('department_id',$did)->where('deleted_at','0000-00-00 00:00:00')->get(); //'Purchase Stock' voucher from account settings...
		...
		if($id!=null){
			return view('body.salessplit.addjm')
					->withUnits($units)
					->withTerms($terms)
		...
		$views=	($this->mod_sp_pettycash->is_active==1)?'addsp':'add';				
		return view('body.salessplit.'.$views)
	...
	//	echo '<pre>';print_r($request->input());exit;
		...
		if( $this->validate(
				...
				{
			return redirect('sales_split/add')->withInput()->withErrors();
		  }
		
		if($this->sales_split->create($request->all())) {
			Session::flash('message', 'Sales Split added successfully.');
		} else {
			Session::flash('error', 'Something went wrong, Invoice failed to add!');
		}
		return redirect('sales_split/add');
	}
	
	public function destroy($id)
	{
			
		if($this->sales_split->delete($id)) {
			Session::flash('message', 'Sales split deleted successfully.');
		} else
			Session::flash('error', 'Something went wrong, split failed to delete!');
		
		return redirect('sales_split');
	}
	
	public function checkRefNo(Request $request) {

		$check = $this->sales_split->check_reference_no($request->get('reference_no'), $request->get('id'));
	...
	public function checkInvoice(Request $request) {
		...
		$check = $this->sales_split->check_invoice_id( $request->get('sales_split_id') );
		$isAvailable = ($check) ? false : true;
		echo $isAvailable;
	}
	...
	public function edit($id) { 
		...
		$orditems = $this->sales_split->getItems($id); 
		...
		$did = ($deptid=='')?0:$deptid;
		$vouchers = DB::table('account_setting')->where('voucher_type_id',24)->where('status',1)->where('department_id',$did)->where('deleted_at','0000-00-00 00:00:00')->get(); //'Purchase Stock' voucher from account settings...
		$views=	($orderrow->is_pettycash==1)?'editsp':'edit';
		return view('body.salessplit.'.$views)
					->withItems($itemmaster)
	...
	{ 	 //echo '<pre>';print_r($request->input());exit;
		$id = $request->input('sales_split_id');
		if( $this->validate(
			$request, 
			...
			return redirect('sales_split/edit/'.$id)->withInput()->withErrors();
		}
		
		if( $this->sales_split->update($id, $request->all()) ) {
	...
	public function getSplit($did=null)
	{
		$data = array();
		$split = [];
		return view('body.salessplit.getsplit')
					->withInvoices($split)
					->withDid($did)
					->withData($data);
	}
		public function ajaxPagingSplitData(Request $request)
	{
		$columns = array( 
                            0 =>'sales_split.id', 
                            1 =>'voucher_no',
		...
		$dept = $request->input('dept');		
		$totalData = $this->sales_split->salesSplitListCount($dept);
		...
		$invoices = $this->sales_split->salesSplitList('get', $start, $limit, $order, $dir, $search, $dept);
		
		if($search)
			$totalFiltered =  $this->sales_split->salesSplitList('count', $start, $limit, $order, $dir, $search, $dept);
	...
	public function checkVchrNo(Request $request) {
		...
		$check = $this->sales_split->check_voucher_no($request->get('voucher_no'), $request->get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	//	$reports = $this->sales_split->customerWiseSummary($request->all());
		...
		if($request->get('search_type')=='summary') {
			$voucher_head = 'Sales Split  by Summary';
		    ...
		    $reports =( $this->makeTreeCus($this->sales_split->customerWiseSummary($request->all())));
		...
		return view('body.salessplit.preprint')
					->withReports($reports)
	...
	public function dataExport(Request $request)
	{
		...
		if($request->get('search_type')=="summary")
		{
			...
			$reports = $this->sales_split->getReport($request->all());
