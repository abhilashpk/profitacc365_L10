...
class ItemTemplateController extends Controller
{
	...
	public function getTemplateEdit($id,$jid,$no) {
		...
		foreach($template as $k => $trow) {
			if($trow->input_type=="item") {
				$items[$k] = DB::table('itemmaster')->where('group_id',$trow->group_id)->where('status',1)->whereNull('deleted_at')->select('id','item_code AS text')->get();
			}
		}
	...
	public function getItemsbkp($gid) { //print_r($request->all());
	...
	public function getItems($gid) { //print_r($request->all());
		if($request->get('term')!='')
		   $items = DB::table('itemmaster')->where('status',1)->whereNull('deleted_at')->where('item_code','like','%'.$request->get('term').'%')->select('id','item_code AS text')->get();
	   else
		   $items = DB::table('itemmaster')->where('group_id',$gid)->where('status',1)->whereNull('deleted_at')->select('id','item_code AS text')->get();
