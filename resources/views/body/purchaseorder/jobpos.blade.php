                    ...
                    <div class="panel panel-success filterable">
                        <div class="panel-heading">
                            <h3 class="panel-title">
                                <i class="fa fa-fw fa-columns"></i> Purchase Orders
                            </h3>
                        </div>
                        <div class="panel-body">
                            ...
                            <div class="table-responsive m-t-10">
                                <table class="table horizontal_table table-striped" id="tablePorders">
                                    <thead>
                                    <tr>
										<th><input type="hidden" name="url" id="url" value=""></th>
										<th>PO. No</th>
										<th>PO. Date</th>
										<th>Supplier</th>
										<th>Amount</th>
                                    ...
                                    <tbody>
										@foreach($jobpos as $row)
										<tr>
											<td><input type="checkbox" name="quotaion[]" value="{{$row->id}}"/></td>
											<td>{{ $row->voucher_no }}</td>
											<td>{{ date('d-m-Y',strtotime($row->voucher_date)) }}</td>
											<td>{{ $row->description }}</td>
											<td>{{ $row->net_total }}</td>
											<td><a href="#" class="quot-data" data-id="{{$row->id}}" data-name="{{$row->voucher_no}}" data-toggle="modal" data-target="#quot_modal">View Items</a></td>
...
function setTransfer() {
	...
	var url = "{{ url('sales_order/poadd/') }}/"+items;
