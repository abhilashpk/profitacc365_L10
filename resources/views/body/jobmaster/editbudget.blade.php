                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                               <i class="fa fa-fw fa-crosshairs"></i> Edit Project Budgeting 
                            </h3>
                        ...
                        <div class="panel-body">
							<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmPurorder" id="frmPurorder" action="{{ url('jobmaster/updatebudget/'.$orderrow->id) }}">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
								<input type="hidden" name="budget_id" id="budget_id" value="{{$orderrow->id }}">
								...
								<input type="hidden" id="rowNuminc" value="{{$numinc}}">
								<input type="hidden" id="remiteminc" name="remove_iteminc">
								
								<div class="itemdivPrntinc">
								
								@foreach($orditemsinc as $item)
								{{--*/ $i++; /*--}}
											...
											<table border="0" class="table-dy-row">
												...
												<tr>
													<td width="16%">
														...
														<input type="hidden" name="order_item_idinc[]" id="p_orditmidinc_{{$i}}" value="{{$item->id}}">
														<input type="hidden" name="account_idinc[]" id="acntidinc_{{$i}}" value="{{$item->account_id}}">
														<input type="text" id="accodinc_{{$i}}" name="ac_codeinc[]" class="form-control" autocomplete="off" readonly value="{{$item->master_name}}" data-toggle="modal" data-target="#acinc_modal" placeholder="A/C Name">
													</td>
													<td width="29%">
														<input type="text" name="item_descriptioninc[]" id="acdesinc_{{$i}}" autocomplete="off" class="form-control" readonly value="{{$item->item_description}}">
													</td>
												
													<td width="8%">
														<input type="number" id="itmcstinc_{{$i}}" autocomplete="off" step="any" name="costinc[]" class="form-control  readonly <?php if($errors->has('cost.'.$i)) echo 'form-error';?> line-costinc" value="{{($item->unit_price)?$item->unit_price:$item->unit_price}}">
													</td>
													
												
													<td width="11%">
														<input type="hidden" id="itmttlinc_{{$i}}" step="any" name="line_totalinc[]" class="form-control line-totalinc"  readonly value="{{($item->item_total)?$item->item_total:$item->item_total}}">
														<input type="hidden" id="itmtotinc_{{$i}}" name="item_totalinc[]" value="{{$item->item_total}}">
													</td>
													<td width="1%">
													@if($numinc==$i)
														<button type="button" class="btn-success btn-addinc-item" >
														 ...
														 <button type="button" class="btn-danger btn-removeinc-item" data-id="reminc_{{$i}}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-2">
										<span class="small" id="fc_label"></span><input type="text" id="totalinc" step="any" name="totalinc" class="form-control spl" readonly value="{{(old('totalinc'))?old('totalinc'):(($orderrow->total_cost)?$orderrow->total_cost:$orderrow->total_cost)}}">
										</div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-2">
											<input type="hidden" step="any" id="discount" name="discount" autocomplete="off" class="form-control spl discount-cal" >
										</div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-2">
											<input type="hidden" step="any" id="subtotal" name="subtotal" class="form-control spl" readonly >
										</div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-2">
											<input type="hidden" id="vatcur" name="vatcur">
											<input type="hidden" step="any" id="vat" name="vat" class="form-control spl"  readonly>
										</div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-2">
											<input type="hidden" step="any" id="net_amount" name="net_amount" class="form-control spl" readonly >
										</div>
											...
											<table border="0" class="table-dy-row">
												...
												<tr>
													<td width="16%">
														...
														<input type="text" id="accod_{{$i}}" name="ac_code[]" class="form-control" autocomplete="off" readonly value="{{$item->master_name}}" data-toggle="modal" data-target="#ac_modal" placeholder="A/C Name">
													</td>
													<td width="29%">
														<input type="text" name="item_description[]" id="acdes_{{$i}}" autocomplete="off" class="form-control" readonly value="{{$item->item_description}}">
													</td>
												
													<td width="8%">
														<input type="number" id="itmcst_{{$i}}" autocomplete="off" step="any" name="cost[]" class="form-control  readonly <?php if($errors->has('cost.'.$i)) echo 'form-error';?> line-cost" value="{{($item->unit_price)?$item->unit_price:$item->unit_price}}">
													</td>
													
												
													<td width="11%">
														<input type="hidden" id="itmttl_{{$i}}" step="any" name="line_total[]" class="form-control line-total"  readonly value="{{($item->item_total)?$item->item_total:$item->item_total}}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-2">
										<span class="small" id="fc_label"></span><input type="text" id="total" step="any" name="total" class="form-control spl" readonly value="{{(old('total'))?old('total'):(($orderrow->total_income)?$orderrow->total_income:$orderrow->total_income)}}">
										</div>
							...
							<div id="ac_modal" class="modal fade animated" role="dialog">
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body" id="acinc_data">
										</div>
										<div class="modal-footer">
											<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
										</div>
...
"use strict";
var lprice;
var taxinclude = false; var srvat=0;//{{$vatdata->percentage}};
 $('#vatdiv_1').val( srvat+'%' );
 $('#vat_1').val( srvat );
 var dptTxt;
$(document).ready(function () { 
	...
	if ( $('.itemdivPrnt').children().length == 1 ) {
		...
		$('.itemdivPrntinc').find('.btn-addinc-item:not(:last)').hide();
...
//calculation item net total, tax and discount...
	function getNetTotal() {
		...
		var lineTotalinc = 0;
		$( '.line-totalinc' ).each(function() { 
		  var res = this.id.split('_');
		  var n = res[1];
		  var amt = getLineTotalinc(n);
		  lineTotalinc = lineTotalinc + parseFloat(amt.toFixed(2));
		...
		var discountinc = parseFloat( ($('#discount').val()=='') ? 0 : $('#discount').val() );
		var vatinc      = parseFloat( ($('#vat').val()=='') ? 0 : $('#vat').val() );
	...
	{ 
		...
		var vatTotalinc = parseFloat( ($('#vatinc').val()=='') ? 0 : $('#vatinc').val() );
			...
			$( '.line-totalinc' ).each(function() {
			  var res = this.id.split('_');
			  var curNum = res[1];
			  var vatinc = parseFloat( ($('#vatinc_'+curNum).val()=='') ? 0 :  $('#vatinc_'+curNum).val() );
			  
			  if($('#txincldinc_'+curNum+' option:selected').val()==1 ) {
				  var vatPlusinc = parseFloat(100 + vatinc);
				  totalinc = parseFloat( $('#itmqtyinc_'+curNum).val() ) * parseFloat( $('#itmcstinc_'+curNum).val() );
			  } else {
				 var vatPlusinc = 100;
				 totalinc = this.value;
			  }
			  ...
			  $('#vatdivinc_'+curNum).val(vatinc+'% - '+vatLineinc);//.toFixed(2)
			  $('#vatlineamtinc_'+curNum).val(vatLineinc);//.toFixed(2)
			  $('#itmdsntinc_'+curNum).val(discountAmtinc.toFixed(2));
...
$(function() {	
    ...
    { 
        ...
        rowNuminc++; 
		e.preventDefault();
        var controlForm = $('.controls .itemdivPrntinc'),
            currentEntry = $(this).parents('.itemdivChldinc:first'),
            newEntry = $(currentEntry.clone()).appendTo(controlForm);
	
			newEntry.find($('input[name="account_idinc[]"]')).attr('id', 'acntidinc_' + rowNuminc);
			newEntry.find($('input[name="ac_codeinc[]"]')).attr('id', 'accodinc_' + rowNuminc);
			newEntry.find($('input[name="item_descriptioninc[]"]')).attr('id', 'acdesinc_' + rowNuminc);

			newEntry.find($('input[name="costinc[]"]')).attr('id', 'itmcstinc_' + rowNuminc);
			newEntry.find($('input[name="line_vatinc[]"]')).attr('id', 'vatinc_' + rowNuminc);
			newEntry.find($('input[name="line_discountinc[]"]')).attr('id', 'itmdsntinc_' + rowNuminc);
			newEntry.find($('input[name="line_totalinc[]"]')).attr('id', 'itmttlinc_' + rowNuminc);
			newEntry.find($('input[name="vatline_amtinc[]"]')).attr('id', 'vatlineamtinc_' + rowNuminc);
			newEntry.find($('input[name="vatdivinc[]"]')).attr('id', 'vatdivinc_' + rowNuminc);
			newEntry.find($('.item-datainc')).attr('id', 'itemDatainc_' + rowNuminc);
			newEntry.find($('button[type="button"]')).attr('data-id', 'reminc_' + rowNuminc); 
			newEntry.find('input').val(''); 
			newEntry.find($('.tax-code')).attr('id', 'taxcodeinc_' + rowNum);
			newEntry.find($('input[name="item_totalinc[]"]')).attr('id', 'itmtotinc_' + rowNuminc);
			newEntry.find($('.hidunt')).attr('id', 'hidunit_' + rowNuminc);
			newEntry.find($('input[name="jobidinc[]"]')).attr('id', 'jobidinc_' + rowNuminc);
			newEntry.find($('input[name="jobcodinc[]"]')).attr('id', 'jobcodinc_' + rowNuminc);
			
			$('#jobcodinc_' + rowNuminc).val( $('#job_idinc option:selected').text() );
			$('#jobidinc_' + rowNuminc).val( $('#job_idinc option:selected').val() );
			
			$('#vatdivinc_'+rowNuminc).val( srvat+'%' );
			$('#vatinc_'+rowNuminc).val( srvat );
					
			controlForm.find('.btn-addinc-item:not(:last)').hide();
			controlForm.find('.btn-removeinc-item').show();
    ...
    { 
		...
		var res = $(this).attr('data-id').split('_');
		var curNum = res[1]; var ids;
		var remiteminc = $('#remiteminc').val();
		ids = (remiteminc=='')?$('#p_orditmidinc_'+curNum).val():remiteminc+','+$('#p_orditmidinc_'+curNum).val();
		$('#remiteminc').val(ids);
	...
	var acurlinc = "{{ url('account_master/get_accountbudginc/') }}";

//	var acurl = "{{ url('account_master/get_account_all/') }}";
	 $(document).on('click', 'input[name="ac_codeinc[]"]', function(e) {
	 	var res = this.id.split('_');
	 	var curNum = res[1]; 
	 	console.log(curNum);
	 	$('#acinc_data').load(acurlinc+'/'+curNum, function(result){ //.modal-body item
			//console.log(result);
	...
	$(document).on('blur', '.line-cost', function(e) {
		...
		if(res) {
			...
			$(document).on('blur', '.line-costinc', function(e) {
		/* if( parseFloat(lprice) < parseFloat(this.value) ) {
			alert('This price is greater than your last purchase.');
		} */
		var res = this.id.split('_');
