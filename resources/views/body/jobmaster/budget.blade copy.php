                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmPurchaseSplit" id="frmPurchaseSplit" action="{{ url('jobmaster/budgsave') }}">
								...
								<br/>
								<fieldset>
								<legend style="margin-bottom:0px !important;"><h5><span class="itmDtls">Indirect and Direct Expense Details</span></h5></legend>
								...
								<fieldset>
								<legend style="margin-bottom:0px !important;"><h5><span class="itmDtls">Indirect and Direct Income Details</span></h5></legend>
									<table class="table table-bordered" style="margin-bottom:0px !important;">
									<thead>
									<tr>
										<th width="10%" class="itmHd">
									...
									<div class="itemdivPrntinc">
										<div class="itemdivChldinc">	
											<table border="0" class="table-dy-row">
												...
												<td width="18%" >
														<input type="hidden" name="account_id[]" id="acntid_1">
														<input type="text" id="accod_1" name="acinc_code[]" class="form-control" autocomplete="off" data-toggle="modal" data-target="#acinc_modal" placeholder="A/C Name">
													</td>
													<td width="29%">
														<input type="text" name="item_description[]" id="acdes_1" autocomplete="off" class="form-control" placeholder="Description">
													</td>
													...
													<td width="25%">
														<input type="number" id="itmcst_1" autocomplete="off" step="any" name="cost[]" class="form-control line-costo" placeholder="Amount">
													</td>
												
												
													<td width="1%">
														<input type="hidden" id="itmttl_1" step="any" name="line_total[]" class="form-control line-totalo" readonly placeholder="Total">
														<input type="hidden" id="itmtot_1" name="item_total[]">
													</td>
													<td width="1%">
														<button type="button" class="btn-success btn-addo-item" >
																<i class="fa fa-fw fa-plus-square"></i>
															 </button>
															 <button type="button" class="btn-danger btn-removeo-item" >
																<i class="fa fa-fw fa-minus-square"></i>
															 </button>
													</td>
												</tr>
...
$(document).ready(function () {
 ...
 var taxinclude = false; var srvat=0;//<?php echo ($vatdata)?$vatdata->percentage:'0';?>;
 $('#vatdiv_1').val( srvat+'%' );
 $('#vat_1').val( srvat );
 var dptTxt;
$(document).ready(function () { 
	...
	function getNetTotalo() {
		
		var lineTotal = 0;
		$( '.line-totalo' ).each(function() { 
		  var res = this.id.split('_');
		  var n = res[1];
		  var amt = getLineTotalo(n);
		  lineTotal = lineTotal + parseFloat(amt.toFixed(2));
	...
	$(document).on('click', '.btn-addo-item', function(e)  { 
        rowNumo++; 
		e.preventDefault(); 
        var controlForm = $('.controls .itemdivPrntinc'),
            currentEntry = $(this).parents('.itemdivChldinc:first'),
            newEntry = $(currentEntry.clone()).appendTo(controlForm);
			newEntry.find($('.line-unit')).attr('id', 'itmunt_' + rowNumo); 
			newEntry.find($('input[name="account_id[]"]')).attr('id', 'acntid_' + rowNumo);
			newEntry.find($('input[name="acinc_code[]"]')).attr('id', 'accod_' + rowNumo);
			newEntry.find($('input[name="item_descriptiono[]"]')).attr('id', 'acdeso_' + rowNumo);
			//newEntry.find($('input[name="unit[]"]')).attr('id', 'itmunt_' + rowNum);
			newEntry.find($('input[name="quantity[]"]')).attr('id', 'itmqty_' + rowNumo);
			newEntry.find($('input[name="cost[]"]')).attr('id', 'itmcst_' + rowNumo);
			newEntry.find($('input[name="line_vat[]"]')).attr('id', 'vat_' + rowNumo);
			newEntry.find($('input[name="line_discount[]"]')).attr('id', 'itmdsnt_' + rowNumo);
			newEntry.find($('input[name="line_total[]"]')).attr('id', 'itmttl_' + rowNumo);
			newEntry.find($('input[name="vatline_amt[]"]')).attr('id', 'vatlineamt_' + rowNumo);
			newEntry.find($('input[name="vatdiv[]"]')).attr('id', 'vatdiv_' + rowNumo);
			newEntry.find($('.item-data')).attr('id', 'itemData_' + rowNumo);
			newEntry.find($('button[type="button"]')).attr('data-id', 'rem_' + rowNumo); 
			newEntry.find('input').val(''); 
			newEntry.find($('.tax-code')).attr('id', 'taxcode_' + rowNumo);
			newEntry.find($('input[name="item_total[]"]')).attr('id', 'itmtot_' + rowNumo);
			newEntry.find($('.hidunt')).attr('id', 'hidunit_' + rowNumo);
			newEntry.find($('input[name="jobid[]"]')).attr('id', 'jobid_' + rowNumo);
			newEntry.find($('input[name="jobcod[]"]')).attr('id', 'jobcod_' + rowNumo);
			
			$('#vatdiv_'+rowNumo).val( srvat+'%' );
			$('#vat_'+rowNumo).val( srvat );
					
			controlForm.find('.btn-addo-item:not(:last)').hide();
			controlForm.find('.btn-removeo-item').show();
			$('#itmcod_'+rowNumo).focus();
			
    }).on('click', '.btn-removeo-item', function(e)
    { 
		//new change..
		...
		//ROWCHNG
		$('.itemdivPrntinc').find('.itemdivChldinc:last').find('.btn-addo-item').show();
		if ( $('.itemdivPrntinc').children().length == 1 ) {
			$('.itemdivPrntinc').find('.btn-removeo-item').hide();
		}
	...
	//new change............
	 ...
	 $(document).on('click', 'input[name="acinc_code[]"]', function(e) {
	 	var res = this.id.split('_');
	 	var curNum = res[1]; 
	 	console.log(curNum);
	 	$('#acinc_data').load(acurl+'/'+curNum, function(result){ //.modal-body item
			//console.log(result);
	...
	$(document).on('blur', '.line-costo', function(e) { 
		
		var res = this.id.split('_');
		var curNum = res[1]; 
		var res = getLineTotalo(curNum);
		if(res) {
			getNetTotalo();
		}
	});
