        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
					...
					<?php if(count($reports) > 0) { 
						if($type=='summary') { ?>
							<table border="0" style="width:100%;height:100%;">
								...
								<td width="20%"><b>PR.No</b></td>
											<td width="8%"><b>PR.Date</b></td>
							...
							<tbody id="bod">
								...
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
									    	<tr>
												<td><b>SI.No:</b> {{ ++$i }}</td>
												<td><b>PR#:</b> {{$report[0]->voucher_no}}</td>
												<td><b>PR Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
												<td><b>PR Ref.#:</b> {{$report[0]->reference_no}}</td>
										...
										<tr>
												...
												<td class="text-right"><b>PR.Qty.</b></td>
										...
										<?php 
											$sptotal = $dtotal = $ctotal = $ptotal = $pertotal = $peravg = $sqty = $tcst =$tprice= 0; $i=1;
											foreach($report as $row) { 
												$sprice = $row['unit_price'] ;
												$tprice=($row->quantity*$row['unit_price'])-($row->discount);
												if($row['class_id']==1)
													$cost = $row['sale_cost'];
												else
													$cost = 0;
												$pprice = $row['unit_vat'] ;
												$profit =  $row['unit_vat'] +$tprice;//$row['net_amount']
					...
					<form class="form-horizontal" role="form" method="POST" name="frmExport" id="frmExport" action="{{ url('purchase_return/export') }}">
