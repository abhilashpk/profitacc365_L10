            ...
            <div class="row">
                <div class="col-md-12">
					...
					<div class="alert alert-warning">
						<p>
							SS. No. is found empty! Please create in Account Settings.
						</p>
					</div>
					<?php } else { ?>
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                                <i class="fa fa-fw fa-crosshairs"></i> New Sales Split
                            </h3>
							
							<div class="pull-right">
							<?php if($printid) { ?>
								@can('pi-print')
								 <a href="{{ url('sales_split/print/'.$printid->id.'/'.$print->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
							<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmSalesSplit" id="frmSalesSplit" enctype="multipart/form-data" action="{{ url('sales_split/save') }}">
								...
								<div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label"><b>Sales Split</b></label>
                                    <div class="col-sm-10">
								...
								@foreach($customer as $cust)
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="customer_name" id="customer_name" value="<?php echo $cust->master_name ?>" class="form-control <?php if($errors->has('customer_name')) echo 'form-error';?>" autocomplete="off" data-toggle="modal" data-target="#customer_modal" placeholder="Customer">
										<input type="hidden" name="customer_id" id="customer_id" value="<?php echo (old('customer_id'))?old('customer_id'):$cust->id; ?>">
										<input type="hidden" name="old_customer_id" id="old_customer_id" value="{{$cust->id}}">
										<div class="col-xs-10" id="newsupplierInfo">
											<div class="col-xs-8">
												<span class="small">Customer Name</span> <input type="text" id="suppliername" name="suppliername" value="{{$cust->master_name}}" class="form-control" autocomplete="off" data-toggle="modal" data-target="#newsupplier_modal">
											</div>
										</div>
									</div>
								@endforeach
								...
								@foreach($customer as $cust)
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="vat_no" autocomplete="off" name="vat_no" value="<?php echo $cust->vat_no ?>" placeholder="TRN No.">
                                    </div>
                                 @endforeach
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select id="job_id" class="form-control select2" style="width:100%" name="job_id">
											@foreach($jobmaster as $job)
											<option value="{{ $job->id}}">{{$job->transport_type.$job->code}}</option>
											@endforeach
                                        </select>
                                    </div>
                                </div>
										...
										<div class="itemdivChld">	
											<table border="0" class="table-dy-row">
												<tr>
													...
													<td width="8%">
														<input type="hidden" name="jobid[]" id="jobid_1" value="{{$jobmaster[0]->id}}">
														<input type="text" id="jobcod_1" autocomplete="off" name="jobcod[]" value="{{$jobmaster[0]->transport_type.$jobmaster[0]->code}}" class="form-control" autocomplete="off" data-toggle="modal" data-target="#job_modal" placeholder="Jobcode">
													</td>
													<td width="7%">
													...
													<td width="6%">
														...
														<option value="SR">SR</option><option value="ZR" selected>ZR</option></select>
													</td>
													<td width="9%">
...
$(document).ready(function () { 
	...
	var urlvchr = "{{ url('sales_split/checkvchrno/') }}";
	var urlcode = "{{ url('sales_split/checkrefno/') }}"; //CHNG
	$('.sedePrntItm').toggle();
	<?php if($formdata['item_import']==0) { ?>
    $('#frmSalesSplit').bootstrapValidator({
        fields: {
			...
			voucher_no: {
                validators: {
					...
					remote: {
                        ...
                        message: 'This SS. No. is already exist!'
                    }
                }
            },
			reference_no: {
...
$(function() {	
	...
	$(document).on('blur', '.line-quantity', function(e) {
		...
		$('#frmSalesSplit').bootstrapValidator('revalidateField', 'quantity[]');
	});
...
function getAccount(e) { 
	...
	var res = e.id.split('_');
	var curNum = res[1]; 
	var itmurl = "{{ url('sales_split/account_data/') }}/"+curNum;
	popup = window.open(itmurl, "Popup", "width=900,height=500,top=100,left=200");
	popup.focus();
	return false
}
...
function getAccountCr(e) { 
	...
	var res = e.id.split('_');
	var curNum = res[1]; 
	var itmurl = "{{ url('sales_split/account_data/') }}/"+curNum+"/cr";
	popup = window.open(itmurl, "Popup", "width=900,height=500,top=100,left=200");
	popup.focus();
	return false
}
