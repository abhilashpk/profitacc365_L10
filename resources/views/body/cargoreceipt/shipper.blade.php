		...
		<div class="panel panel-success filterable">
			<div class="panel-heading">
				<h3 class="panel-title">
					<i class="fa fa-fw fa-columns"></i> Shipper List
				</h3>
			</div>
			<div class="panel-body">
				<div class="table-responsive m-t-10">
					<button type="button" class="btn btn-primary createShipper">Create</button>
					<table class="table horizontal_table table-striped" id="tblShipper">
						<thead>
						<tr>
							<th>Name</th>
							<th>Phone</th>
						...
						<tr>
							<td><a href="" class="shipperRow" data-id="{{$row->id}}" data-name="{{$row->shipper_name}}" data-dismiss="modal">{{$row->shipper_name}}</a></td>
							<td><a href="" class="shipperRow" data-id="{{$row->id}}" data-name="{{$row->shipper_name}}" data-dismiss="modal">{{$row->phone}}</a></td>
						</tr>
					   @endforeach
						</tbody>
					</table>
...
<div class="panel panel-success filterable" id="newShipperFrm">
	...
	<div class="panel-body"><button type="button" class="btn btn-primary listShipper" >List Shipper</button>
		<div class="col-xs-10">
			<div id="addressDtls2">
...
$(document).ready(function() {  //createSalesman
	...
	if( $('#newShipperFrm').is(":visible") )
		$('#newShipperFrm').toggle();
	
    $('#sucessmsg2').hide();
    
    $('.createShipper').on('click', function() {
        
        if( $('#shipperList').is(":visible") )
            $('#shipperList').toggle();
		
        if( $('#sucessmsg2').is(":visible") )
            $('#sucessmsg2').toggle();
        
        if( $('#newShipperFrm').is(":hidden") )
            $('#newShipperFrm').toggle();

        $('.listShipper').on('click', function() {
        
			if( $('#newShipperFrm').is(":visible") )
				$('#newShipperFrm').toggle();
			
			if( $('#shipperList').is(":hidden") )
				$('#shipperList').toggle();
