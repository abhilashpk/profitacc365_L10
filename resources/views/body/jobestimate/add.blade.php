                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
							...
							<?php if($printid) { ?>
								@can('qs-print')
								 <a href="{{ url('job_estimate/print/'.$printid->id.'/'.$print->id) }}" target="_blank" class="btn btn-info btn-sm">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmQuotationSales" id="frmQuotationSales" action="{{ url('job_estimate/save') }}">
								...
								@if(Session::get('mod_vehicle_cust')==0)
    								...
    								<div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="joborder_id" readonly name="joborder_id" placeholder="Job Order No" autocomplete="off" onclick="getJoborder()">
                                        </div>
                                    </div>
    								<?php } ?>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="kilometer" name="kilometer" value="{{ old('kilometer') }}" placeholder="Kilometer">
                                    </div>
                                </div>
								<?php } else { ?>
			...
			/* voucher_no: { //CHNG
                validators: {
					...
					remote: {
                        ...
                        message: 'This Estimate No. is already exist!'
                    }
                }
            }, */
			reference_no: {
...
function getJoborder() { 
	...
	var wt = $(window).width();
	var customer_id = $("#customer_id").val();
	var pourl = "{{ url('job_order/get_order/') }}/"+customer_id+"/JE";
