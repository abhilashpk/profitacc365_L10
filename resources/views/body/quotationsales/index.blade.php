...
@section('content')
    <!-- Content Header (Page header) -->
		...
		<section class="content">
			<form class="form-horizontal" role="form" method="POST" name="frmQReport" id="frmQReport" target="_blank" action="{{ url('quotation_sales/search') }}">
			 ...
			 <div class="col-lg-12">
					<div class="panel panel-info">
						<div class="panel-heading clearfix">
							<h3 class="panel-title pull-left m-t-6">
								<i class="fa fa-fw fa-list-alt"></i> Quotation Sales Report
							</h3>
						</div>
						<div class="panel-body">
								<div class="row">
...
function funDelete(id,no) {
	if(no=='0') {
		...
		if(con==true) {
			var url = "{{ url('quotation_sales/delete/') }}";
...
$(function() {
		...
		var dtInstance = $("#tablePordersList").DataTable({
			...
			"searching": true,
			"ajax":{
					 "url": "{{ url('quotation_sales/paging/') }}",
					 "dataType": "json",
					 "type": "POST",
