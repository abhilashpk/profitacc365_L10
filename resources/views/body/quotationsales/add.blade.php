                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmQuotationSales"  id="frmQuotationSales" enctype="multipart/form-data" action="{{ url('quotation_sales/save') }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										<input type="text" class="form-control" id="quotation_id" readonly name="quotation_id" placeholder="Copy from existing Quotation" autocomplete="off" onclick="getQuotation()">
                                    </div>
                                </div>
								<?php } else { ?>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										<input type="text" class="form-control" id="quotation_id" readonly name="quotation_id" placeholder="Enquiry No" autocomplete="off" onclick="getEnquiry()">
                                    </div>
                                </div>
								<?php } else { ?>
								<input type="hidden" name="quotation_id" id="quotation_id">
								...
								<?php } ?>	
									...
									<?php $i++; } } else { ?>
										...
										<?php } else { ?>
								<input type="hidden" name="more_info_id" id="more_info_id">
								<?php } ?><?php if($formdata['desc_item']==1) { ?>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea name="foot_description" id="foot_description" class="form-control editor-cls" placeholder="Description">{{$headerdata}}</textarea>
                                    </div>
                                </div>
								<?php } ?>
...
//	$('#lpo_date').datepicker( { dateFormat: 'dd-mm-yyyy',minDate: new Date('{{$settings->from_date}}'),maxDate: new Date('{{$settings->to_date}}') } );
	...
	$(document).on('click', '.btn-add-item', function(e)  { 
        ...
        var controlForm = $('.controls .itemdivPrnt'),
			...
			newEntry.find($('.margin')).attr('id', 'marginItm_' + rowNum);
			...
			$.get("{{ url('purchase_order/getunit/') }}/" + itm_id, function(data) {
				...
				$('#itmunt_1').find('option').end()
				 .append($("<option></option>")
							.attr("value",value.id)
							.text(value.unit_name)); 
							$('#hidunit_1'+rowNum).val(value.unit_name);
				});
			});
		},
        minLength: 1,
    ...
    $(document).on('click', '#importFile', function(e) { 
		if($('#import_file').val()=='') {
			...
			$('#frmQuotationSales').attr('action', "{{url('quotation_sales/import')}}").submit();
		}
	});
...
function getQuotation() { 
	...
	var ht = $(window).height();
	var wt = $(window).width();
	var pourl = "{{ url('quotation_sales/get_quotations/') }}";
	popup = window.open(pourl, "Popup", "width=900,height=500,top=100,left=200");
	popup.focus();
	return false
}
