        ...
        <section class="content">
            ...
            <div class="col-lg-12">
               <div class="panel panel-info">
                        ...
                        <div class="panel-body" style="padding-left:50px;padding-bottom:40px;">
							<form class="form-horizontal" role="form" method="POST" name="frmJobReport" id="frmJobReport" target="_blank" action="{{ url('job_report/search') }}">
								...
								<div class="row">
									...
									<div class="col-xs-3">
										...
										<select id="vehicle" class="form-control select2" style="width:100%" name="vehicle_id">
									...
									<div class="col-xs-3">
										...
										<select id="search_type" class="form-control select2" style="width:100%" name="search_type">
											...
											<!--<option value="qtyhand_ason_priordate" <?php //if($type=='qtyhand_ason_priordate') echo 'selected';?>>Job in Hand as on Prior Date</option>-->
										</select>
										<span>Job:</span><br/>
										
										<select id="jobid" class="form-control select2" name="job_id" style="width:100%">
											<option value="">Select</option>
										</select>
									</div>
									...
									<div class="col-xs-3">
										<br/>
										<input type="checkbox" name="is_workshopsplit" value="1"> Vehicle
										 &nbsp;  <input type="checkbox" name="is_jobsplit" value="1"> Job Cargo
										 <br/>
...
$(document).ready(function () {
	...
	$(document).on('change', '#select22', function(e) {
		$.get("{{ url('job_order/getvehicle/') }}/" + this.value, function(data) { 
				//$('#vehicle').find('option').remove().end();
				$.each(data, function(key, value) {   
					$('#vehicle').find('option').end()
	...
	$(document).on('change', '#vehicle', function(e) {
		$.get("{{ url('job_order/vehiclejob/') }}/" + this.value, function(data) { 
				$('#jobid').find('option').remove().end();
				$.each(data, function(key, value) {   
					$('#jobid').find('option').end()
					 .append($("<option></option>")
								.attr("value",value.id)
