        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
						...
						if($type=='detail') { ?>
							<table border="0" style="width:100%;height:100%;">
								...
								<tr>
										...
										<b>Mfg. No: {{$report[0]->voucher_no}} </b>
										</td>
										<td  align="right" style="padding-left:0px;">
										<b>DATE:{{$report[0]->voucher_date}}  </b>
										</td>
								...
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										<td width="10%"><b>Finishned Product Code</b></td>
											<td width="10%"><b>Finished Product Name</b></td>
											<td width="0.005%"></td>
											...
											<td width="7%"><b>Other Cost:</b></td>
											<td width="8%" class="text-right"><b>Net Total</b></td>
										...
										<?php 
											$sptotal = $dtotal =$other_costs=$profits= $profit=$other_cost=$ctotal =$i = $total = $net_total = $ptotal=$potatal = $pertotal = $peravg = $sqty = $tcst = 0; $i=1;
										...
										<tr>
											...
											<td class="text-right">{{number_format($other_cost,2)}}</td>
											<td class="text-right">{{number_format($profits+$other_cost,2)}}</td>
										</tr>
										...
										<tr>
											...
											<td colspan="5"><b>Raw Materials</b>
												<table class="table horizontal_table table-striped" id="postsrw">
													...
													@php $rmtotal += $raw->total; @endphp
													<tr>
													<td>{{$raw->item_code}}</td>
														<td>{{$raw->description}}</td>
														<td>{{$raw->quantity}}</td>
														<td class="text-right">{{number_format($raw->unit_price,2)}}</td>
														<td class="text-right">{{number_format($raw->total,2)}}</td>
													</tr>
												 @endforeach  
									...
									<td></td>
										...
										<td class="text-right"><b>{{number_format($other_costs,2)}}</b></td>
										<td class="text-right"><b>{{number_format($ptotal+$other_costs,2)}}</b></td>
								...
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										<td></td>
											...
											<td width="8%" class="text-right">{{number_format($ndtotal,2)}}</td>
											<td width="8%" class="text-right"><b>{{number_format($nptotal+$ndtotal,2)}}</b></td>
						...
						<?php } else if($type=='summary') { ?>
							...
							<tbody id="bod">
								...
								<tr>
										...
										<td  align="left" style="padding-left:0px;">
										<h6><b>Mfg. No: {{$report[0]->voucher_no}} </b><br/>DATE:{{$report[0]->voucher_date}} 
								...
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										<?php 
											$sptotal = $dtotal =$other_costs=$profits= $profit=$other_cost=$ctotal =$i = $total = $net_total = $ptotal = $pertotal = $peravg = $sqty = $tcst = 0; $i=1;
											foreach($report as $row) { 
							  ...
							  <?php } ?>
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										<td></td>
											<td width="22%" colspan="4" align="right"><b>Net Total:</b></td>
											<td width="10%" class="text-right"><b>{{number_format($nsptotal,2)}}</b></td>
											<td width="8%" class="text-right">{{number_format($ndtotal,2)}}</td>
											<td width="8%" class="text-right"><b>{{number_format($nptotal+$ndtotal,2)}}</b></td>
					...
					<form class="form-horizontal" role="form" method="POST" name="frmExport" id="frmExport" action="{{ url('manufacture/export') }}">
