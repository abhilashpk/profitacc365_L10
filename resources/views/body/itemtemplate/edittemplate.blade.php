			...
			<div id="collapse{{$i}}" class="panel-collapse collapse {{($i==0)?'in':''}}">
				<div class="panel-body" style="padding-left:80px !important;">
					...
					<div id="itemdivPrnt_{{$i+1}}">
						...
						<input type="hidden" name="tplid[]" value="{{$row->id}}">
						...
						<label for="input-text" class="col-sm-1 control-label">{{$row->type_name}} - {{$jobdata[$k]->input_value}}</label>
							...
							<div class="col-sm-1">
								<input type="text" id="rmrk_{{$i+1}}" name="remarks[]" value="{{isset($jobdata[$k]->other_value->remarks)?$jobdata[$k]->other_value->remarks:''}}" class="form-control">
							</div>
							...
							<div class="col-sm-1" style="width:6% !important;">
								<input type="text" class="form-control ht" name="height[]" id="ht_{{$i+1}}" value="{{$jobdata[$k]->other_value->height}}" autocomplete="off" >
							</div>
							<label for="input-text" class="col-sm-1 control-label tiny" style="width:3% !important;">W</label>
							<div class="col-sm-1" style="width:6% !important;">
								<input type="text" class="form-control wt" name="width[]" id="wt_{{$i+1}}" value="{{$jobdata[$k]->other_value->width}}" autocomplete="off" >
							</div>
							...
							<div class="col-sm-1" style="width:8% !important;">
								<input type="text" class="form-control qty" name="quantity[]" value="{{$jobdata[$k]->other_value->quantity}}" id="qty_{{$i+1}}" readonly>
							</div> 
							@else
							<label for="input-text" class="col-sm-1 control-label tiny" >Qty.</label>
							<div class="col-sm-1" style="width:8% !important;">
								<input type="text" class="form-control qty" name="quantity[]" value="{{$jobdata[$k]->other_value->quantity}}" id="qty_{{$i+1}}">
						...
						<div class="form-group" id="{{str_replace(' ', '_',$row->type_name)}}">
							<label for="input-text" class="col-sm-2 control-label">{{$row->type_name}}</label>
							<div class="col-sm-6">
								<input type="file" id="input-20" name="attachment" class="file-loading" data-show-preview="true" accept="image/png, image/gif, image/jpeg" data-url="{{url('item_template/upload-attachment/')}}">
...
$(function() {	
	...
	$(document).on('click', '.btn-add-row', function(e)  { 
        ...
        var controlForm = $('.controlsP #itemdivPrnt_'+$(this).attr("data-id")),
			...
			newEntry.find($('input[name="tplid[]"]')).val('');
			//initailizeSelect2();
