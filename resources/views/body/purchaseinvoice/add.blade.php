                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmPurchaseInvoice" id="frmPurchaseInvoice" enctype="multipart/form-data" action="{{ url('purchase_invoice/save') }}">
                                    ...
                                    <div class="col-sm-10">
                                       <select id="department_id" class="form-control select2" style="width:100%; background-color:#85d3ef;" name="department_id">
											@foreach($departments as $drow)
												<option value="{{ $drow->id }}" {{($pideptid==$drow->id)?'selected':''}} >{{ $drow->name }}</option>
											@endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-10">
                                       <select id="voucher_id" class="form-control select2" style="width:100%" name="voucher_id">
                                           @foreach($vouchers as $voucher)
											<option value="{{ $voucher['id'] }}" {{($pivchrid==$voucher['id'])?'selected':''}}>{{ $voucher['voucher_name'] }}</option>
											@endforeach
                                        </select>
                                    </div>
                                </div>
										...
										<div class="itemdivChld">	
											<table border="0" class="table-dy-row">
												<tr>
													...
													<td width="7%">
														<select id="itmunt_{{$j}}" class="form-control select2 <?php if($errors->has('unit_id.'.$i)) echo 'form-error';?> line-unit" style="width:100%" name="unit_id[]"><option value="{{old('unit_id')[$i]}}">{{old('unit_id')[$i]}}</option></select>
													</td>
								...
								<div id="itmInfo">
									<button type="button" id="itmInfo_{{$j}}" class="btn btn-primary btn-xs">Dimension</button>
								</div>
									<?php } else { ?>
										...
										<?php $i++; } } else { ?>
											...
											<table border="0" class="table-dy-row">
												<tr>
													...
													@endif
													<td width="8%" class="itcod">
														<input type="number" id="itmqty_1" autocomplete="off" step="any" name="quantity[]" class="form-control line-quantity" placeholder="Qty." {{($formdata['location_item']==1)?'readonly':''}}><!--NOV24-->
													</td>
													<td width="8%">
													...
													<td width="6%">
														<select id="txincld_1" class="form-control select2 taxinclude" style="width:100%" name="tax_include[]"><option value="0" {{($settings->pi_vat_inc==0) ? 'selected' : ''}}>No</option><option value="1" {{($settings->pi_vat_inc==1) ? 'selected' : ''}}>Yes</option></select>
													</td>
													<td width="9%">
								...
								<fieldset>
									...
									<?php if(old('dr_acnt')) { $k = 0; 
										...
										<div class="OCdivChld">
											...
											<table border="0" class="table-dy-row">
												<tr>
													<td width="15%">
														<input type="hidden" name="dr_acnt_id[]" value="{{ $vouchers[0]['dr_account_master_id'] }}" id="dracntid_{{$l}}">
														<span class="small">Debit Account</span>
														<input type="text" id="dracnt_{{$l}}" name="dr_acnt[]" class="form-control" autocomplete="off" value="{{ $voucher['account_id'].'-'.$voucher['master_name'] }}" data-toggle="modal" data-target="#account_modal" placeholder="Debit Account">
													</td>
													<td width="1%">
									...
									<div class="OCdivChld">
											...
											<table border="0" class="table-dy-row">
												<tr>
													<td width="15%">
														<input type="hidden" name="dr_acnt_id[]" id="dracntid_1" value="{{ $vouchers[0]['dr_account_master_id'] }}">
														<span class="small">Debit Account</span>
														<input type="text" id="dracnt_1" name="dr_acnt[]" class="form-control" value="{{ $voucher['account_id'].'-'.$voucher['master_name'] }}" autocomplete="off" data-toggle="modal" data-target="#account_modal" placeholder="Debit Account">
													</td>
													<td width="10%">
														<span class="small">Reference</span>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea name="foot_description" id="foot_description" class="form-control editor-cls" placeholder="Description">{{isset($footer)?$footer:''}}</textarea>
                                    </div>
                                </div>
								<?php } ?>
								<hr/>
...
$(document).ready(function () { 
	...
	if(localStorage.getItem("PvcId")!=null) {
		
		$("#voucher_id").val(localStorage.getItem("PvcId")).change();
		$('#account_master_id').val(localStorage.getItem("PcrId"));
		$('#purchase_account').val(localStorage.getItem("pAc"));
...
$(function() {	
    ...
    { 
        ...
        var controlForm = $('.controls .OCdivPrnt'),
			...
			newEntry.find('input').val(''); 
			$('#dracntid_' + ocrowNum).val( $('#dracntid_1').val() );
			$('#dracnt_' + ocrowNum).val( $('#dracnt_1').val() );
	...
	$('#voucher_id').on('change', function(e){
		...
		localStorage.setItem("PvcId", vchr_id);
		$.get("{{ url('purchase_invoice/getvoucher/') }}/" + vchr_id, function(data) { console.log(data);
			...
			if(data.account_id!=null && data.account_name!=null) {
				...
				$('#dracntid_1').val(data.id);
				$('#dracnt_1').val(data.account_id+'-'+data.account_name);
				
				localStorage.setItem("pAc", data.account_id+'-'+data.account_name);
				localStorage.setItem("PcrId", data.id);
				
			} else {
				$('#purchase_account').val('');
				$('#account_master_id').val('');
				
				localStorage.setItem("pAc", '');
				localStorage.setItem("PcrId", '');
			}
