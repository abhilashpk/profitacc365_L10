...
@section('content')
    <!-- Content Header (Page header) -->
                ...
                <div class="panel panel-info">
                    <div class="panel-heading clearfix">
                        ...
                        <div class="pull-right">
							@can('ps-create')
                             <a href="{{ url('purchase_split_return/add') }}" class="btn btn-primary btn-sm">
		...
		<section class="content">
			<form class="form-horizontal" role="form" method="POST" name="frmPOReport" id="frmPOReport" action="{{ url('purchase_split_return/search') }}" target="_blank">
			 ...
			 <div class="col-lg-12">
					<div class="panel panel-info">
						<div class="panel-heading clearfix">
							<h3 class="panel-title pull-left m-t-6">
								<i class="fa fa-fw fa-list-alt"></i> Purchase Split Return Report
							</h3>
						</div>
						<div class="panel-body">
								<div class="row">
...
function funDelete(id) {
	var con = confirm('Are you sure delete this purchase split return?');
	if(con==true) {
		var url = "{{ url('purchase_split_return/delete/') }}";
...
$(function() {
		...
		var dtInstance = $("#tablePurSplit").DataTable({
			...
			"order": [[ 0, 'desc' ]],
			"ajax":{
					 "url": "{{ url('purchase_split_return/paging/') }}",
					 "dataType": "json",
					 "type": "POST",
					 "data": function(data){
   ...
   /*  $("#multiselect2").multiselect({
	...
	$(document).on('change', '#search_type', function(e) { //$('#search_type').on('change', function(e){
		...
		if(vchr=='supplier') {
			...
			$.ajax({
				url: "{{ url('purchase_split_return/getcustomer/') }}",
				//type: 'get',
			...
			$.ajax({
				url: "{{ url('sales_split/getsalesman/') }}",
				//type: 'get',
				//data: 'item_id='+item_id+'&unit_id='+unit_id,
				success: function(data) {
					$('#selsale').html(data);
