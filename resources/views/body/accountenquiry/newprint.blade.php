...
@php
    if (!function_exists('formatAmount')) {
        function formatAmount($amount) {
            ...
            if ($amount > 0) return number_format($amount, 2);
            if ($amount < 0) return '(' . number_format(abs($amount), 2) . ')';
            return '';
        }
    }

    $amtdate1 = formatAmount($amtdate1 ?? 0);
    $amtdate2 = formatAmount($amtdate2 ?? 0);
    $amtdate3 = formatAmount($amtdate3 ?? 0);
    $amtdate4 = formatAmount($amtdate4 ?? 0);
    $amtdate5 = formatAmount($amtdate5 ?? 0);
            ...
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
						...
						<table border="0" style="width:100%;height:100%;">
							...
							<tbody id="bod">
								<tr style="border:0px solid black;">
									...
									@if($iscustom==1)<div><h5>
										<b>@if(isset($headarr[$head]))
											{{$headarr[$head]->heading}}
										@endif
										</b></h5></div>
									@endif
									
									@if($iscustom==1 && $saleman!='')
									...
									<table border="0" style="width:100%;" >
										<tr>
											<td align="left" style="padding-left:0px;">
												...
												<br/>{{$resultrow[$key]->address}}{{($resultrow[$key]->phone!='')?' Ph:'.$resultrow[$key]->phone:''}} TRN No: {{$resultrow[$key]->vat_no}}</b>
												@if($saleman)
													<br/><b>Salesman: {{$saleman}}</b>
												@endif
												</p>
											</td>
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										<tr>
											...
											<td style="word-break: break-all;">{{$reference_from}}</td>	
									...
									<table style="width:100%;" class="tblstyle table-bordered">
										...
										<tr>
											<td class="highrow text-right" width="45%">
												@if(isset($headarr[$head]))
												<strong>{{$headarr[$head]->heading}} <?php echo ($ispdc && count($pdcs) > 0)?'Total with PDC':' Total';?>:</strong>
												@endif
											</td>
								...
								<?php } // ?>
									...
									<table style="width:100%;" class="tblstyle table-bordered">
										...
										<tr>
											...
											<td class="emptyrow text-right"><strong>{{number_format((int)$Sdr_total,2)}}</strong></td>
											<td class="emptyrow text-right"><strong>{{number_format((int)$Scr_total,2)}}</strong></td>
							...
							<tbody id="bod">
									...
									@if(is_array($pdclist) > 0)
										...
										<table border="0" style="width:100%;" >
											...
											<tr>
												...
												<td width="10%" class="text-right"><b>Balance</b></td>
											</tr> @php $pdcamt = 0; $ptype=''; @endphp
											
											@foreach($pdclist[$key] as $pdc)
											@php $pdcamt += $pdc->amount; $ptype=$pdc->type; @endphp
											<tr>
												<td>{{date('d-m-Y', strtotime($pdc->voucher_date))}}</td>
												<td >{{$pdc->cheque_no}}</td>
												<td >{{date('d-m-Y', strtotime($pdc->cheque_date))}}</td>
												<td >{{$pdc->code}}</td>
												<td >{{$pdc->description}}</td>
												<td class="emptyrow text-right">{{($pdc->type=='Dr')?$pdc->amount:''}}</td>
												<td class="emptyrow text-right">{{($pdc->type=='Cr')?$pdc->amount:''}}</td>
											...
											<tr>
												...
												<td></td> 
												<td class="highrow text-right"><strong>Total:</strong></td>
												<td class="emptyrow text-right"><strong>{{($ptype=='Dr')?number_format($pdcamt,2):''}}</strong></td>
												<td class="emptyrow text-right"><strong>{{($ptype=='Cr')?number_format($pdcamt,2):''}}</strong></td>
												<td class="emptyrow text-right"><strong>{{number_format($pdcamt,2)}}</strong></td>
											</tr>
											</table>
										<?php } ?>
									@else
								...
								<tr>
									<td>
										...
										<td class="emptyrow text-right"><strong>{{($grandTotal > 0)?number_format($grandTotal,2):'('.number_format(($grandTotal*-1),2).')'}}</strong></td>
										</tr>
										</table>
										@endif
									</td>
								...
								<tr>
									<td colspan="2"><br/>
										...
										<tbody>
											...
											<td align="center">{{((int)$balance > 0)?number_format((int)$balance,2):'('.number_format(((int)$balance*-1),2).')'}}</td>
										</tbody>
									</table>
									</td>
								</tr>
							...
							<table border="0" style="width:100%;height:100%;">
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										<td colspan="2" align="center">
										@php $grandTotal = 0; @endphp
										@if($type=='ageing_summary')
											@foreach($transactions as $agecat => $transactions2)
											<table border="0" style="width:100%;" >
												<tr>
													<td align="left" style="padding-left:0px;">
														<p><b>Category: {{$agecat}}</b></p>
													</td>
													<td align="right" style="padding-left:0px;">
														<p><b>From: {{$fromdate}} - To: {{$todate}}</b></p>
													</td>
											...
											<table class="tblstyle table-bordered" border="0" width="100%">
												...
												@foreach($transactions2 as $results)
													<?php 
														if(count($results) > 0) { 
										...
										@else
											...
											@if($pdclist)
													...
													<tr>
														...
														<td width="10%" class="text-right"><b>Credit</b></td>
														<td width="10%" class="text-right"><b>Balance</b></td>
													</tr> @php $pdcamt = 0; @endphp
													@foreach($pdclist[$key] as $pdc)
													@php $pdcamt += $pdc->amount; @endphp
													<tr>
														<td>{{date('d-m-Y', strtotime($pdc->voucher_date))}}</td>
														<td >{{$pdc->cheque_no}}</td>
														<td >{{date('d-m-Y', strtotime($pdc->cheque_date))}}</td>
													...
													<tr>
														...
														<td></td> 
														<td class="highrow text-right"><strong>Total:</strong></td>
														<td class="emptyrow text-right"><strong>{{($pdc->type=='Dr')?number_format($pdcamt,2):''}}</strong></td>
														<td class="emptyrow text-right"><strong>{{($pdc->type=='Cr')?number_format($pdcamt,2):''}}</strong></td>
														<td class="emptyrow text-right"><strong>{{number_format($pdcamt,2)}}</strong></td>
													</tr>
													</table>
											@else
											<table class="tblstyle table-bordered" border="0" width="100%">
												...
												//if($transaction->voucher_type == 'OBD') {
														...
														$balance_prnt = (float)($balance_prnt ?? 0);
														...
														//abi
															if($balance_prnt > 0)
																...
																$balance_prnt = '('.number_format(abs($balance_prnt),2).')';
															else $balance_prnt = '';
                    ...
                    <div class="btn-section">
						...
						<form class="form-horizontal" role="form" method="POST" name="frmSend" id="frmSend" target="_blank" enctype="multipart/form-data" action="{{ url('account_enquiry/send') }}">
							 ...
							 <div class="email details">
								<div class="container" >
									...
									<div class="col-md-10 col-sm-10 col-xs-10">
										<div class="col-sm-7">
										<input type="text" name="mailid" id="mailid"class="form-control"  value="{{(isset($key) && isset($resultrow[$key]))?$resultrow[$key]->email:''}}" style="width:100%" > 
										</div>
										<div class="col-sm-2">
