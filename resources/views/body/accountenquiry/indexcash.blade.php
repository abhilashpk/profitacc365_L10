            ...
            <div class="col-lg-12">
               <div class="panel panel-info">
                        <div class="panel-heading clearfix">
							...
							<br>
								<br>
								<form role="form" method="POST" name="frmAccUtility" id="frmAccUtility">
								<input type="hidden" name="_token" value="{{ csrf_token() }}">
								<input type="hidden" name="pono" id="pono">
								<input type="hidden" name="sono" id="sono">
                                        <button type="button" class="btn btn-primary btn-sm " onClick="updateCB()">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="table-responsive m-t-10">
								...
								@can('stmnt')
								<button type="button" class="btn btn-primary statement" id= "stmt" data-toggle="modal" data-target="#date_modal">Statement</button>
								@endcan
								<!--@can('out-stnd')
								...
								<button type="button" class="btn btn-primary ageingsmry" data-toggle="modal" data-target="#date_modal">Ageing Summary</button>
								@endcan
					...
					<div id="date_modal" class="modal fade animated" role="dialog" >
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<form class="form-horizontal" role="form" method="POST" target="_blank" name="frmSearchAccount" id="frmSearchAccount" action="{{ url('account_enquiry/search_account') }}">
									<input type="hidden" name="_token" value="{{ csrf_token() }}">
									...
									<div id="custom">
										<div class="form-group">
											...
											<div class="col-md-7">
												<select id="select1" multiple style="width:100%" class="form-control select2" name="category_id[]">
										...
										<div class="form-group">
											...
											<div class="col-md-7">
												<select id="select3" multiple style="width:100%" class="form-control select2" name="type_id[]">
										...
										<div class="form-group">
											...
											<div class="col-md-7">
												<input type="text" name="date_from" id="date_from" value="{{date('d-m-Y',strtotime($settings->from_date))}}" class="form-control input-sm" data-language='en' autocomplete="off">
										...
										<div class="form-group">
											...
											<div class="col-md-7">
												<input type="text" name="date_to" id="date_to" value="{{date('d-m-Y')}}" class="form-control input-sm" data-language='en' autocomplete="off">
										...
										<div class="form-group">
											<label for="inputUsername" class="col-md-4 control-label">Salesman</label>
											<div class="col-md-7">
												<select id="salesman_id" class="form-control select2" style="width:100%" name="salesman_id">
												<option value="">Select Salesman...</option>
												@foreach($salesman as $sman)
												<option value="{{$sman->id.'|'.$sman->name}}">{{$sman->name}}</option>
												@endforeach
											</select>
											</div>
										</div>
...
$(document).on('click','#stmt',function(e){
	//alert("hai");
	//$('#date_from').datepicker('enable');
	//$(this).datepicker('show');
	$("#date_from").load();
	//$('#date_from').datepicker( { autoFocus:true,autoClose:true ,dateFormat: 'dd-mm-yyyy' } );
...
$(function() {	
	...
	$(document).on('click', '.ageingsmry', function(e) { 
	   $('#type').val('ageing_summary');
	   if($('#is_custom').val()==1)
		   $('#custom').show();
	   else {
		   $('#custom').hide();
	...
	$(function() {
		...
		var dtInstance = $("#Acenquiry").DataTable({
			...
			"serverSide": true,
			"ajax":{
					 "url": "{{ url('account_enquiry/cashpaging/') }}",
					 "dataType": "json",
					 "type": "POST",
					 "data": function(data){
