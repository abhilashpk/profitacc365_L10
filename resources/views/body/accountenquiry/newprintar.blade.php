        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
					<form class="form-horizontal" role="form" method="POST" name="frmSave" action="{{ url('account_enquiry/save_reconciliation') }}">
						...
						<table border="0" style="width:100%;height:100%;">
							...
							<tbody id="bod">
								<tr style="border:0px solid black;">
									...
									@php $Sbalance_prnt = $Sdr_total = $Scr_total = $balance_prnt_cb = $balance_prnt_ucb = 0; @endphp
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										@if($uncleared)
											...
											<tr>
												...
												<td>
													...
													<input type="text" autocomplete="off" name="bank_date[{{$urow->id}}]" value="{{isset($actrndata[$urow->id])?date('d-m-Y',strtotime($actrndata[$urow->id])):''}}" class="chqdate" data-language='en'>
													<input type="hidden" autocomplete="off" name="hd_bank_date[{{$urow->id}}]" value="{{isset($actrndata[$urow->id])?date('d-m-Y',strtotime($actrndata[$urow->id])):''}}" class="chqdate" data-language='en'>
													@else
													<input type="text" autocomplete="off" name="bank_date[{{$urow->id}}]" value="" class="chqdate" data-language='en'>
													<input type="hidden" autocomplete="off" name="hd_bank_date[{{$urow->id}}]" value="{{isset($actrndata[$urow->id])?date('d-m-Y',strtotime($actrndata[$urow->id])):''}}" class="chqdate" data-language='en'>	
													@endif
												</td>
										...
										<tr>
											...
											<td>@if($trans->voucher_type!='OB')
													@if($trdate >= $fdate && $trdate <= $tdate)
													<input type="text" autocomplete="off" name="bank_date[{{$trans->id}}]" value="{{isset($actrndata[$trans->id])?date('d-m-Y',strtotime($actrndata[$trans->id])):''}}" class="chqdate" data-language='en'>
													<input type="hidden" autocomplete="off" name="hd_bank_date[{{$trans->id}}]" value="{{isset($actrndata[$trans->id])?date('d-m-Y',strtotime($actrndata[$trans->id])):''}}" class="chqdate" data-language='en'>
													@else
													<input type="text" autocomplete="off" name="bank_date[{{$trans->id}}]" value="" class="chqdate" data-language='en'>
													<input type="hidden" autocomplete="off" name="hd_bank_date[{{$trans->id}}]" value="{{isset($actrndata[$trans->id])?date('d-m-Y',strtotime($actrndata[$trans->id])):''}}" class="chqdate" data-language='en'>	
													@endif
												@endif
											</td>
