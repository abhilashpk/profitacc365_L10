                    ...
                    <div class="panel panel-<?php echo ($orderrow->is_editable==1)?'danger':'primary';?>">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                                <i class="fa fa-fw fa-crosshairs"></i> <?php if($orderrow->is_editable==1) { echo 'Non-Editable'; } else echo 'Goods Receipt Note';?>
                            </h3>
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmPurorder" id="frmPurorder" action="{{ url('suppliers_do/update/'.$orderrow->id) }}">
													...
													<?php 
														...
														<div class="col-xs-4">
															<table class="table table-bordered table-hover">
																...
																//foreach($itemlocedit[$item] as $row) { 
																$quantity = $id = '';
																...
																<tr>
																	<td>{{ $row->name }}</td>
																	<td>{{ $cqty }}</td>
																	<td class="num"><input type="number" name="locqty[<?php echo $i-1;?>][]" value="{{$quantity}}" class="loc-qty-{{$j}}" data-id="{{$j}}">
											...
											<table border="0" class="table-dy-row">
												<tr>
													...
													<td width="8%">
														...
														<input type="number" id="itmqty_{{$i}}" autocomplete="off"step="any"  name="quantity[]" class="form-control line-quantity" value="{{$item->quantity}}" {{($formdata['location_item']==1)?'readonly':''}}  {{($orderrow->is_editable==1)?'readonly':''}}>
											...
											<div class="locPrntItm" id="locPrntItm_{{$i}}">
												<div class="locChldItm">							
													<div class="table-responsive loc-data" id="locData_{{$i}}"> 
													 <?php if($formdata['location_item']==1 && array_key_exists($item->id, $itemlocedit)) {  ?>
														<br/>
														 <div class="col-xs-4">
															<table class="table table-bordered table-hover">
																<thead>
																...
																<?php $tQty = 0; //MAY25
																foreach($itemloc as $key => $row) {
																//foreach($itemlocedit[$item] as $row) {  type
																$quantity = $id = $qty_entry = '';
																$location_id = $row->id; $cqty = $row->cqty; //0;//$row->cqty;
																if(array_key_exists($row->id, $itemlocedit[$item->id])) { 
																	$cqty = isset($itemlocedit[$item->id][$row->id])?$itemlocedit[$item->id][$row->id]->cqty:'';
																	$quantity = isset($itemlocedit[$item->id][$row->id])?$itemlocedit[$item->id][$row->id]->quantity:'';
																	$location_id = isset($itemlocedit[$item->id][$row->id])?$itemlocedit[$item->id][$row->id]->location_id:'';
																	$id = isset($itemlocedit[$item->id][$row->id])?$itemlocedit[$item->id][$row->id]->id:'';
																	//$tQty += $quantity; 
																...
																<tr>
																	<td>{{ $row->name }}</td>
																	<td>{{ $cqty }}</td>
																	<td class="num"><input type="number" name="locqty[<?php echo $i-1;?>][]" value="{{$qty_entry}}" class="loc-qty-{{$i}}" data-id="{{$i}}" {{($orderrow->is_editable==1)?'readonly':''}}>
																	<input type="hidden" name="locid[<?php echo $i-1;?>][]" value="{{$location_id}}"/>
																	<input type="hidden" name="editid[<?php echo $i-1;?>][]" value="{{$id}}"/></td>
																</tr>
																<?php } ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<a href="{{ url('suppliers_do/edit/'.$orderrow->id) }}" class="btn btn-warning">Clear</a>
                                    </div>
                                </div>
...
$(document).ready(function () { 
	...
	$('.custom_icheck').on('ifChecked', function(event){ 
		$("#currency_id").prop('disabled', false);
		$("#currency_rate").prop('disabled', false);
		$("#fc_label").toggle();$("#c_label").toggle(); $("#total_fc").toggle(); $("#discount_fc").toggle(); $("#net_amount_fc").toggle();$("#vat_fc").toggle();$("#other_cost_fc").toggle();
	});
...
$(function() {	
		...
		e.preventDefault();
		 var controlForm = $('.controls .itemdivPrnt'),
			...
			@if($formdata['location_item']==1)
			    newEntry.find( $('#frmPurchaseInvoice').bootstrapValidator('addField',"iloc[]") ); 
			@endif
	...
	$(document).on('click', '.loc-info', function(e) { 
	   ...
	   if(item_id!='') {
		   var locUrl = "{{ url('itemmaster/get_locinfo/') }}/"+item_id+"/"+curNum+"/"+pi_id+"/SDO";
