											...
											<td>
											    <div class='btn-group drop_btn' role='group'>
													...
													<ul style='min-width:100px !important;' class='dropdown-menu' aria-labelledby='exampleIconDropdown1' role='menu'>
														<li role='presentation'><a href="{{ url('suppliers_do/edit/'.$order->id)}}"  role='menuitem'>Edit</a></li>
														<li role='presentation'><a href="{{ url('suppliers_do/refresh_sdo/'.$order->id)}}" role='menuitem'>Refresh</a></li>
													</ul>
												</div>
												
												<p></p>
											</td>
											<td>	<button class="btn btn-primary btn-xs" onClick="location.href='{{ url('suppliers_do/viewonly/'.$order->id) }}'">
	...
	var dtInstance = $("#tablePorders").DataTable({
		...
		$('#select22').on('change', function(e){
			    var sup_id = $('#select22').val();
			     ...
			     if(sup_id !=''){
			    $.get("{{ url('suppliers_do/getjob/') }}/" + sup_id, function(data) {
