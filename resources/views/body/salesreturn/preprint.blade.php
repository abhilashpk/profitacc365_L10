        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
						...
						if($type=='summary') { ?>
							<table border="0" style="width:100%;height:100%;">
                                ...
                                <th>SI.#</th>
									...
									<th class="text-right">SR.Date</th>
									...
									<th class="text-right">Net Total</th>
											...
											<tbody>
											<?php $gross_total = $discount_total = $vat_total = $net_total = $total_amt=0; ?>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										...
										<table class="tblstyle table-bordered" width="100%">
											...
											@php $i++; @endphp
											<tr>
												<td><b>SI.No:</b> {{$i}} &nbsp;  <b>SR Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
												<td><b>SR.#:</b> {{$report[0]->voucher_no}}</td>
											...
											<?php $gtotal =$namount=$namount_total=$vtotal=$qtotal =$total_amt=0; ?>
												...
												$qtotal += $row->quantity;
												 ...
												 if($row->tax_include==1 && $row->discount==0){
												  ...
												  $namount=$row->total_price;
												     ...
												     $total_amount=$row->total_price-$row->discount-$row->item_vat;
												 $namount=($row->total_price-$row->discount);
												 }
												 else{
												...
												<tr>
													...
													<td class="text-right">{{number_format($row->item_vat,2)}}</td>
													<td class="text-right">{{number_format($namount,2)}}</td>
												</tr>
												@endforeach
												<tr>
													<td></td>
													<td class="text-right"><b>Sub Total:</b></td>
													<td class="text-right"><b>{{$qtotal}}</b></td>
													<td class="text-right"></td>
													<td class="text-right"></td>
													<td class="text-right"><b>{{number_format($gtotal,2)}}</b></td>
													<td class="text-right"><b>{{number_format($vtotal,2)}}</b></td>
													<td class="text-right"><b>{{number_format($namount_total,2)}}</b></td>
												</tr>
					...
					<form class="form-horizontal" role="form" method="POST" name="frmExport" id="frmExport" action="{{ url('sales_return/export') }}">
