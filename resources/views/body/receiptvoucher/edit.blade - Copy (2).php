...
// Keep your existing datepicker (AirDatePicker)
"use strict";

// === Helper: tell JS if this RV has any PDC lines (from server) ===
var IS_PDC = {{ $ispdc ? 'true' : 'false' }};

// === jQuery Validation setup (added) ===
(function setupValidation() {
	// custom rule for DR = CR
	...
	// custom rule: required if PDC context
	// ✅ row-aware PDC check (per-row, not global)
jQuery.validator.addMethod("requiredIfPDC", function(value, element) {
	...
	if (!isPdc) return true; // not PDC row
	return $.trim(value).length > 0;
}, "This field is required for PDC rows.");


	$('#frmJournal').validate({
		ignore: [], // validate hidden fields too (like select2 originals if any)
		...
		rules: {
			...
			'line_amount[]':  { required: true, number: true },
			// PDC only fields (gate behind custom rule)
			'bank_id[]':   { requiredIfPDC: true },
			'cheque_no[]': { requiredIfPDC: true },
			'cheque_date[]': { requiredIfPDC: true },
			'party_name[]':  { requiredIfPDC: true },

			// virtual field checks for DR/CR equality
			debit:  { balanced: true },
		...
		// place errors right after the input (works with arrays)
		errorPlacement: function(error, element) {
			if (element.hasClass('select2') || element.next('.select2').length) {
				// if select2, place after its container
				error.insertAfter(element.closest('.select2'));
			} else {
				error.insertAfter(element);
			}
		},
		// when cloning rows, we add rules programmatically — but this keeps existing ones working too
		highlight: function(element) { $(element).addClass('has-error'); },
		unhighlight: function(element) { $(element).removeClass('has-error'); },
		invalidHandler: function() {
			// ensure totals validate again
			$('#debit, #credit').valid();
		},
		submitHandler: function(form) {
			// Final safety: make sure balanced before submit
			var dr = parseFloat($('#debit').val() || 0);
			var cr = parseFloat($('#credit').val() || 0);
	...
	// Add rules for already-rendered rows (server output)
	function addRulesForExistingRows() {
		$('.itemdivPrnt .itemdivChld').each(function() {
			...
			if (IS_PDC) {
				$(this).find('select[name="bank_id[]"]').rules('add',   { requiredIfPDC:true });
				$(this).find('input[name="cheque_no[]"]').rules('add',  { requiredIfPDC:true });
				$(this).find('input[name="cheque_date[]"]').rules('add',{ requiredIfPDC:true });
				$(this).find('input[name="party_name[]"]').rules('add', { requiredIfPDC:true });
			}
		});
	}
	addRulesForExistingRows();

		// Expose helper for new rows
		...
		// Row-aware PDC condition (not global)
		const groupVal = $('#groupid_'+rowNum).val();
		const isPdc = (groupVal === 'PDCR' || groupVal === 'PDCI');
		if (isPdc) {
...
$(document).ready(function () {
	...
	// (Removed BootstrapValidator init)
...
// Keep your existing helpers and events unchanged below
...
// Clone row — unchanged, plus call addRulesForRow(rowNum)
 $(document).on('click', '.btn-add-item', function(e)  { 
        ...
        var controlForm = $('.controls .itemdivPrnt'),
			...
			// ✅ Properly register new inputs for validation
			setTimeout(function() {
				const $form = $('#frmJournal');
				if ($.data($form[0], 'validator')) {
					// Tell validator to forget cached elements
					$form.removeData('validator');
					$form.validate(); // reinitialize
				}
	...
	// Removed bootstrapValidator revalidate on datepicker click.
	$(document).on('click','.datepicker--cell-day',function() {
		// no-op; jQuery Validate reacts on blur/change
	});
	...
	// Accounts modal loaders: unchanged
	...
	$(document).on('click', '#account_modal .custRow', function(e) { 
		...
		if( $(this).attr("data-group")=='SUPPLIER' || $(this).attr("data-group")=='CUSTOMER') {
		...
		// validate account for this row
		$('#draccount_'+num).valid();
	});
	
	// Removed bootstrapValidator revalidates:
	...
	$(document).on('click', '.add-invoice', function(e)  { 
		...
		$.each(refs,function(i) { 
			...
			// add validation rules for the row we just set up
			var target = (no < $('#rowNum').val()) ? rowNum : rnum;
			if (typeof window.addRulesForRow === 'function') {
				window.addRulesForRow(target);
			}
	...
	$(document).on('click', 'input[name="party_name[]"]', function(e) {
		var res = this.id.split('_');
		var curNum = res[1]; 
		$('#paccount_data').load(acurlall+'/'+curNum, function(result){ 
			$('#myModal').modal({show:true});
		});
	});
	...
	$(document).on('click', '#paccount_data .custRow', function(e) { 
		...
		checkChequeNo(num);
		// jQuery validate this party field now
		$('#party_'+num).valid();
	});
});
		...
		$.ajax({
			...
			success: function(data) { 
				if(data=='') {
					...
					$('#chkno_'+curNum).val('');
					$('#chkno_'+curNum).valid(); // trigger revalidation
				}
			}
		})
	}
