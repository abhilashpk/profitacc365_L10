                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-12">
							<?php if($type=="detail") { ?>
								<table class="table" border="0">
									...
									<tr>
										...
										<td><b>SO.#:</b> {{$report[0]->voucher_no}}</td>
							...
							<?php } elseif($type=="detail_pending") { ?>
								<table class="table" border="0">
									...
									<tr>
										<td><b>SI.No:</b> {{$report[0]->voucher_no}}</td>
										<td><b>SO.#:</b> {{$report[0]->voucher_no}}</td>
										<td colspan="2"><b>SO.Ref.#:</b> {{$report[0]->reference_no}}</td>
							...
							<?php } elseif($type=="jobwise" || $type=="customer_wise") { ?>
							<table class="table" border="0">
								<thead>
									<th>Customer</th>
									<th>Date</th>
									...
									<!--<th class="text-right">Received Retention</th>-->
									<th class="text-right">Balance</th>
								</thead>
								<body>
									<?php $total_net = $discount = $total_vat = $total_gross = $tot_retention = $tot_balance = 0;?>
									@foreach($reports as $row)
									<?php 
										  $total_net += $row->net_total;
										  $retention = $row->less_amount + $row->less_amount2 + $row->less_amount3;
										  $tot_retention += $retention;
										  $balance = $row->net_total - $retention;
										  $tot_balance +=  $balance;
									?>
									<tr>
										<td>{{$row->master_name}}</td>
									...
									<tr>
										...
										<td class="text-right"><b>Total:</b></td>
										<td class="text-right"><b>{{number_format($total_net,2)}}</b></td>
										<!--<td class="text-right"><b>{{number_format($tot_retention,2)}}</b></td>-->
										<td class="text-right"><b>{{number_format($tot_balance,2)}}</b></td>
									</tr>
								</body>
							</table>
							<?php } else { ?>
