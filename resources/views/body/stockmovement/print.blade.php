        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
						...
						<table border="0" style="width:100%;height:100%;">
							<thead>
								...
								<tr>
									...
									@if($fromdate!='' && $todate!='')
										<h6><b>Date: <b>{{date('d-m-Y', strtotime($fromdate))}} 
											   - <b>{{date('d-m-Y', strtotime($todate))}}</b></h6>
									@endif
									</td>
							...
							<tbody id="bod">
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										@php $i=$total_balance=$total_value=$total_in=$total_out=0;$total_opnqty=$total_unitcost=$total_purcost=$total_salecost=0; @endphp
										@foreach($results as $row)
										@php $i++; $total_balance += $row->qty_bal; $total_value += $row->net_value; $total_in += $row->qty_in; $total_out += $row->qty_out;$total_opnqty += $row->opn_qty;$total_unitcost +=$row->unit_cost;$total_purcost +=$row->pur_cost;$total_salecost +=$row->sale_cost; @endphp
										<tr>
											<td>{{$i}}</td>
											<td>{{$row->item_code}}</td>
											<td>{{$row->description}}</td>
											<td class="text-right">{{$row->opn_qty}}</td>
											<td class="text-right">{{$row->opn_cost}}</td>
											<td class="text-right">{{$row->unit_cost}}</td>
											<td class="text-right">{{number_format($row->cost_avg,2)}}</td>
											<td class="text-right">{{$row->qty_in}}</td>
											<td class="text-right">{{$row->pur_cost}}</td>
											<td class="text-right">{{$row->qty_out}}</td>
											<td class="text-right">{{$row->sale_cost}}</td>
											<td class="text-right">{{$row->qty_bal}}</td>
										
											<td class="text-right">{{number_format($row->net_value,2)}}</td>
										</tr>
										@endforeach
										<tr>
											<td colspan="3" class="text-right"><b>Total: </b></td>
											<td class="text-right"><b>{{$total_opnqty}}</b></td>
											<td class="text-right"><b></b></td>
											<td class="text-right"><b>{{$total_unitcost}}</b></td>
											<td class="text-right"><b></b></td>
											<td class="text-right"><b>{{$total_in}}</b></td>
											<td class="text-right"><b>{{$total_purcost}}</b></td>
											<td class="text-right"><b>{{$total_out}}</b></td>
											<td class="text-right"><b>{{$total_salecost}}</b></td>
											<td class="text-right"><b>{{$total_balance}}</b></td>
							...
							<tbody id="bod">
								<tr style="border:0px solid black;">
									...
									<table border="0" style="width:100%;" class="tblstyle table-bordered">
										...
										@if($result) @php $ctotal = 0; @endphp
										<tr>
											<td colspan="8"><b>{{$key}}</b></td>
										</tr>
										@foreach($result as $row)
										@php $ctotal += $row->sale_reference; @endphp
										<tr>
					...
					<form class="form-horizontal" role="form" method="POST" name="frmExport" id="frmExport" action="{{ url('stock_movement/export') }}">
