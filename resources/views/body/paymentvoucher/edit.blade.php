                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                                <i class="fa fa-fw fa-crosshairs"></i> Edit Payment Voucher
                            </h3>
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmJournal" id="frmJournal" action="{{ url('supplier_payment/update/'.$crrow->id) }}">
								...
								<input type="hidden" name="from_pv" value="1">
								...
								<legend><h5>Transactions</h5></legend>
										@php $i = 0; $num = count($invoicerow); @endphp
...
$(document).ready(function () {
	...
	var confirmBtnDefaultHtml = $confirmBtn.length ? $confirmBtn.html() : '';
...
$(function(){
  ...
  $(document).on('click', '.btn-add-item', function(e)  { 
		...
		$('#rowNum').val(rowNum);
		e.preventDefault();
		if (typeof window.validateAllRows === 'function') {
			if (!window.validateAllRows()) {
...
var controlForm = $('.controls .itemdivPrnt'),
    ...
    { 
		...
		var curNum = (parts.length > 1) ? parts.pop() : null;
		if(!curNum || curNum === '') {
			var rowId = $(this).closest('.itemdivChld').find('.classtrn').attr('id') || '';
			var rowParts = rowId.split('_');
			curNum = (rowParts.length > 1) ? rowParts.pop() : null;
		}
		if(!curNum){
			e.preventDefault();
			return false;
		...
		var jeVal = $('#jeid_'+curNum).val();
		if(jeVal){
			ids = remitem ? remitem + ',' + jeVal : jeVal;
			$('#remitem').val(ids);
		}
		...
		var lastIndex = window.getLastRowIndex ? window.getLastRowIndex() : 1;
		if($('#rowNum').length){ $('#rowNum').val(lastIndex); }
		if (typeof window.refreshRowButtons === 'function') {
			window.refreshRowButtons();
		}
		if (typeof window.validateAllRows === 'function') {
			window.validateAllRows();
		}
				if (typeof window.checkDuplicateChequeTriplet === 'function') {
			window.checkDuplicateChequeTriplet();
		}
...
$(document).on('click', '#account_modal .custRow', function(e) { 
		...
		if($(this).attr("data-group")=='PDCR') { 
			...
			if($('#jeid_'+num).val()!='')
			    $('#trns_'+num).load( trnurl+'/PDC/'+$(this).attr("data-id")+'/'+num+'/'+$('#jeid_'+num).val(), function(){
					if (typeof window.addRulesForRow === 'function') { window.addRulesForRow(num); }
				});
			else
			...
			var trnurl = "{{ url('journal/set_transactions/') }}";
			if($('#jeid_'+num).val()!='')
			    $('#trns_'+num).load( trnurl+'/BANKS/'+$(this).attr("data-id")+'/'+num+'/'+$('#jeid_'+num).val(), function(){
					if (typeof window.addRulesForRow === 'function') { window.addRulesForRow(num); }
				});
			else
			    $('#trns_'+num).load( trnurl+'/BANKS/'+$(this).attr("data-id")+'/'+num, function(){
					if (typeof window.addRulesForRow === 'function') { window.addRulesForRow(num); }
				});
    ...
    $(document).on('click', '#paccount_data .custRow', function(e) {
		...
		// ✅ Force full revalidation after selecting party
		if (window.validator) {
			$('#party_' + num).removeClass('is-invalid');
			window.validator.element('#party_' + num);   // validate immediately
			validator.resetForm();                       // clear stale errors
			addRulesForRow(num);                         // reattach dynamic rules
		}
...
(function(){
  ...
  function isPdcRow(idx){
    var g = ($('#groupid_'+idx).val() || '').toString().trim().toUpperCase();
       return (g === 'PDCI' || g === 'PDCR'); // mirror add.blade behaviour
  }
 ...
 $.validator.addMethod('balanced', function(value, element){
	...
	submitHandler: function(form){
		...
		if(!ok){
			...
			if(!$firstInvalid.length){
				$firstInvalid = $('.dup-error-label:visible, label.text-danger:visible').first();
			}
			if($firstInvalid.length){
				$('html, body').animate({ scrollTop: Math.max($firstInvalid.offset().top - 80, 0) }, 300);
				if($firstInvalid.is(':input')){
					$firstInvalid.focus();
				} else {
					var $focusable = $firstInvalid.closest('.form-group').find('.is-invalid:visible:first');
					if($focusable.length){ $focusable.focus(); }
				}
			}
			return false;
		}
		window.openVoucherConfirmModal(form);
		return false;
	}
  });
  ...
  function addRulesForRow(idx){
    ...
    var rowIsPdc = isPdcRow(idx);
    var requiresPdcFields = function(){ return isPdcRow(idx); };
    var chequeFieldMessages = {
      '#bankid_': 'Bank is required',
      '#chkno_': 'Cheque no is required',
      '#chkdate_': 'Cheque date is required',
      '#party_': 'Party is required'
    };
    $.each(chequeFieldMessages, function(prefix, message){
      add(prefix + idx, {
        required: { depends: requiresPdcFields },
        messages: { required: message }
      });
      if (!rowIsPdc && window.validator && typeof window.validator.element === 'function'){
        var $el = $(prefix + idx);
        if ($el.length){
          window.validator.element($el); // clears stale errors when rule no longer applies
        }
      }
    });
  }
...
function validateAllRows(){
  ...
  window.refreshRowButtons = refreshButtons;
...
window.checkDuplicateChequeTriplet = checkDuplicateChequeTriplet;
  ...
  $(document).on('keyup blur change', 'input[name="cheque_no[]"], input[name="cheque_date[]"], select[name="bank_id[]"], input[name="party_name[]"]', function(){
    if (window.validator && this.id){
      window.validator.element('#'+this.id);
    }
