...
$(function(){
  ...
  $(document).on('click', '.btn-add-item', function(e)  { 
    ...
    if (typeof validateRow === 'function') {
        // If row fails validation, stop cloning
        if (!validateRow(rowNum)) {
            alert("Please fill all required fields correctly before adding a new row.");
            return false;
        }
    } else {
        // Fallback (your old direct checks)
        var group_id = $('#groupid_'+rowNum).val();
        if($('#draccount_'+rowNum).val()=='') { alert('Account name is required!'); return false; }
        if($('#ref_'+rowNum).val()=='') { alert('Reference No. is required!'); return false; }
        if($('#amount_'+rowNum).val()=='') { alert('Amount is required!'); return false; }
        if(group_id=='PDCR' || group_id=='PDCI') {
            if($('#bankid_'+rowNum).val()=='') { alert('Bank is required!'); return false; }
            if($('#chkno_'+rowNum).val()=='') { alert('Cheque No. is required!'); return false; }
            if($('#chkdate_'+rowNum).val()=='') { alert('Cheque date is required!'); return false; }
            if($('#party_'+rowNum).val()=='') { alert('Party account is required!'); return false; }
        }
    }
