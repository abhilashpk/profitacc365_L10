...
@section('content')
    <!-- Content Header (Page header) -->
        ...
        <section class="content">
            <div class="row">
                <div class="col-md-12">
                    <div class="panel panel-<?php echo ($crrow->approval_status==1)?'warning':'primary';?>">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                                <i class="fa fa-fw fa-crosshairs"></i><?php if($crrow->approval_status==1) { echo 'Approved'; } else echo 'Supplier payment'; ?>
                            </h3>
                           
						   <div class="pull-right">
								@can('pv-print')
								 <a href="{{ url('supplier_payment/printgrp/'.$crrow->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmJournal" id="frmJournal" action="{{ url('supplier_payment/update/'.$crrow->id) }}">
                                ...
                                <div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label"></label>
                                    <div class="col-sm-10">
                                         <button type="button" onClick="funApproval('{{ $crrow->id }}')" class="btn btn-primary" <?php if($crrow->approval_status==1) echo 'disabled';?>>Approve</button>
...
function funApproval(id) {
	var con = confirm('Are you sure you want to approve this payment voucher?');
	if(con==true) {
		var url = "{{ url('supplier_payment/approve/') }}";
