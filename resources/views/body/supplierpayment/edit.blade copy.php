...
$(function(){
  ...
  $(document).on('click', '.btn-add-item', function(e)  { 
		...
		//	newEntry.find($('.line-job')).attr('id', 'jobid_' + rowNum);
			...
			if (typeof window.applyRowLayout === 'function') {
				window.applyRowLayout(rowNum);
			}
    ...
    { 
		...
		if(jeVal){
			var ids = remitem ? remitem + ',' + jeVal : jeVal;
			$('#remitem').val(ids);
		}
...
function checkChequeNo(curNum) { 
	...
	$.ajax({
		...
		success: function(data) { 
			...
			if(window.validator){
				window.validator.element($field);
			}
		}
	});
}
...
(function(){
  "use strict";
  var trnurlBase = "{{ url('journal/set_transactions') }}/";

  /* ---------- utilities ---------- */
  function rowIdxFrom(container){
  ...
  function applyRowLayout(idx){
    var grp = ($('#groupid_'+idx).val() || '').toString().trim().toUpperCase();
    var $row = $('#trns_'+idx);
    if(!$row.length) return;
    var $nopdc = $row.find('[class*="nopdc"]');
    var $pdc   = $row.find('.pdcfm');
    if(grp === 'PDCI' || grp === 'PDCR'){
      $pdc.show();
      $nopdc.hide();
    } else {
      $pdc.hide();
  ...
  window.applyRowLayout = applyRowLayout;
  window.addRulesForRow = addRulesForRow;
  window.validateRow = validateRow;
  window.validateAllRows = validateAllRows;

  function initialiseRow(idx){
    addRulesForRow(idx);
    applyRowLayout(idx);
  }
  function hydrateRowIfNeeded(idx){
    var accountId = $('#draccountid_'+idx).val();
    if(!accountId){ return false; }
    var grp = ($('#groupid_'+idx).val() || '').toString().trim().toUpperCase();
    var path = '';
    if(grp === 'PDCI' || grp === 'PDCR'){
      path = 'PDC/' + accountId + '/' + idx;
    } else if (grp === 'BANK'){
      path = 'BANKS/' + accountId + '/' + idx;
    } else {
      return false;
    }
    var jeId = $('#jeid_'+idx).val();
    if(jeId){ path += '/' + jeId; }
    $('#trns_'+idx).load(trnurlBase + path, function(){
      initialiseRow(idx);
      refreshButtons();
