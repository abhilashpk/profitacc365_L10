        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										<td colspan="2" align="center">
											...
											<body>
												<?php $total_net = $discount = $total_vat = $total_gross =$total_amt=$total= $i = 0;?>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										<td colspan="2" align="center">
											...
											<body><?php $total_net = $discount = $total_vat = $total_gross =$total_amt=$total= $i = 0;?>
						...
						<?php } else if($type=='detail') { ?>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										...
										<table class="tblstyle table-bordered" width="100%">
											<body>
											<?php $qty_total = $net_total = $vat_total = $gross_total = $i = 0; ?>
											@foreach($reports as $report)
											@php $i++; @endphp
											<tr>
												<td><b>SI.No:</b> {{$i}} &nbsp; <b>DO.#:</b> {{$report[0]->voucher_no}} &nbsp; <b>DO Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
						...
						<?php } elseif($type=="detail_pending") { ?>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										...
										<table class="tblstyle table-bordered" width="100%">
											<body>
											<?php $poqty_total = $ponet_total = $invqty_total = $pndqty_total = $pndnet_total = $i = 0; ?>
											@foreach($reports as $report) @php $i++; @endphp
											<tr>
												<td><b>SI.No:</b> {{$i}} &nbsp; <b>DO.#:</b> {{$report[0]->voucher_no}} &nbsp; <b>DO Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
												<td><b>DO.Ref.#:</b> {{$report[0]->reference_no}}</td>
												<td colspan="2"><b>Job No</b> {{$report[0]->jobcode}}</td>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										...
										<table class="tblstyle table-bordered" width="100%">
											...
											@foreach($reports as $report)
											<tr>
												<td><b>SI.No:</b> {{$report[0]->voucher_no}} &nbsp; <b>DO.#:</b> {{$report[0]->voucher_no}} &nbsp; <b>DO.Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
												<td><b>DO.Ref.#:</b> {{$report[0]->reference_no}}</td>
												<td colspan="1"><b>Job No.</b> {{$report[0]->jobcode}}</td>
												<td colspan="1"><b>Salesman:</b> {{$report[0]->salesman}}</td>
												<td colspan="1" class="text-right"><b>Customer:</b> {{$report[0]->master_name}}</td>
											</tr>
											<tr>
												<td><b>Item Code</b></td>
												<td><b>Description</b></td>
					...
					<form class="form-horizontal" role="form" method="POST" name="frmExport" id="frmExport" action="{{ url('customers_do/export') }}">
