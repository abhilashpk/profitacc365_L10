...
@section('content')
    <!-- Content Header (Page header) -->
		...
		<section class="content">
			<form class="form-horizontal" role="form" method="POST" name="frmQReport" id="frmQReport" target="_blank" action="{{ url('customers_do/search') }}">
			 ...
			 <div class="col-lg-12">
					<div class="panel panel-info">
						<div class="panel-heading clearfix">
							<h3 class="panel-title pull-left m-t-6">
								<i class="fa fa-fw fa-list-alt"></i> @php echo (Session::get('trip_entry')==1)?'Daily Entry':'Delivery Order'; @endphp Report
							</h3>
						</div>
						<div class="panel-body">
								<div class="row">
									...
									<div class="col-xs-6">
										...
										<select id="search_type" class="form-control select2" style="width:100%" name="search_type">
											...
											<!--<option value="summary_pending">Summary(Pending @php echo (Session::get('trip_entry')==1)?'DE':'DO'; @endphp)</option>
											
											<option value="detail_pending">Detail(Pending @php echo (Session::get('trip_entry')==1)?'DE':'DO'; @endphp)</option>
...
function funDelete(id,no) {
    if(no=='0') {
		...
		alert('This order is transfered to invoice, you cannot edit or delete');
}
...
$(function() {
		...
		var dtInstance = $("#tablePordersList").DataTable({
			...
			$('#select23').on('change', function(e){
			    ...
			    console.log(cust_id);
			     if(cust_id !=''){
			    $.get("{{ url('customers_do/getjob/') }}/" + cust_id, function(data) {
