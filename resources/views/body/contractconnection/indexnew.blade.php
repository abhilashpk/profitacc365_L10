...
var bid = $('#building_id option:selected').val();		
  $('#reportResult').load("{{ url('contract-connection/connection_list/') }}/"+bid);
});
$(document).on('click', '.searchRead', function(e) {
	...
	if(bid=='') {
		...
		if(fno!='')
  			$('#reportResult').load("{{ url('contract-connection/connection_list/') }}/"+bid+"/"+fno);
			else
  $('#reportResult').load("{{ url('contract-connection/connection_list/') }}/"+bid);
...
$(document).on('click', '.clearFun', function(e) {
	localStorage.clear();
	location.href="{{ url('contract-connection') }}";
});
