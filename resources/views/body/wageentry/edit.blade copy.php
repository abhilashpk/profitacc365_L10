            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="panel panel-primary">
						...
						</div>		<?php	if($wagerow->entry_type=="daily") {  ?>		
								<hr/>
								<table border="0" class="table-dy-row">
								    <tr>
								        <td></td>
								        ...
								        <td></td>
								        <td></td>
								        <td><input type="hidden" id="total_wdays" step="any" name="total_wdays" value="{{$wagerow->wdays_total}}" class="form-control" readonly placeholder="0"></td>
								        <td width="8%">
...
var otgs_nett=parseFloat({{$wagerow->otgs_total}});	
...
function getLineTotal(n) {
    // Per-hour wage & inputs (safe-parsed)
    var wage = num(wg); // if you want row-specific: num($('#wage_'+n).val())
    var nwh  = num($('#nwh_'+n).val());
    var nod  = num($('#nodays_'+n).val());
    var dayhr = (nod > 0) ? (nwh / nod) : 0;

    // OT hours entered
    var otg = num($('#otg_'+n).val()); // General OT hours
    var oth = num($('#oth_'+n).val()); // Holiday OT hours

    // OT amount per type (wage/hour * hours * multiplier)
    var otg_tot = (wage * otg * num(otper));   // amount
    var oth_tot = (wage * oth * num(othper));  // amount

    // Allowance per row (per-hour allowance * worked hours)
    var aln = num(alhr) * nwh;

    // Unpaid leave deduction (monthly only)
    var uplev = 0;
    <?php if($wagerow->entry_type=="monthly") { ?>
        uplev = num($('#uplevd_'+n).val());
    <?php } ?>
    var uplevwg = wage * dayhr * uplev;

    // Normal wage (no OT), amount
    var nor_wage = wage * nwh;

    // Line amount = normal + OT amounts
    var lineTotal = nor_wage + otg_tot + oth_tot;

    // Update row fields
    $('#itmttl_'+n).val(lineTotal.toFixed(3));     // amount
    $('#alw_'+n).val(aln.toFixed(3));              // amount
    $('#otgttl_'+n).val(otg_tot.toFixed(3));       // amount
    $('#othttl_'+n).val(oth_tot.toFixed(3));       // amount
    $('#deductions').val(uplevwg.toFixed(3));      // amount (global input used later)
    $('#otgst_'+n).val(otg.toFixed(3));            // HOURS
    $('#othst_'+n).val(oth.toFixed(3));            // HOURS

    return lineTotal;
}

function getNetTotal() {
    // --- Sum line amounts fresh (no carry-over!) ---
    var wage_total = 0;
    $('.line-total').each(function(){ wage_total += num(this.value); });

    var alw_total = 0;
    $('.line-alwnc').each(function(){ alw_total += num(this.value); });

    var wdays_net = 0;
    $('.line-nod').each(function(){ wdays_net += num(this.value); });

    // OT HOURS (for the “OT Hr Total (G/H)” boxes)
    var ot_hours_g = 0;
    $('.otgst').each(function(){ ot_hours_g += num(this.value); });

    var ot_hours_h = 0;
    $('.othst').each(function(){ ot_hours_h += num(this.value); });

    // OT AMOUNTS (for the “OT(General/Holiday)” net boxes)
    var ot_amt_g = 0;
    $('.otgtotal').each(function(){ ot_amt_g += num(this.value); });

    var ot_amt_h = 0;
    $('.othtotal').each(function(){ ot_amt_h += num(this.value); });

    // Unpaid leave days (monthly) -> deduction already set per-row in getLineTotal()
    var uplev = 0;
    $('.line-uplevd').each(function(){ uplev = num(this.value) * wg * nwhr; }); // keep your original behavior

    // Half-day handling
    var hd = 0;
    $('.leave').each(function(){ if (num(this.value) === 4) hd += 1; });
    var hd_de = (hd > 0) ? ((nwhr / 2) * hd * wg) : 0;

    // “lev” = rows with zero per-hour wage (treated as absent in your logic)
    var lev = 0;
    $('.wph').each(function(){ if (num(this.value) === 0) lev += 1; });

    var now_net = wdays_net - lev;

    // --- Write HOURS totals (NOT AMOUNTS) ---
    $('#otgs_tot').val(ot_hours_g.toFixed(3)); // “OTHr Total (G)” -> HOURS
    $('#oths_tot').val(ot_hours_h.toFixed(3)); // “OTHr Total (H)” -> HOURS
    $('#total_wdays').val(now_net.toFixed(3)); // days

    // --- Write AMOUNT totals for OT nets ---
    $('#otg_net').val(ot_amt_g.toFixed(3));    // amount
    $('#oth_net').val(ot_amt_h.toFixed(3));    // amount

    // Your existing net section math (cleaned: no double-counting)
    var alt  = lev * num(altrans);
    var alh  = lev * num(alhra);
    var bnet = lev * (nwhr * wg); // itl = nwhr*wg

    var basic_src = num($('#bas_net').val());  // original base
    var basic_net = basic_src - bnet;
    $('#basic_net').val(basic_net.toFixed(3));

    var hra_src = num($('#hraa_net').val());
    var hra_net = hra_src - alh;
    $('#hra_net').val(hra_net.toFixed(3));

    var allow_src = num($('#allow_net').val());
    var allowance_net = allow_src - alt;
    $('#allowance_net').val(allowance_net.toFixed(3));

    var deduction = num($('#deductions').val()); // from latest line calc
    deduction += hd_de;

    // Other allowances/deductions
    var oth_allw = num($('#oth_allowance1').val()) + num($('#oth_allowance2').val())
                 + num($('#oth_allowance3').val()) + num($('#oth_allowance4').val());

    var oth_ded = num($('#oth_deduction1').val()) + num($('#oth_deduction2').val())
                + num($('#oth_deduction3').val()) + num($('#oth_deduction4').val());

    $('#othr_allowance').val(oth_allw.toFixed(3));
    $('#deductions').val((deduction + oth_ded).toFixed(3));

    // Final Net Total = base components + OT AMOUNTS + other allowances - deductions
    var netTotal = basic_net + hra_net + allowance_net + ot_amt_g + ot_amt_h + oth_allw - (deduction + oth_ded);
...
$(function() {	
	...
	$(document).ready(function () {
    // Recompute all lines once to normalize hidden fields, then totals
