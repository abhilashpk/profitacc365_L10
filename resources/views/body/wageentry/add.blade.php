                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmWageEntry" id="frmWageEntry" action="{{ url('wage_entry/save') }}">
								...
								<table border="0" class="table-dy-row">
								    <tr>
								        ...
								        <td></td>
								        <td></td>
								         <td ><input type="hidden" id="total_wdays" step="any" name="total_wdays" class="form-control" readonly placeholder="0"></td>
								        <td width="8%">
						                 <span class="small">OTHr Total(G)</span> 	<input type="number" id="otgs_tot" step="any" name="otgs_tot" class="form-control" readonly placeholder="0">
					                     </td>
								        <td width="8%">
						                  <span class="small">OTHr Total(H)</span> 	<input type="number" id="oths_tot" step="any" name="oths_tot" class="form-control" readonly placeholder="0">
					                    </td>
								        <td></td>
								        <td></td>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<input type="hidden" id="bas_net" step="any" name="bas_net" class="form-control" >
										</div>
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<input type="hidden" id="hraa_net" step="any" name="hraa_net" class="form-control" >
										</div>
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<input type="hidden" id="allow_net" step="any" name="allow_net" class="form-control" >
										</div>
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-4">
										<input type="number" id="otg_net" step="any" name="otg_net" class="form-control" value="{{$parameter->ot_general}}" readonly>
										</div>
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-4">
										<input type="number" id="oth_net" step="any" name="oth_net" class="form-control" value="{{$parameter->ot_holiday}}" readonly>
										</div>
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-4">
										<input type="number" id="othr_allowance" step="any" name="othr_allowance" class="form-control" readonly placeholder="0">
										</div>
									</div>
                                </div>
...
function getLineTotal(n) { 
	...
	var lineTotal = nor_wage + otg_tot + oth_tot - uplevwg;
	//console.log('alw'+aln);
	//$('#nwh_'+n).val(nwh);
...
function getNetTotalDefault() {  
	...
	$('#total_wdays').val(wdays_net.toFixed(3));
	
	$('#basic_net').val(basicpay) //$('#basic_net').val(Math.round(wage_total));
...
/*	var alwtrans_total = 0;
	$( '.alwtrans' ).each(function() { 
		alwtrans_total = alwtrans_total + parseFloat( (this.value=='')?0:this.value );
	});
	
	var alwhra_total = 0;
	$( '.alwhra' ).each(function() { 
		alwhra_total = alwhra_total + parseFloat( (this.value=='')?0:this.value );
	});*/
	...
	//$('#hra_net').val(alwhra_total.toFixed(3));
	//$('#allowance_net').val(alwtrans_total.toFixed(3));
...
//	$('#basic_net').val(basic_net.toFixed(3));
	...
	var netTotal = basic_net + otg_net + oth_net + hra_net + allowance_net + oth_allw - deduction - oth_ded;
	$('#net_total').val(netTotal.toFixed(1));
...
$(function() {	
	...
	var et = $('#entry_type option:selected').val();
	   ...
	   $.get("{{ url('employee/get_empdata/') }}/" + $(this).attr("data-id"), function(data) { console.log(data);
		   ...
		   if(et=='monthly')
			 ...
			 $('#bas_net').val(data.basic);
             $('#otg_net').val(data.otg); 
			 $('#oth_net').val(data.oth); 
		   $('#hra_net').val(data.hra);
		   $('#hraa_net').val(data.hra);
	...
	$(document).on('change', '.leave', function(e) {
		...
		if(leave==1) { //status Off
			...
			$('#wage_'+curNum).val(0);
			$('#alw_'+curNum).val(0);
			$('#alwtrans_'+curNum).val(0);
			$('#alwhra_'+curNum).val(0);
			$('#nwh_'+curNum).val(0);
			if( $('#infodivLevItm_'+curNum).is(":visible") )
				$('#infodivLevItm_'+curNum).toggle();
			...
			$('#alw_'+curNum).val(al);
			$('#alwtrans_'+curNum).val(altrans);
			$('#alwhra_'+curNum).val(alhra);
