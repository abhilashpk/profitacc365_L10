								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                       <select id="voucher" class="form-control select2" style="width:100%" name="voucher">
									    ...
									    <input type="hidden" name="voucher_id" id="voucher_id" value="{{isset($vouchers[0])?$vouchers[0]['id']:''}}">
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-9">
										<div class="input-group">
											...
											@else
											<span class="input-group-addon">{{$vchrdata['prefix']}}<input type="hidden" id="hdn_voucher_no" placeholder="{{$vchrdata['voucher_no']}}" name="voucher_no" ></span>
  ...
  $(':input[type=number]').on('mousewheel',function(e){ $(this).blur(); });
   ...
   $('#department_id').on('change', function(e){ 
	...
	$(document).on('click', '.custRow', function(e) {
	   ...
	   $('#crTransactionData').load(url, function(result) {
		  $('#myModal').modal({show:true});  $('.bl-dtls').hide(); $('.ob-dtls').hide();
	   });
	...
	$(document).on('keyup', '#voucher_no', function(e) {
		//$('#hdn_voucher_no').val('{{$vchrdata['prefix']}}'+$('#voucher_no').val()); 
		$('#hdn_voucher_no').val('{{$vchrdata['prefix']}}'+$('#voucher_no').attr("placeholder", data.voucher_no)); 
	});
