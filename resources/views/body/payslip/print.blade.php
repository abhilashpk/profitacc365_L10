...
<style>
		...
		?>
					...
					<table width="100%" border="1">
						<thead>
							...
							?>
								...
								<td class="text-right" style="height:20px;padding:8px;"><strong>{{number_format($basicpay,2)}} &nbsp; </strong></td>
			...
			<?php } else { echo '<div class="warning"><p><br/><br/> No records were found!</p></div>'; }?>
	...
	<b>Month: <?php echo date('F', mktime(0, 0, 0, $wages['month'], 10));?></b>
		<table border="1" width="100%" class="pay">
			...
			<tr style="height:50px;">
				...
				<td>{{number_format($wages['net_allw'],2)}}</td>
				<td>--</td>
				<td>--</td>
				<td>0</td>
	...
	<b>Job Code: {{$result[0]->job_code}} Job Name: {{$result[0]->job_name}}</b><br/><br/>
		<table border="1" width="100%" class="pay">
			...
			foreach($result as $result) {
				...
				if($result->job_type==1 || $result->job_type==21)	
			...
			<tr style="height:25px;">
				...
				<td>{{$result->code}}</td>
				<td>{{($result->job_type==0)?$result->hour:0}}</td>
				<td>{{($result->job_type==1 || $result->job_type==2)?$result->hour:0}}</td>
