                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmWageEntry" id="frmWageEntry" action="{{ url('wage_entry/save') }}">
								...
								<div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label">Entry Type</label>
                                    <div class="col-sm-10">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="number" class="form-control" id="employee_no" autocomplete="off" name="employee_no" placeholder="Employee No.">
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										...
										<div class="col-xs-2"></div>
										<div class="col-xs-4">
										<input type="text" id="basic_net" step="any" name="basic_net" class="form-control" readonly placeholder="0">
										</div>
									</div>
                                </div>
...
function getNetTotal() {  
	...
	wage_total = ($('#itmttl_1').val()=='')?0: parseFloat($('#itmttl_1').val().replace(/,/g, '')); //console.log(wage_total);
	
	alw_total = ($('#alw_1').val()=='')?0: parseFloat($('#alw_1').val().replace(/,/g, ''));
	
	otg_net = parseFloat( ($('#otgttl_1').val()=='')?0:$('#otgttl_1').val());
	
	oth_net = parseFloat( ($('#othttl_1').val()=='')?0:$('#othttl_1').val());
	
	var dwage = wg * nwhr; 
	var ded = dwage * parseFloat(($('#levd_1').val()=='')?0:$('#levd_1').val()); //console.log('ded'+ded);
		
	//var ded = (parseFloat($('#levd_1').val()) * $('#wage_1').val());
	
	$('#otg_net').val(otg_net.toFixed(3));
	$('#oth_net').val(oth_net.toFixed(3));
	
	$('#basic_net').val(basicpay);
	
	var hra_net = parseFloat( ($('#hra_net').val()=='') ? 0 : $('#hra_net').val() );
	
	var allowance_net = parseFloat( ($('#allowance_net').val()=='') ? 0 : $('#allowance_net').val() );
	
	//var deduction = parseFloat( ($('#deductions').val()=='') ? 0 : $('#deductions').val() );
	deduction = parseFloat(ded);
	...
	var netTotal = basicpay + otg_net + oth_net + hra_net + allowance_net + oth_allw - deduction - oth_ded;
...
$(function() {	
	...
	$(document).on('blur', '#levd_1', function(e) {
		getNetTotal();
	});
