            ...
            <div class="row">
                <div class="col-md-12">
					...
					<div class="alert alert-warning">
						<p>
							SSR. No. is found empty! Please create in Account Settings.
						</p>
					</div>
					<?php } else { ?>
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
							...
							<?php if($printid) { ?>
								 ...
								 <a href="{{ url('sales_split_return/print/'.$printid->id.'/'.$print->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
							<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmSalesSplitReturn" id="frmSalesSplitReturn" enctype="multipart/form-data" action="{{ url('sales_split_return/save') }}">
                                ...
                                <div class="form-group">
                                    <font color="#16A085"><label for="input-text" class="col-sm-2 control-label"> <b>Sales Split No</b></label></font>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control " id="sales_split_id" name="sales_split_id" value="" placeholder="Sales Split No" autocomplete="off" onclick="getSalesSplit(this)">
...
$(document).ready(function () { 
	...
	var urlvchr = "{{ url('sales_split_return/checkvchrno/') }}";
	var urlcode = "{{ url('sales_split_return/checkrefno/') }}"; //CHNG
	$('.sedePrntItm').toggle();
	<?php if($formdata['item_import']==0) { ?>
    $('#frmSalesSplitReturn').bootstrapValidator({
        fields: {
			...
			voucher_no: {
                validators: {
					...
					remote: {
                        ...
                        message: 'This SSR. No. is already exist!'
                    }
                }
            },
			reference_no: {
        ...
        $('#frmSalesSplitReturn').data('bootstrapValidator').resetForm();
    });
	<?php } ?>
...
function getSalesSplit(e) {
	if(e.value=='') { 
		...
		if($('#department_id').length) {
			...
			var pourl = "{{ url('sales_split/split_data/') }}/"+did;
		} else
			var pourl = "{{ url('sales_split/split_data/') }}";
...
$(function() {	
	...
	$(document).on('blur', '.line-quantity', function(e) {
		...
		if(res)
			getNetTotal();
		$('#frmSalesSplitReturn').bootstrapValidator('revalidateField', 'quantity[]');
	});
