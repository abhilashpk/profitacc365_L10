                        ...
                        <div class="panel-body">
                            <form class="form-horizontal" role="form" method="POST" name="frmContract" id="frmContract" action="{{ url('contract/save') }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="contract_no" name="contract_no" value="{{$contract->contract_no}}" readonly>
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										<input type="text" class="form-control pull-right" name="date" autocomplete="off" readonly id="date" value="{{date('d-m-Y',strtotime($contract->contract_date))}}"/>
                                    </div>
                                </div>
								...
								<div class="form-group">
                                 <label for="input-text" class="col-sm-2 control-label"><b>Customer</b></label>
                                    <div class="col-sm-10">
										<input type="text" name="customer_name" id="customer_name" value="{{$contract->master_name}}" class="form-control" autocomplete="off" data-toggle="modal" data-target="#customer_modal" placeholder="Customer" readonly>
										<input type="hidden" name="customer_id" id="customer_id" value="{{$contract->customer_id}}">
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select id="multiselect5" multiple="multiple" class="form-control" name="type_id[]" style="width:100%">
										<?php foreach($types as $row) { ?>
										<option value="{{$row->id}}" {{(in_array($row->id,unserialize($contract->contract_type_id)))?'selected':''}}>{{$row->name}}</option>
										<?php } ?>
										</select>
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										<input type="text" class="form-control pull-right" name="start_date" value="{{date('d-m-Y',strtotime($contract->start_date))}}" autocomplete="off" data-language='en' readonly id="start_date" />
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<option value="182" {{($contract->duration==182)?'selected':''}}>6 Months</option>
										<option value="365" {{($contract->duration==365)?'selected':''}}>12 Months</option>
										<option value="730" {{($contract->duration==730)?'selected':''}}>24 Months</option>
										<option value="1095" {{($contract->duration==1095)?'selected':''}}>36 Months</option>
										<option value="1460" {{($contract->duration==1460)?'selected':''}}>48 Months</option>
										<option value="1825" {{($contract->duration==1825)?'selected':''}}>60 Months</option>
										</select>
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										<input type="text" class="form-control pull-right" name="end_date" autocomplete="off" value="{{date('d-m-Y',strtotime($contract->end_date))}}" readonly id="end_date" />
                                    </div>
                                </div>
								...
								<div class="form-group">
                                 <label for="input-text" class="col-sm-2 control-label">Machine</label>
                                    <div class="col-sm-10">
										 ...
										 <option value="{{$mrow->id}}" {{($mrow->id==$contract->machine_id)?'selected':''}}>{{$mrow->brand.'  -  '.$mrow->name.'  ('.$mrow->model.')'}}</option>
										 @endforeach
										 </select>
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select id="select22" class="form-control select2" name="paper_id[]" multiple style="width:100%">
											...
											<option value="{{$row->id}}" {{(in_array($row->id,unserialize($contract->paper_id)))?'selected':''}}>{{$row->name}}</option>
											@endforeach
										</select>
                                    </div>
                                </div>
