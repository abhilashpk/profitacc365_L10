                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            <i class="fa fa-fw fa-crosshairs"></i> Edit Driver
                        </h3>

                    </div>
                    <div class="panel-body">
                        <form class="form-horizontal" role="form" method="POST" name="frmRentalVehicle"
                            id="frmRentalVehicle" action="{{ url('rental_driver/update/' . $rvedit->id) }}">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input type="hidden" name="vehicle_id" value="<?php echo $rvedit->id; ?>">
                           ...
                           <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" autocomplete="off" id="code" name="code"
                                        placeholder="Driver Code" value="{{ $rvedit->code }}" required>
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="input-text" class="col-sm-2 control-label">Diver Name</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="dname" name="dname"
                                        placeholder="Driver Name" value="{{ $rvedit->driver_name }}">
                                </div>
                            </div>
                             <div class="form-group">
                                <label for="input-text" class="col-sm-2 control-label">Mobile NO:1</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="mb1" name="mb1"
                                        placeholder="Enter Mobile No:" value="{{ $rvedit->mobile1 }}">
                                </div>
                            </div>
                             <div class="form-group">
                                <label for="input-text" class="col-sm-2 control-label">Mobile No: 2</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="mb2" name="mb2"
                                        placeholder="Enter Mobile No:" value="{{ $rvedit->mobile2 }}">
                                </div>
                            </div>
                             <div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label">Driver Type</label>
                                    <div class="col-sm-10">
                                     <select id="driver_type" class="form-control select" style="width:100%" name="driver_type">
								    <option value="supplier" {{($rvedit->driver_type=='supplier')?"selected":""}}>SUPPLIER</option>
									<option value="customer" {{($rvedit->driver_type=='customer')?"selected":""}}>CUSTOMER</option>
								</select>
                                     </div>
                            </div>
							...
							<div class="form-group" id="supTxt">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" autocomplete="off" readonly class="form-control" value="{{($rvedit->driver_type=='supplier')?$rvedit->master_name:''}}" id="supplier_name" name="supplier_name" placeholder="Supplier" data-toggle="modal" data-target="#supplier_modal">
									<input type="hidden" name="supplier_id" id="supplier_id" value="{{($rvedit->driver_type=='supplier')?$rvedit->account_id:''}}">
                                </div>
                            </div>
							...
							<div class="form-group" id="custTxt">
                                ...
                                <div class="col-sm-10">
                                   <input type="text" name="customer_name" id="customer_name" value="{{($rvedit->driver_type=='customer')?$rvedit->master_name:''}}" class="form-control" autocomplete="off" data-toggle="modal" data-target="#customer_modal" placeholder="Customer" readonly>
								   <input type="hidden" name="customer_id" id="customer_id" value="{{($rvedit->driver_type=='customer')?$rvedit->account_id:''}}">
								</div>
                            </div>
...
@section('footer_scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
			@if($rvedit->driver_type=='supplier')
				$('#custTxt').hide();
				$('#supTxt').show();
			@else
				$('#custTxt').show();
                        ...
                        remote: {
                                ...
                                data: function(validator) {
                                    return {
                                        ...
                                        id:validator.getFieldElements('vehicle_id').val(),
                                    };
                                },
