                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                                <i class="fa fa-fw fa-crosshairs"></i> Edit Credit Note Journal 
                            </h3>
                           
						   <div class="pull-right">
								@can('rv-print')
								 <a href="{{ url('creditnotejournal/print/'.$jrow->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
							<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmJournal" id="frmJournal" action="{{ url('creditnotejournal/update/'.$jrow->id) }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select id="voucher_type" class="form-control select2" style="width:100%" name="voucher_type">
											<option value="7" <?php if($jrow->voucher_type=='CNJV') echo 'selected'; ?>>CNJV - Credit Note Journal Voucher</option>
										</select>
                                    </div>
                                </div>
										...
										{{--*/ $i++; /*--}}
											<div class="itemdivChld">							
												...
												@if($item->category=='PDCR' || $item->category=='PDCI')
													@if($isdept)
													<div class="col-sm-0.5 " style="width:1%;">
	                                                <input type="checkbox" id="tag_{{$i}}" name="tag[]" class="form-control tagchk" value="{{$i}}" {{$item->is_print==1?"checked":""}} >
	                                                </div>
													...
													@else <!--PDC ELSE (CASH)-->
														...
														<div class="col-sm-0.2 " style="width:1%;">
	                                                    <input type="checkbox" id="tag_{{$i}}" name="tag[]" class="form-control tagchk" value="{{$i}}" {{$item->is_print==1?"checked":""}} >
	                                                    </div>
															...
															@else <!-- CASH DEPARTMENT ELSE -->
																...
																<input type="hidden" name="bill_type[]" id="biltyp_{{$i}}" value="">
																	...
																	<div class="col-xs-1 abc" style="width:3%;"><br/>
																		 ...
																		 <?php if($jrow->group_id!=1) { ?> 
																			<button type="button" data-id="add_{{$i}}" class="btn-success btn-add-item" >
																				<i class="fa fa-fw fa-plus-square"></i>
																			</button>
																		<?php } ?>
																	</div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        ...
                                        <a href="{{ url('creditnotejournal') }}" class="btn btn-danger">Cancel</a>
										<a href="{{ url('creditnotejournal') }}" class="btn btn-warning">Clear</a>
                                    </div>
                                </div>
                            </form>
...
$(document).ready(function () {
	...
	$('#voucher_type').on('change', function(e){
		...
		$.get("{{ url('creditnotejournal/getvouchertype/') }}/" + vchr_id, function(data) { 
			$('#voucher').empty();
	...
	$('#voucher').on('change', function(e){
		var vchr_id = e.target.value; 
		$.get("{{ url('creditnotejournal/getvoucher/') }}/" + vchr_id, function(data) { //console.log(data);
			$('#voucher_no').val(data);
		});
...
$(function(){
  ...
  $(document).on('click', '.btn-add-item', function(e)  { 
        ...
        var controlForm = $('.controls .itemdivPrnt'),
			...
			newEntry.find($('input[name="tag[]"]')).attr('id', 'tag_' + rowNum);
			...
			newEntry.find($('.tagchk:checkbox')).attr("checked", false);
			//newEntry.find($(document.getElementById('.tagchk:checkbox'))).checked= false;
			$('.chqdate').datepicker({
				language: 'en',
	...
	$(document).on('click', '.accountRow', function(e) { 
		...
		if($(this).attr("data-group")=='PDCR') { console.log('hi');
			$('#acnttype_'+num).find('option').remove().end().append('<option value="Dr">Dr</option>');
			var trnurl = "{{ url('creditnotejournal/set_transactions/') }}";
...
function tagVal(){
  var id=[];	
	$('.tagchk:checkbox').each(function() { 
		...
		//console.log(curNum);
		//$('#tag_'+curNum).val(1); 
		var dr=	$('#draccountid_'+curNum).val();
	id.push(dr);
	}	
	}); 
