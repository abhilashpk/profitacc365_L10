                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
							...
							<?php if($printid) { ?>
								@can('so-print')
								 <a href="{{ url('job_order/print/'.$printid->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmSalesOrder" id="frmSalesOrder" action="{{ url('job_order/save') }}">
								...
								<div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label <?php if($errors->has('voucher_no')) echo 'form-error';?>">Job Order No.</label>
                                    <div class="col-sm-10">
								...
								<div class="form-group" id="vclData">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="vehicle_name" id="vehicle_name" value="{{old('vehicle_name')}}" class="form-control <?php if($errors->has('vehicle_name')) echo 'form-error';?>" autocomplete="off" data-toggle="modal" data-target="#vehicle_modal" placeholder="Vehicle">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select id="job_id" class="form-control select2" style="width:100%" name="job_id">
                                            <option value="">Job - {{$voucherno->prefix.$voucherno->no}}</option>
											@foreach($jobs as $job)
									...
									<div class="itemdivChld">
										<table border="0" class="table-dy-row">
											<tr>
												...
												<td width="1%">
													<br/><?php if(count(old('lbitem_code'))==$j) { ?>
...
$(function() {	
	...
	$(document).on('click', '#item_data .itemRow', function(e) { 
		...
		if($('.export').is(":checked") ) { 
			$('#vatdiv_'+num).val( '0%' );
			$('#vat_'+num).val( 0 );
			$('#itmcst_'+num).val( $(this).attr("data-cost") );//AG24 //$('#itmcst_'+num).val( $(this).attr("data-price") );
		} else {
			$('#vatdiv_'+num).val( $(this).attr("data-vat")+'%' );
			$('#vat_'+num).val( $(this).attr("data-vat") );
			$('#itmcst_'+num).val( $(this).attr("data-cost") );//$('#itmcst_'+num).val( $(this).attr("data-price") );
			srvat = $(this).attr("data-vat");
		}
		
		var itm_id = $(this).attr("data-id")
		$.get("{{ url('purchase_order/getunit/') }}/" + itm_id, function(data) { //console.log(data);
	...
	$(document).on('keyup', '#voucher_no', function(e) { 
		...
		$('#job_id option:selected').text('Job - {{$voucherno->prefix}}'+this.value);
