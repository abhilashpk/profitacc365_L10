                ...
                <div class="col-md-12">
                    <div class="panel panel-<?php echo ($orderrow->is_editable==1)?'danger':'primary';?>">
                        <div class="panel-heading clearfix">
							...
							<?php if($isprint) { ?>
							@can('so-print')
							 <a href="{{ url('proforma_invoice/print/'.$orderrow->id.'/'.$print->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
							<div class="controls"> 
							<form class="form-horizontal" role="form" method="POST" name="frmSalesOrder" id="frmSalesOrder" action="{{ url('proforma_invoice/update/'.$orderrow->id) }}">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
								<input type="hidden" name="proforma_invoice_id" id="proforma_invoice_id" value="{{ $orderrow->id }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="less_description" autocomplete="off" value="<?php echo (old('less_description'))?old('less_description'):$orderrow->less_description; ?>" name="less_description" placeholder="Subject">
                                    </div>
                                </div>
								...
								<?php if(old('item_code')) { $i = 0; 
									...
									<div class="itemdivChld portlet box">							
										<table border="0" class="table-dy-row">
											<tr>
												<td width="16%">
												    <input type="hidden" name="orderno[]" id="ordr_{{$j}}" readonly class="form-control" value="{{$item->orderno}}">
								...
								<?php if($orderrow->is_fc==1) {
									...
									<div class="itemdivChld portlet box">							
										<table border="0" class="table-dy-row">
											<tr>
												<td width="16%">
												    <input type="hidden" name="orderno[]" id="ordr_{{$i}}" readonly class="form-control" value="{{$item->orderno}}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<a href="{{ url('proforma_invoice/edit/'.$orderrow->id) }}" class="btn btn-warning">Clear</a>
                                    </div>
                                </div>
					...
					/*remote: {
                        ...
                        data: function(validator) {
                            return {
								...
								id: validator.getFieldElements('proforma_invoice_id').val()
                            };
                        },
