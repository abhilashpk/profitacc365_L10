                    ...
                    <div class="panel panel-success filterable">
                        <div class="panel-heading">
                            <h3 class="panel-title">
                                <i class="fa fa-fw fa-columns"></i> Proforma Invoice
                            </h3>
                        </div>
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-xs-6">
                                   <!-- <span>SO. No:</span>
                                    <input type="text" name="name" placeholder="SO. No..." class="form-control input-sm">-->
                                ...
                                <table class="table horizontal_table table-striped" id="tablePorders">
                                    ...
                                    <tbody>
										...
										<tr>
											...
											<td><a href="#" class="quot-data" data-id="{{$order->id}}" data-name="{{$order->voucher_no}}" data-toggle="modal" data-target="#quot_modal">View Items</a></td>
										</tr>
										@endforeach
                                    </tbody>
                                </table>
...
function setTransfer() {
	...
	//alert(items);
	if(curl=='SO')
		url = "{{ url('production/add/') }}/"+items+"/SO"; //url = "{{ url('customers_do/add/') }}/"+items+"/SO";
	else if(curl=='SOD')
		url = "{{ url('customers_do/add/') }}/"+items+"/SO";
	else if(curl=='SOI')
		url = "{{ url('sales_invoice/add/') }}/"+items+"/SO";
     else if(curl=='SRL')
		url = "{{ url('sales_rental/add/') }}/"+items+"/SO";
...
$(document).on('click', '.quot-data', function(e) {
	...
	var itmurl = "{{ url('proforma_invoice/item_details/') }}";
