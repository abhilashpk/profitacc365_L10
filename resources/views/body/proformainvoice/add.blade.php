                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
							...
							<?php if($printid) { ?>
								@can('so-print')
								 <a href="{{ url('proforma_invoice/print/'.$printid.'/'.$print->id) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmSalesOrder" id="frmSalesOrder" action="{{ url('proforma_invoice/save') }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control <?php if($errors->has('reference_no')) echo 'form-error';?>" id="reference_no" autocomplete="off" value="{{ old('reference_no') }}" name="reference_no" placeholder="LPO Reference No">
                                    </div>
                                </div>
								<?php } else { ?>
								...
								<div class="form-group parnt-job">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control select2" name="parent_job" id="select22" style="width:100%" />
											@foreach($prntjobs as $pjob)
											<option value="{{$pjob->id}}">{{$pjob->voucher_no}}</option>
											@endforeach
                                        </select>
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="description" autocomplete="on" value="{{ old('description') }}" name="description" placeholder="Description">
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="less_description" autocomplete="on" value="{{ old('less_description') }}" name="less_description" placeholder="Subject">
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label"> Select PO</label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="po_doc" readonly name="po_doc" placeholder="PO No" autocomplete="off" onclick="getPO()">
                                    </div>
                                </div>
								<?php } else { ?>
								...
								<?php if(old('item_code')) { $i = 0; 
									...
									<div class="itemdivChld portlet box">							
										<table border="0" class="table-dy-row">
											<tr>
												<td width="16%">
												     <input type="hidden" name="orderno[]" id="ordr_{{$j}}" readonly class="form-control ord-no" value="1">
											...
											<tr>
												<td width="16%">
													<!--<span class="small itmHd">Item Code</span>-->
													 <input type="hidden" name="orderno[]" id="ordr_1" readonly class="form-control ord-no" value="1">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										<div class="col-xs-8">
											<?php if($formdata['vat_description']==1) { ?><input type="text" step="any" class="form-control" id="vatdesc" name="vatdesc" value="5% VAT Extra" readonly><input type="hidden" id="vatd" name="vatfrm_subtotal" value="5"/><?php } ?>
										</div>
										<div class="col-xs-2">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <button type="submit" value="submit" name="btn" class="btn btn-primary">Submit</button>
										<a href="{{ url('proforma_invoice') }}" class="btn btn-danger">Cancel</a>
										<a href="{{ url('proforma_invoice/add') }}" class="btn btn-warning">Clear</a>
										@can('so-history')
...
$(document).ready(function () { 
	...
	var urlvchr = "{{ url('proforma_invoice/checkvchrno/') }}"; //CHNG
...
$(function() {	
	...
	$("#select22").select2({
		...
		ajax: { 
         url: "{{ url('proforma_invoice/get_orderno/') }}",
         type: "post",
         dataType: 'json',
         delay: 250,
	...
	$(document).on('change', '#select22', function(e) {  
		$.get("{{ url('proforma_invoice/getcounter/') }}/" + $(this).val(), function(data) {
			data = $.parseJSON(data);
...
function getQuotation() { 
	...
	var wt = $(window).width();
	var customer_id = $("#customer_id").val();
	var sourl = "{{ url('quotation_sales/get_quotation/') }}/"+customer_id+"/so";
