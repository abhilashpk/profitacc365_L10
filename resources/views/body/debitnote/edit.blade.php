                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            <h3 class="panel-title">
                                <i class="fa fa-fw fa-crosshairs"></i> Edit Debit Note 
                            </h3>
                           
                        </div>
                        <div class="panel-body">
						<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmDebitNote" id="frmDebitNote" action="{{ url('debit_note/update/'.$cnrow->id) }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<input type="hidden" name="cr_account_id" id="cr_account_id" value="{{ $cnrow->cr_account_id }}" class="form-control">
										...
										?>
											<div class="itemdivChld">							
												<div class="form-group">
													...
													<input type="hidden" name="dr_account_id[]" value="{{$item->dr_account_id}}" id="draccountid_{{$i}}"><!-- acntid_1 -->
													...
													<div class="col-xs-15">
														<div class="col-xs-3">
															<span class="small">Description</span> <input type="text" id="descr_{{$i}}" value="{{$item->dr_description}}" name="dr_description[]" class="form-control">
														</div>
														<div class="col-xs-2" style="width:10%;">
															<span class="small">Reference</span> 
															<div id="refdata_1" class="refdata">
															<input type="text" id="ref_{{$i}}" name="dr_reference[]" value="{{$item->dr_reference}}" class="form-control ref-invoice" data-toggle="modal" data-target="#reference_modal">
															</div>
														...
														<div class="col-xs-2" style="width:15%;">
															<span class="small">Amount</span> <input type="number" id="amount_{{$i}}" value="{{$item->dr_amount}}" step="any" name="dr_amount[]" class="form-control line-amount">
														</div>
														<div class="col-sm-2"> 
...
$(document).ready(function () {
    ...
    $('#frmDebitNote').bootstrapValidator({
                ...
                validators: {
					callback: {
                            ...
                            callback: function(value, validator, $field) {
                                var voucher_type = $('#frmDebitNote').find('[name="voucher_type"]').val();
        ...
        $('#frmDebitNote').formValidation('revalidateField', 'cheque_no');
		$('#frmDebitNote').formValidation('revalidateField', 'cheque_date');
		$('#frmDebitNote').formValidation('revalidateField', 'bank_id');
	...
	}).on('click', '#frmDebitNote', function(e) { 
		$('#frmDebitNote').bootstrapValidator('revalidateField', 'debit');
		$('#frmDebitNote').bootstrapValidator('revalidateField', 'credit');
	});
...
$(function(){
  ...
  var custurl = "{{ url('sales_invoice/customer_data/') }}";
	...
	$('#voucher').on('change', function(e){
		...
		$.get("{{ url('debit_note/getvoucher/') }}/" + vchr_id+'/'+vchr_type, function(data) {
	...
	$('#voucher_type').on('change', function(e){
		...
		$.get("{{ url('debit_note/getvoucher/') }}/" + vchr_id+'/'+vchr, function(data) {
			$('#voucher_no').val(data.voucher_no);
			if(data.id!=null && data.account_name!=null) {
				$('#dr_account').val(data.account_name);
				$('#dr_account_id').val(data.id);
