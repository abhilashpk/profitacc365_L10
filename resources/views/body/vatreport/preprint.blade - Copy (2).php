							...
							<?php } else if($type=='detail') { ?>	
									<table class="table" border="0">
										...
										@foreach($reports as $key => $report) 
											...
											@php $i = $dramount = $cramount = $vatamount = 0; @endphp
											@foreach($report as $row)
												...
												<tr> <td>{{$i}}</td>
													...
													@endphp
													<td class="emptyrow text-right">{{number_format($gross_total,2)}}</td>
													<td class="emptyrow text-right">{{number_format($row->discount,2)}}</td>
													<td class="emptyrow text-right">{{number_format(($row->gross_total - $row->discount),2)}}</td>
													<td class="emptyrow text-right">{{($row->trtype=='Dr')?$row->vat_amount:'('.$row->vat_amount.')'}}</td>
											...
											<tr><td colspan="9" class="text-right"><strong>{{$vatname}} Total:</strong></td>
