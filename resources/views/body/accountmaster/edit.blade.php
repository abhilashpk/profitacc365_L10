                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmMaster" id="frmMaster" action="{{ url('account_master/update/'.$masterrow->id) }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="account_no" name="account_no" placeholder="Account No" value="{{ $masterrow->ac_no }}">
                                    </div>
                                </div>
									...
									<div class="itemdivChld">
											<div>
												<div class="col-xs-12">
													...
													<div class="form-group col-xs-1" style="width:9% !important;">
													...
													<div class="form-group col-xs-1" style="width:7% !important;">
														...
														<select id="occrncy_1" class="form-control select2 curr" name="currency[]">
															...
															<option value="{{$curr->id}}" {{($settings->bcurrency_id==$curr->id)?'selected':''}}>{{$curr->code}}</option>
															@endforeach
														</select>
													</div>
													
													<div class="form-group col-xs-1" style="width:10% !important;">
													...
													<div class="form-group col-xs-1" style="width:5% !important;">
													...
													<div class="form-group col-xs-1" style="width:8% !important;">
													...
													<div class="col-xs-1" style="width:4% !important;"><span class="small">&nbsp; </span><br/>
									...
									<div class="itemdivChld">							
										<div>
											<div class="col-xs-12">
												...
												<div class="form-group col-xs-2" style="width:8% !important;">
													<span class="small">Ref. No</span> <input type="text" id="refno_{{$i}}" name="reference_no[]" autocomplete="off" value="{{$row->reference_no}}" class="form-control ref-no">
												</div>
												<div class="form-group col-xs-2" style="width:14% !important;">
												...
												<div class="form-group col-xs-1" style="width:10% !important;">
													<span class="small lbamt">Amount</span> <input type="number" id="amount_{{$i}}" step="any" name="amount[]" autocomplete="off" value="{{$row->fc_amount}}" class="form-control line-amount">
												</div>
												
												<div class="form-group col-xs-1" style="width:5% !important;">
													<span class="small ">Rate</span> <input type="number" id="rate_{{$i}}" step="any" autocomplete="off" name="rate[]" class="form-control rate" value="{{($row->rate==0)?1:$row->rate}}" >
												</div>
												
												<div class="form-group col-xs-1" style="width:9% !important;">
													<span class="small lbamtfc">FC Amt</span> <input type="number" id="cnvtamt_{{$i}}" step="any" autocomplete="off" name="cnvt_amt[]" value="{{$row->amount}}" class="form-control cnvt-amt">
												</div>

												<div class="form-group col-xs-1" style="width:10% !important;">
													<span class="small">Salesman</span> 
													<select id="sman_{{$i}}" class="form-control select2 slmn" name="slsman[]">
														<option value="">Select</option>
														@foreach($salesman as $sm)
														<option value="{{$sm['id']}}" <?php if($row->salesman_id==$sm['id']) echo 'selected';?> >{{$sm['name']}}</option>
														@endforeach
													</select>
												</div>
												<div class="form-group col-xs-1" style="width:8% !important;">
													<span class="small">Job No</span> <input type="text" id="jbno_{{$i}}" autocomplete="off" value="{{$row->jobno}}" name="jobno[]" class="form-control">
												</div>
												<div class="form-group col-xs-1" style="width:9% !important;">
													<span class="small">Due Date</span> <input type="text" id="duedate_{{$i}}" value="{{($row->duedate!=null)?date('d-m-Y',strtotime($row->duedate)):''}}" autocomplete="off" name="duedate[]" class="form-control duedate">
												</div>
													
												<div class="col-xs-1" style="width:4% !important;"><br/>
									...
									<div class="itemdivPrntch">
										...
										<?php if($num==0) { ?>
											<div class="itemdivChldch">							
												<div class="col-xs-12">
													<input type="hidden" name="tr_id[]" id="trid_{{$i}}">
													<div class="form-group col-sm-2" style="width:13%"> 
														<span class="small">Amount</span> <input type="number" autocomplete="off" id="amountch_{{$i}}" step="any" name="amount[]" autocomplete="off" class="form-control chline-amount">
													</div>
													...
													<div class="form-group col-sm-2" style="width:10%">
														<span class="small">Cheque No.</span><input type="text" id="chqno_{{$i}}" name="cheque_no[]" class="form-control"  autocomplete="off">
													</div>
													<div class="form-group col-sm-2" style="width:13%">
														<span class="small">Cheque Date</span> <input type="text" id="chqdate_{{$i}}" autocomplete="off" name="cheque_date[]" class="form-control chkdate" data-language='en'>
													</div>
													...
													<div class="form-group col-sm-1" style="width:7%"> 
														...
														<select id="trtypech_{{$i}}" class="form-control select2 linech-tr" style="width:100%" name="tr_type[]">
														    ...
														    <option value="Dr">Dr</option>
														    @elseif($masterrow->category == 'PDCI')
														    <option value="Cr">Cr</option>
														    @endif
														</select>
													</div>
													...
													<div class="form-group col-sm-2" style="width:13%;display:none;">
														<span class="small">Tr. Date</span> <input type="hidden" name="tr_date[]">
													</div>
													<div class="form-group col-sm-2" style="width:15%">
														<span class="small">Description</span> <input type="text" id="description_{{$i}}" name="description[]" autocomplete="off" class="form-control"> 
													</div>
													<div class="form-group col-sm-1" style="width:3%"><br/>
...
$(document).ready(function () {
	...
	<?php if($masterrow->currency_id != 0) { ?>
	$('.lbamt').html('FC Amt');
	$('.lbamtfc').html('Amount');
	<?php } ?>
...
$(document).on('blur', 'input[name="cheque_no[]"]', function(e) {
    ...
    $('#frmMaster').bootstrapValidator('revalidateField', 'cheque_no[]');
});
...
$(function() {
	...
	/*$(document).on('blur', 'input[name="cheque_no[]"]','.line-bank', function(e) {
		e.preventDefault();
		var res = this.id.split('_');
		var no = res[1];
		validateCheque(no)
	...
	$(document).on('click', '.btn-add-item', function(e)  { 
		...
		if(curid==''){
			...
			newEntry.find( $('#occrncy_'+rowNum+' option[value="{{$settings->bcurrency_id}}"]').attr("selected",true) );
	...
	$(document).on('click', '.btn-add-itemch', function(e)  { 
        ...
        var frmTmplate = `<div class="itemdivChldch">
                			...
                			<div class="form-group">
								...
								<div class="col-sm-1" style="width:3%"><br/>
									 <button type="button" class="btn-success btn-danger btn-remove-itemch" data-id="rem_`+rowNum+`">
										<i class="fa fa-fw fa-minus-square"></i>
									</button>
									<button type="button" class="btn-success btn-add-itemch" >
										<i class="fa fa-fw fa-plus-square"></i>
    ...
    { 
		...
		ids = (remitem=='')?$('#trid_'+curNum).val():remitem+','+$('#trid_'+curNum).val();
		$('#remitem').val(ids);
		var amt = parseFloat( ($('#amountch_'+curNum).val()=='') ? 0 :  $('#amountch_'+curNum).val() );
		$(this).parents('.itemdivChldch:first').remove();
		var opbal= parseFloat($('#op_balance').val());
	...
	$(document).on('blur', '.chline-amount', function(e) {
	    ...
	    var opbal= parseFloat($('#op_balance').val());
	    var clbal= parseFloat($('#cl_balance').val());
	    var amt = parseFloat( ($('#amountch_'+no).val()=='') ? 0 :  $('#amountch_'+no).val() );
		var res = getLineTotalChq();
		$('#op_balance').val(res.toFixed(2));
	...
	$(document).on('blur', 'input[name="reference_no[]"]', function(e) {
		...
		for (var i = 0; i < myarray.length; i++) {
			for (var j = i + 1; j < myarray.length; j++) {
				...
				if (myarray[i] == myarray[j]) {
					$('#refno_'+curNum).val(''); $('#refno_'+curNum).focus();
		...
		if(isalert==false) {
			...
			type: 'get',
			data: 'refno='+refno+'&acid='+$('#acid').val(),
			success: function(data) { 
				if(data=='') {
