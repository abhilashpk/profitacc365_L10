                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
									...
									<table class="table" border="0">
										...
										<body><?php $total_bal = $total_trans=$date_trans  = $net_bal = 0; ?>
											@foreach($reports as $report)
											<tr>
												<?php
													$total_bal += $report->cl_balance;
													if($report->op_balance==0){
													$date_trans = $report->cr_balance-$report->dr_balance; 
													}else {
													$date_trans = ($report->cr_balance-$report->dr_balance)+($report->op_balance);
													}
													$total_trans += $date_trans;
													
													if($report->op_balance==0){
														$net_balance = $report->cr_balance-$report->dr_balance;
													}else {
													$net_balance = ($report->cr_balance-$report->dr_balance)+($report->op_balance);
                                                }
													$net_bal += $net_balance;
												?>
												...
												<td class="text-right">
													<?php if($date_trans < 0) {
															echo '('.number_format(( $date_trans *-1),2).')';
													  } else echo number_format($date_trans,2); 
													?>
												</td>
												<?php }  ?> 
											...
											<tr>
												...
												<td class="text-right"><b>
													<?php if($total_trans < 0) {
															echo '('.number_format(( $total_trans *-1),2).')';
													  } else echo number_format($total_trans,2); 
													?>
												</b></td>
												<?php }  ?> 
