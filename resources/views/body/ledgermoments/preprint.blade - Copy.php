                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
									...
									<table class="table" border="0">
										...
										<body><?php $total_bal = $total_pdcr = $total_pdci = $net_bal = $total_cash = 0; ?>
											@foreach($reports as $report)
											<tr>
												<?php  //pdcr_amount_rv pdcr_amount_obt
													$total_bal += $report->cl_balance;
													$balance = $report->cl_balance + $report->pdcr_amount_rv - $report->pdci_amount_pv;
													$net_bal += $balance;
													if($type=='CUSTOMER') {
														$total_pdcr += $report->pdc_amount;
														//$cash = $report->pdcr_amount_rv - $report->pdc_amount + $report->op_balance;
														$cash = $report->cl_balance - $report->pdc_amount;

													} else {
														$total_pdci += $report->pdc_amount;
														//$cash = $report->pdci_amount_pv - $report->pdc_amount;
														$cash = $report->cl_balance - $report->pdc_amount;
													}
													$total_cash += $cash;
												?>
												...
												<td class="text-right">
													<?php if($cash < 0) {
															echo '('.number_format(( $cash *-1),2).')';
													  } else echo number_format($cash,2); 
													?>
												</td>
												<?php if($type=='CUSTOMER') { ?>
												<td class="text-right">
													<?php if($report->pdc_amount < 0) {
															echo '('.number_format(( $report->pdc_amount *-1),2).')';
													  } else echo number_format($report->pdc_amount,2); 
													?>
												</td>
											...
											<tr>
												...
												<td class="text-right"><b>
													<?php if($total_cash < 0) {
															echo '('.number_format(( $total_cash *-1),2).')';
													  } else echo number_format($total_cash,2); 
													?>
												</b></td>
											</tr>
										</body>
