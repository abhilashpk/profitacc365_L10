                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
							...
							<?php if($printid) { ?>
								@can('ce-print')
								 <a href="{{ url('customer_enquiry/print/'.$printid) }}" target="_blank" class="btn btn-info btn-sm">
                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmQuotationSales" enctype="multipart/form-data"  id="frmQuotationSales" action="{{ url('customer_enquiry/save') }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control <?php if($errors->has('reference_no')) echo 'form-error';?>" id="reference_no" autocomplete="off" name="reference_no" value="{{ ($lrow)?$lrow->lead_no:old('reference_no') }}" placeholder="Reference No.">
								...
								<!-- <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										 ...
										 <input type="text" class="form-control pull-right" value="{{(old('voucher_date')=='')?date('d-m-Y'):old('voucher_date')}}"  name="voucher_date" autocomplete="off" data-language='en' id="voucher_date" placeholder="{{date('d-m-Y')}}"/> 
                                    </div>
                                </div> -->
								...
								<div class="form-group">
                                   <font color="#16A085"> <label for="input-text" class="col-sm-2 control-label <?php if($errors->has('customer_name')) echo 'form-error';?>"><b>Customer</b></label></font>
                                    <div class="col-sm-10">
                                        <input type="text" name="customer_name" id="customer_name" class="form-control <?php if($errors->has('customer_name')) echo 'form-error';?>" value="{{ ($lrow)?$lrow->master_name:old('customer_name') }}" autocomplete="off" data-toggle="modal" data-target="#customer_modal" placeholder="Customer">
										<input type="hidden" name="customer_id" id="customer_id" value="{{ ($lrow)?$lrow->customer_id:old('customer_id') }}">
									</div>
                                </div>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="salesman" id="salesman" class="form-control" value="{{ ($lrow)?$lrow->name:old('salesman') }}" autocomplete="off" data-toggle="modal" data-target="#salesman_modal" placeholder="Salesman">
										<input type="hidden" name="salesman_id" id="salesman_id" value="{{ ($lrow)?$lrow->salesman_id:old('salesman_id') }}">
									</div>
                                </div>
								<?php } else { ?>
									...
									<?php if(old('item_code')) { $i = 0; 
										foreach(old('item_code') as $product) { $j = $i+1;?>
											<div class="itemdivChld">
                                            	...
                                            	<?php } else { ?>
								<input type="hidden" name="more_info_id" id="more_info_id">
								<?php } ?>
								<?php if($formdata['desc_item']==1) { ?>
								...
								<?php if($formdata['sales_item']==1) { ?>
												...
												<?php } else { ?>
								<input type="hidden" name="sales_item_id" id="sales_item_id">
								<?php } ?>
								<?php if($formdata['loc_item']==1) { ?>	
												<div id="loc">
													<button type="button" id="loc_{{$j}}" class="btn btn-primary btn-xs loc-info">Location</button>
												</div>
												<?php } else { ?>
								<input type="hidden" name="loc_item_id" id="loc_item_id">
								<?php } ?>	
							...
							<div id="showmenu">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<a href="{{ url('customer_enquiry') }}" class="btn btn-danger">Cancel</a>
										<a href="{{ url('customer_enquiry/add') }}" class="btn btn-warning">Clear</a>
										@can('ce-history')
...
$(document).ready(function () { 
	...
	var urlcode = "{{ url('customer_enquiry/checkrefno/') }}";
	var urlvchr = "{{ url('customer_enquiry/checkvchrno/') }}"; //CHNG
	
	@if($formdata['item_import']==0)
...
$(function() {	
	...
	var custurl = "{{ url('sales_order/customer_data/') }}";//customer_enquiry
	...
	var supurl = "{{ url('customer_enquiry/salesman_data/') }}";
    ...
    $(document).on('click', '#importFile', function(e) { 
        ...
        if($('#import_file').val()=='') {
			...
			$('#frmQuotationSales').attr('action', "{{url('customer_enquiry/import')}}").submit();
		}
