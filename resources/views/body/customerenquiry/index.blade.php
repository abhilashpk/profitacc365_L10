...
@section('content')
    <!-- Content Header (Page header) -->
                ...
                <div class="panel panel-info">
                    <div class="panel-heading clearfix">
                        ...
                        <div class="pull-right">
							@can('ce-create')
                             <a href="{{ url('customer_enquiry/add') }}" class="btn btn-primary btn-sm">
		...
		<section class="content">
			<form class="form-horizontal" role="form" method="POST" name="frmQReport" id="frmQReport" action="{{ url('customer_enquiry/search') }}" target="_blank">
...
function funDelete(id,no) {
	if(no=='0') {
		...
		if(con==true) {
			var url = "{{ url('customer_enquiry/delete/') }}";
...
$(function() {
		...
		var dtInstance = $("#tablePordersList").DataTable({
			...
			"searching": true,
			"ajax":{
					 "url": "{{ url('customer_enquiry/paging/') }}",
					 "dataType": "json",
					 "type": "POST",
			...
			@can('ce-edit'){ "data": "edit","bSortable": false },@endcan
			@can('ce-print'){ "data": "print","bSortable": false },@endcan
			{ "data": "open","bSortable": false },
			@can('ce-delete'){ "data": "delete","bSortable": false },@endcan
		]	
		...
		$('#select23').on('change', function(e){
			    ...
			    console.log(cust_id);
			     if(cust_id !=''){
			    $.get("{{ url('customer_enquiry/getjob/') }}/" + cust_id, function(data) {
