    ...
    <!-- Content Header (Page header) -->
        <section class="content-header">
            ...
            <ol class="breadcrumb">
				...
				<li>
                    <a href="#">Daily Report Settings</a>
                </li>
            </ol>
        </section>
        <!--section ends-->
							...
							<div class="row">
								<div class="col-md-12">
									...
									<div class="panel-heading">
										<h3 class="panel-title"><i class="fa fa-fw fa-crosshairs"></i></h3>
									</div>
									<div class="panel-body">
										<form class="form-horizontal" role="form" method="POST" name="frmDesign" id="frmDesign" action="{{ url('daily_report_setting/update/') }}">
										   ...
										   <table id="user" class="table table-bordered table-striped m-t-8">
											<thead>
												...
												<tr>
													...
													<td><input type='checkbox' name='dailyid[]' class='chk-dailyid' value='{{$row->gid}}' {{($groups && in_array($row->gid, $groups))?'checked':''}} /></td>
												</tr>
												@if(isset($accounts[$row->gid]))
												@foreach($accounts[$row->gid] as $acrow)
												<tr>
													<td></td>
												    <td>{{$acrow->master_name }}</td>
													<td><input type='checkbox' name='acntid[]' class='chk-dailyid' value='{{$acrow->id}}' {{($accountdat && in_array($acrow->id, $accountdat))?'checked':''}}/></td>
													<td></td>
												</tr>
												@endforeach
												@endif
