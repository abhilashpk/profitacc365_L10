                    ...
                    <div class="row">
                        <div class="col-md-12">
							<table border="0" style="width:100%;">
								<tr><td align="center" colspan="3"><b style="font-size:30px;">{{Session::get('company')}}</b><br/>
						...
						?>
                        <div class="col-md-12">
							<div style="text-align:center;"><h5><p>Received with thanks from M/S. <b>{{$name}}</b> the sum of <b>Dhs. {{$amtwords}}</b> against invoice nos: {{$refnos}}.</p></h5></div>
							...
							<table class="table" border="1">
								...
								@foreach($invoicerow as $row)
									<tr>
										...
										<td>{{$row->reference}}</td>
										<td>{{($row->master_name=='PDC RECEIVED' || $row->master_name=='BANK')?$row->cheque_no:''}}</td>
										<td>{{($row->master_name=='PDC RECEIVED' || $row->master_name=='BANK')?date('d-m-Y',strtotime($row->cheque_date)):''}}</td>
										<td>{{($row->master_name=='PDC RECEIVED' || $row->master_name=='BANK')?$row->name:''}}</td>
