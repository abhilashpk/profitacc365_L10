                                ...
                                <div id="myTabContent" class="tab-content">
									<div class="tab-pane fade {{(Session::get('active')=='home') ? ' active in' : '' }}" id="home">
										...
										<div class="form-group">
											...
											<div class="col-sm-4">
												<input type="number" class="form-control" id="rent_total" step="any" name="rent_total" value="{{$crow->total_rent}}" autocomplete="off" placeholder="Total Rent">
											</div>
										</div>
										<div class="form-group">
										...
										<div class="form-group">
											...
											<hr/>
											 ...
											 <div class="form-group">
												...
												<div class="col-sm-2"><button type="button" class="btn btn-primary Reload">Reload</button></div>
											</div>
										...
										<fieldset>
											...
											<div class="col-xs-15" id="receipt_add">
												...
												<div class="col-xs-3" style="width:15%;">
													<span class="small">Debit A/c.1</span> 
												
												<input type="text" id="dracname_{{$i}}" value="{{$row->master_name}}" autocomplete="off" name="drac_name[]" class="form-control" data-toggle="modal" data-target="#accounts_modal">
									...
									<div class="tab-pane fade {{(Session::get('active')=='deposit') ? ' active in' : '' }}" id="deposit">
										...
										<div class="form-group">
											...
											@if($crow->renew_id=='' && $rdoly=='')
												...
												@endif
											@else
												@if($crow->renew_id!='' && isset($payacnts[1]) && $payacnts[1]->is_add==1 && $rdoly=='')
													@if($crow)
													<button type="submit" class="btn btn-primary">Update</button>
													@endif
													<a href="{{ url('contractbuilding') }}" class="btn btn-danger">Cancel</a>
											...
											<fieldset>
												...
												@foreach($orvs as $orow)
												  ...
												  <div class="itemdivChldDr">
													<div class="col-xs-15">
														<div class="col-xs-3" style="width:15%;">
															<span class="small">Debit A/c.</span> <input type="text" id="rvOdracname_{{$i}}" value="{{($orow)?$orow->master_name:(($rvrow)?$rvrow->cash:'')}}" autocomplete="off" name="drac_name[]" class="form-control" data-toggle="modal" data-target="#accounts_modal">
															<input type="hidden" id="rvOdracid_{{$i}}" name="drac_id[]" value="{{($orow)?$orow->account_id:(($rvrow)?$rvrow->cashid:'')}}" >
														</div>
														<div class="col-xs-3" style="width:10%;">
															<span class="small">Ref.No.</span> 
														...
														<div class="col-xs-2" style="width:10%;">
															<span class="small">Amount</span> <input type="number" id="drdamount_{{$i}}" {{$rly}} placeholder="Amount" value="{{$orow->amount}}" autocomplete="off" step="any" name="Damount[]" class="form-control">
														</div>
															...
															<div class="form-group" style="margin-bottom: 1px;">
																...
																<div class="col-xs-15 divchq">
																	<div class="col-xs-2" style="width:25%;">
																		<span class="small">Reference</span> <input type="text" id="crref_{{$i}}" name="reference[]" value="{{$orow->reference}}" autocomplete="off" class="form-control  desc-bill" data-toggle="modal" data-target="#reference_modal">
																	</div>
																	<div class="col-xs-2" style="width:25%;">
																		<span class="small">Description</span> 
																		<div id="refdata_{{$i}}" class="refdata">
																			<input type="text" id="crdescr_{{$i}}" value="{{$orow->description}}" autocomplete="off" name="description[]" class="form-control">
																		</div>
																	...
																	<div class="col-xs-2" style="width:20%;">
																		<span class="small">Amount</span> <input type="number" id="ramount_{{$i}}" value="{{$orow->amount}}" autocomplete="off" step="any" name="line_amount[]" class="form-control orvAmt">
																	</div>
																</div>	
															</div>
														</div>
...
$(document).on('click', '.rvadd', function(e) { //NOV22
	...
	$.ajax({
		...
		type: 'get',
		data: 'cid='+cid+'&amount='+amt+'&inst='+inst+'&cac='+cac+'&cacid='+cacid+'&ref='+encodeURIComponent(ref)+'&dec='+encodeURIComponent(dec),
		success: function(data) { 
...
$(document).on('click', '#accounts_data .accountRow', function(e) {
	var num = $('#num').val();  
