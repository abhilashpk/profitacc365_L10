                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
							<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmQuotation" id="frmQuotation" enctype="multipart/form-data" action="{{ url('quotation/save') }}">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
														
								<div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label <?php if($errors->has('voucher_no')) echo 'form-error';?>">QP. No.</label>
                                    <div class="col-sm-10">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										<input type="text" class="form-control" id="quotationp_id" readonly name="quotationp_id" placeholder="Copy from existing Quotation" autocomplete="off" onclick="getQuotation()">
                                    </div>
                                </div>
								<?php } else { ?>
									<input type="hidden" name="quotationp_id" id="quotationp_id">
								<?php } ?>
...
$(document).ready(function () { 
	...
	function getLineTotal(n) {
		...
		console.log('lineQuantity '+taxLineCost);
...
$(function() {	
    ...
    $(document).on('click', '#importFile', function(e) { 
		if($('#input-23').val()=='') {
			...
			$('#frmQuotation').attr('action', "{{url('quotation/import')}}").submit();
		}
	});
...
function getEnquiry() { 
	...
	var wt = $(window).width();
	var supplier_id = $("#supplier_id").val();
	var pourl = "{{ url('material_requisition/get_enquiry/') }}/"+supplier_id+"/PO";
...
function getQuotation() { 
	...
	var ht = $(window).height();
	var wt = $(window).width();
	var pourl = "{{ url('quotation/get_quotations/') }}";
	popup = window.open(pourl, "Popup", "width=900,height=500,top=100,left=200");
	popup.focus();
	return false
}
