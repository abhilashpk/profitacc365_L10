        ...
        <section class="content p-l-r-15" id="invoice-stmt">
            <div class="panel">
                ...
                <div class="panel-body" style="width:100%; !important; border:0px solid red;">
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										<td colspan="2" align="center">
											...
											<body>
											<?php $total_net = $net_amount = $total_vat = $total_gross = $i =$total_amt=$total_netamt= 0;?>
												...
												<?php $i++;
													  ...
													  $total_amt=$total-($row->discount);
													  $total_netamt+=$total_amt;
												...
												<tr>
													...
													<td class="text-right">{{number_format($row->discount,2)}}</td>
													<td class="text-right"><b>{{number_format($total_netamt,2)}}</b></td>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										<td colspan="2" align="center">
											...
											<body>
											<?php 		
												$total_net = $net_amount = $total_vat = $total_gross = $i =$total_amt=$total_netamt= 0;?>
												@foreach($reports as $row)
											<?php $i++;
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										...
										<table class="tblstyle table-bordered" width="100%">
											...
											<tr>
												<td><b>SI.No:</b> {{ ++$i }}</td>
												<td><b>QP#:</b> {{$report[0]->voucher_no}}</td>
												<td><b>QP Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
												<td><b>QP Ref.#:</b> {{$report[0]->reference_no}}</td>
											...
											<tr>
												...
												<td class="text-right"><b>QP.Qty.</b></td>
											...
											<?php $qty_total = $net_total = $vat_total = $gross_total =$total_disc=$total_netamt= 0; ?>
												@foreach($report as $row)
											<?php 
												  $unit_price = ($cur=='')?$row->unit_price:($row->unit_price / $row->currency_rate);
												  $total_price = $row->quantity * $unit_price;
												  $total_amt = ($row->quantity * $unit_price)-($row->discount);
												  $unit_vat = ($cur=='')?$row->unit_vat:($row->unit_vat / $row->currency_rate);
												  $qty_total += $row->quantity;
												  $gross_total += $total_price; //$row->total_price;
												  $net_amount = $unit_vat + $total_amt;
												  $vat_total += $unit_vat;
											...
											?>
												...
												<tr>
													...
													<td class="text-right"><b>{{$total_netamt}}</b></td>
					...
					<form class="form-horizontal" role="form" method="POST" name="frmExport" id="frmExport" action="{{ url('quotation/export') }}">
