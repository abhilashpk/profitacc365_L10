								...
								<tbody id="bod">
									...
									<th class="text-right">SI.Date</th>
										...
										<td class="text-right">{{date('d-m-Y',strtotime($row->voucher_date))}}</td>
										 ...
										 <td class="text-right"><b>{{$row->customer}}</b></td>
										
											<td class="text-right"><b>{{number_format((($row->disc > 0)?($netsale+$row->disc):$row->subtotal),2)}}</b></td>
											<td class="text-right"><b>{{number_format($row->disc,2)}}</b></td>
											<td class="text-right"><b>{{number_format($netsale,2)}}</b></td>
											<td class="text-right"><b>{{number_format($row->vat_total,2)}}</b></td>
											<td class="text-right"><b>{{number_format($row->net_total,2)}}</b></td>
										</tr>
										<?php if($row->disc > 0)
									...
									<td></td><td></td>
											...
											<td class="text-right"><b>{{number_format($saletotal,2)}}</b></td>
								...
								<tbody id="bod">
									<tr style="border:0px solid black;">
										...
										<table class="tblstyle table-bordered" width="100%">
											...
											@php $i++; @endphp
											<tr>
												<td><b>SI.No:</b> {{$i}} &nbsp; <b>SI.#:</b> {{$report[0]->voucher_no}} &nbsp; <b>SI Date</b> {{date('d-m-Y', strtotime($report[0]->voucher_date))}}</td>
												<td><b>SI.#:</b> {{$report[0]->voucher_no}}</td>
											...
											<?php $line_total=$net_subtotal=$vat_subtotal=0;?>
												...
												<tr>
													...
													<td class="text-right"></td>
													<td class="text-right"><b>{{number_format($line_total,2)}}</b></td>
													<td class="text-right"><b>{{number_format($vat_subtotal,2)}}</b></td>
													<td class="text-right"><b>{{number_format($net_subtotal,2)}}</b></td>
												</tr>
										...
										<tbody id="bod">
											<tr style="border:0px solid black;">
												...
												<table border="0" style="width:100%;" class="tblstyle table-bordered">
													...
													@php $nettotal=$netdiscount=$netvat_amount=$net_amount_total=0; @endphp
														<tr><td colspan="9"><b>{{$report[0]->voucher_name}}</b></td></tr>
														@foreach($report as $row)
														<tr>
															<td>{{ ++$i }}</td>
															<td>{{ $row->voucher_no }}</td>
															...
															<td>{{$row->vat_no}}</td>
															<td>{{$row->salesman}}</td>
															<td class="text-right">{{number_format(($row->discount > 0)?($row->subtotal+$row->discount):$row->subtotal,2)}}</td>
															<td class="text-right">{{number_format($row->discount,2)}}</td>
															<td class="text-right">{{number_format($row->vat_amount,2)}}</td>
															<td class="text-right">{{number_format($row->net_total,2)}}</td>
														</tr>
