                        ...
                        <div class="panel-body">
                            ...
                            <form class="form-horizontal" role="form" method="POST" name="frmSalesInvoice" id="frmSalesInvoice" action="{{ url('sales_invoice/update/'.$orderrow->id) }}">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										<div class="col-xs-8">
											<input type="hidden" step="any" class="form-control spl" id="les_amt_desc" name="less_description" value="{{(old('less_description'))?old('less_description'):$orderrow->less_description}}" placeholder="Description" autocomplete="on">
										</div>
										...
										<div class="col-xs-2">
											<input type="number" step="any" id="less_amountfc" name="less_amountfc" value="{{(old('less_amount'))?old('less_amount'):$orderrow->less_amount}}" class="form-control spl" placeholder="0">
										</div>
									</div>
                                </div>
								<?php } else { ?>
								...
								<div class="form-group">
                                    ...
                                    <div class="col-xs-10">
										<div class="col-xs-8">
											<input type="hidden" step="any" class="form-control spl" id="les_amt_desc2" name="less_description2" value="{{(old('less_description2'))?old('less_description2'):$orderrow->less_description2}}" placeholder="Description" autocomplete="on">
										</div>
										...
										<div class="col-xs-2">
											<input type="number" step="any" id="less_amount2fc" name="less_amount2fc" value="{{(old('less_amount2'))?old('less_amount2'):$orderrow->less_amount2}}" class="form-control spl lesamt" placeholder="0">
										</div>
									</div>
                                </div>
								<?php } else { ?>
...
$(document).ready(function () { 
	...
	$('.custom_icheck').on('ifChecked', function(event){ 
		$("#currency_id").prop('disabled', false);
		$("#currency_rate").prop('disabled', false);
		$("#fc_label").toggle(); $("#c_label").toggle();$("#total_fc").toggle(); $("#discount_fc").toggle();$("#vat_fc").toggle(); $("#less_amountfc").toggle(); $("#less_amount2fc").toggle(); $("#net_amount_fc").toggle();
	});
	$('.custom_icheck').on('ifUnchecked', function(event){ 
		$("#currency_id").prop('disabled', true);
		$("#currency_rate").prop('disabled', true);
		$("#fc_label").toggle(); $("#c_label").toggle();$("#total_fc").toggle(); $("#discount_fc").toggle(); $("#less_amountfc").toggle(); $("#less_amount2fc").toggle();  $("#vat_fc").toggle();$("#net_amount_fc").toggle();
	});
	...
	$(document).on('keyup', '#less_amount', function(e) { 
			console.log('1');
			if( $('#is_fc').is(":checked") ) { 
				var rate       = parseFloat($('#currency_rate').val()); console.log('r:'+rate);
				$('#less_amountfc').val( $(this).val() * rate ); console.log('v:'+$(this).val() * rate);
