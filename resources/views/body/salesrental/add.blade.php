        ...
        <section class="content">
            <div class="row">
				...
				<div class="alert alert-warning">
					<p>
						Sales Rental voucher is not found! Please create a voucher in Account Settings.
					</p>
				</div>
				<?php } else { ?>
                    <div class="panel panel-primary">
                        <div class="panel-heading clearfix">
                            <h3 class="panel-title pull-left m-t-6">
                                <i class="fa fa-fw fa-crosshairs"></i> New Sales Rental
                            </h3>
                        ...
                        <div class="panel-body">
							<div class="controls"> 
                            <form class="form-horizontal" role="form" method="POST" name="frmSalesInvoice" id="frmSalesInvoice" action="{{ url('sales_rental/save') }}">
							...
							<input type="hidden" name="is_rental" id="is_rental" value="1">
								...
								<div class="form-group">
                                    ...
                                    <select id="voucher_id" class="form-control select2" style="width:100%" name="voucher_id">
											...
											<?php if($svchrid) { ?>
												<option value="{{ $voucher['id'] }}" <?php if($svchrid==$voucher['id']) echo 'selected'; ?>>{{ $voucher['voucher_name'] }}</option>
											<?php } else { ?>
												<option value="{{ $voucher['id'] }}" <?php if($svchrid==$voucher['id']) echo 'selected'; ?>>{{ $voucher['voucher_name'] }}</option>
											<?php } ?>
											@endforeach
							...
							<div class="form-group">
                                     <font color="#16A085"> <label for="input-text" class="col-sm-2 control-label"><b>SRL. No.</b></label></font>
								...
								<div class="form-group">
                                    <label for="input-text" class="col-sm-2 control-label">SRL. Date</label>
                                    <div class="col-sm-10">
								...
								<div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<input type="text" name="sales_account" id="sales_account" class="form-control" value="{{ (old('sales_account'))?old('sales_account'):$vouchers[0]['master_name'] }}" readonly>
											<?php } ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
										...
										<a href="{{ url('sales_rental/add') }}" class="btn btn-warning">Clear</a>
										@can('si-history')
...
$(document).ready(function () { 
	...
	var urlvchr = "{{ url('sales_rental/checkvchrno/') }}"; //CHNG
...
$(function() {	
	...
	$('#sales_location').on('change', function(e){
		...
		$.get("{{ url('sales_rental/getsaleloc/') }}/" + loc_id, function(data) {
			if(data) {
...
//	$('#department_id').on('change', function(e){ 
	...
	//	dptTxt = $( "#department_id option:selected" ).text();
	//	console.log(dept_id);
	//	$.get("{{ url('sales_rental/getdeptvoucher/') }}/" + dept_id, function(data) { 
	//	
...
//	});
	...
	var ordhisurl = "{{ url('sales_rental/order_history/') }}";
	...
	$('#document_type').on('change', function(e){
		...
		$.ajax({
			url: "{{ url('sales_rental/set_session/') }}",
			type: 'post',
	...
	$(document).on('focus', '#customer_trn', function(e) { 
		...
		$.get("{{ url('sales_rental/get_trnno/') }}/" + name, function(data) { //console.log(data);
