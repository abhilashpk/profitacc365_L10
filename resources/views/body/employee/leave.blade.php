                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            <h3 class="panel-title">
                                <i class="fa fa-fw fa-crosshairs"></i> Leave Process
                            </h3>
                           
                        </div>
                        <div class="panel-body">
                            <form class="form-horizontal" role="form" method="POST" name="frmLeave" id="frmLeave" action="{{ url('employee/save_leave') }}">
								...
								<?php
									if($masterrow->rejoin_date=='0000-00-00')
										$joindate = ($masterrow->join_date=='0000-00-00')?'':date('d-m-Y', strtotime($masterrow->join_date));
									else
										$joindate = ($masterrow->rejoin_date=='0000-00-00')?'':date('d-m-Y', strtotime($masterrow->rejoin_date));
								?>
								<div class="form-group">
                                    <label for="input-text" class="col-sm-3 control-label">Joining/Last Rejoining Date</label>
                                    <div class="col-sm-9">
                                        <input type="text" class="form-control" readonly id="rejoin_date" name="rejoin_date" value="{{$joindate}}">
                                    </div>
                                </div>
								...
								<div class="form-group">
                                    <label for="input-text" class="col-sm-3 control-label">Alloted Leave Days</label>
                                    <div class="col-sm-6">
                                        <input type="number" class="form-control" step="any" id="alo_leave_days" name="alo_leave_days">
                                    </div>
								...
								<div class="form-group">
                                    <label for="input-text" class="col-sm-3 control-label">Paid Leave Salary</label>
                                    <div class="col-sm-9">
                                        <input type="number" class="form-control" step="any" id="paid_leave_salary" name="paid_leave_salary">
                                    </div>
                                </div>
...
$(document).ready(function () {
    ...
    $('#frmLeave').bootstrapValidator({
        ...
        $('#frmLeave').data('bootstrapValidator').resetForm();
    });
...
$(function() {	
	...
	$(document).on('click', '.apply', function(e) {
		...
		if($('#start_date').val()=='') {
			...
			var To = moment($("#rejoin_date").val(), "dd-MM-YYYY");
			var From = moment($('#start_date').val(), "dd-MM-YYYY");
			var mth = parseFloat( moment.duration(From.diff(To)).asMonths()).toFixed(); //console.log('mt'+mth);
			...
			$('#frmLeave').bootstrapValidator('revalidateField', 'start_date');
			$('#frmLeave').bootstrapValidator('revalidateField', 'cal_leave_days');
			//console.log(mth);
		}
