...
class StiMySqlAdapter {
	...
	private function connect() {		 
		...
		// Override with Laravel's database credentials
		...
		$this->info->password = getenv('DB_PASSWORD') !== false ? getenv('DB_PASSWORD') : '';
...
public function getValue($type, $value) {
    switch ($type) {
        ...
        case 'datetime':
            // FIX: Handle zero dates and empty values
            if (strlen($value) == 0 || $value == '0000-00-00' || $value == '0000-00-00 00:00:00') {
                return null;
            }
            ...
            return date("Y-m-d\TH:i:s.v", $timestamp);
        
        case 'time':
            // FIX: Handle zero times and empty values
            if (strlen($value) == 0 || $value == '00:00:00' || $value == '0000-00-00 00:00:00') {
                return null;
            }
            ...
            $hours = intval($value);
            return $hours >= 0 && $hours <= 23 ? date("H:i:s.v", $timestamp) : $value;
    }
