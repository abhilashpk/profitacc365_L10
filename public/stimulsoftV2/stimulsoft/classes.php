...
class StiRequest {
    ...
    public $escapeQueryParameters = null;
    public $isWizardUsed = null;
    ...
    public function parse() {
        ...
        $parameterNames = array(
            'sender', 'event', 'command', 'connectionString', 'queryString',
            'database', 'dataSource', 'connection', 'timeout', 'data',
            'fileName', 'action', 'printAction', 'format', 'formatName',
            'settings', 'variables', 'parameters',
            'escapeQueryParameters', 'isWizardUsed'
        );
        ...
        if (!isset($obj->event) && isset($obj->command) &&
            ($obj->command == StiCommand::TestConnection || $obj->command == StiCommand::ExecuteQuery)
        ) {
...
class StiResult {
    ...
    // ---- PHP 8.2 compatibility (no dynamic properties) ----
